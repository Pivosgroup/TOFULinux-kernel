...
static int __init mmc_lp_suspend_init(void)
{
	printk("DDR low power is ");

	if(aml_read_reg32(P_MMC_LP_CTRL1) & (1<<28))
	{
		printk("enable.\n");
...
late_initcall(mmc_lp_suspend_init);
