...
DEFINE_MUTEX(clock_ops_lock);
#ifdef CONFIG_SET_MALI_FREQ_EN
static unsigned int mali_max = CONFIG_MALI_MAX_FREQ;
#else 
...
long clk_round_rate(struct clk *clk, unsigned long rate)
{
	...
	/* Round down clock to 24MHz multiple. */
	if (rate < clk->min)
		rate = clk->min;
	else if (rate > clk->max)
		rate = clk->max;

	rate = (rate / 24000000) * 24000000;
	return rate;
}
...
uint32_t meson_get_cpu_ctrl_reg(void)
{
    uint32_t ret = 0;
...
static inline void meson_smp_init_transaction(void)
{
    ...
    for_each_online_cpu(cpu) {
        meson_set_cpu_status_reg(cpu, 0);
    }
}
...
static unsigned sys_pll_settings[][6] = {
	...
	{0x04237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1344
	{0x08237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1368
	{0x0c237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1392
	{0x44237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1416
	{0x48237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1440
	{0x4c237, M6_SYS_PLL_CNTL_2, M6_SYS_PLL_CNTL_3, M6_SYS_PLL_CNTL_4, 0},  // 1464
...
static int set_sys_pll(struct clk *clk, unsigned long src, unsigned long dst, unsigned * scale_divn)
{
	...
	if (cpu_clk_cntl != curr_cntl) {
		...
		if (src <= dst) {
			...
			// when decreasing frequency, lpj has not yet been adjusted
			do {
				udelay_scaled(100, src / 1000000, 24 /*clk_get_rate_xtal*/);
			} while ((aml_read_reg32(P_HHI_SYS_PLL_CNTL) & 0x80000000) == 0);
		}
#else
