...
static struct usb_request *adb_request_new(struct usb_ep *ep, int buffer_size)
{
	...
	//req->buf = get_zeroed_page(GFP_KERNEL);
	if (!req->buf) {
...
static int adb_setup(void)
{
	...
	if (!dev)
		return -ENOMEM;
	for(i = 0;i < TX_REQ_MAX + 1; i++){
		dev->req_buf[i] = kmalloc(ADB_BULK_BUFFER_SIZE,GFP_KERNEL);
	}
