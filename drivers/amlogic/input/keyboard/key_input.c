...
#define AML_RTC_READ(addr)  meson_smc2(addr-IO_AHB_BASE+IO_AHB_PHY_BASE)
#define AML_RTC_WRITE(addr, data)   meson_smc3((addr-IO_AHB_BASE+IO_AHB_PHY_BASE), data)
#else
...
#if MESON_CPU_TYPE >= MESON_CPU_TYPE_MESON6
	//aml_write_reg32(P_AO_RTC_ADDR1, (aml_read_reg32(P_AO_RTC_ADDR1) | (0x0000c000)));
	AML_RTC_READ(P_AO_RTC_ADDR1, (aml_read_reg32(P_AO_RTC_ADDR1) | (0x0000c000)));
#else    
    WRITE_AOBUS_REG(AO_RTC_ADDR1, (READ_AOBUS_REG(AO_RTC_ADDR1) | (0x0000c000)));
#endif    
#endif
...
static int key_input_resume(struct platform_device *dev)
{
    ...
    if((READ_AOBUS_REG(AO_RTI_STATUS_REG2) != CALL_FLAG) &&
        (READ_AOBUS_REG(AO_RTI_STATUS_REG2) != BT_WAKE_FLAG) &&
        (READ_AOBUS_REG(AO_RTI_STATUS_REG2) != WIFI_WAKE_FLAG))
        WRITE_AOBUS_REG(AO_RTI_STATUS_REG2, 0);
