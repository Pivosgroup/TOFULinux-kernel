...
static void remote_timer_sr(unsigned long data)
{
	...
	} else if (remote_data->work_mode == REMOTE_WORK_MODE_G19) {
		remote_data->repeate_flag = 0;
		remote_send_key(remote_data->input, (remote_data->savecur_keycode>>4)&0xf, 0);
	} else if (remote_data->work_mode == REMOTE_WORK_MODE_RCA) {
		remote_send_key(remote_data->input, ~(remote_data->cur_keycode>>16)&0xff, 0);
	} else {
		if(remote_data->bit_count == 16)
			remote_send_key(remote_data->input, remote_data->cur_lsbkeycode & 0xff, 0);
		else
			remote_send_key(remote_data->input, (remote_data->cur_keycode >> 16) & 0xff, 0);
		repeat_flag = 0;
	}
...
static void remote_fiq_interrupt(void)
{
	...
	} else if (gp_remote->work_mode == REMOTE_WORK_MODE_G19) {
		remote_g19_reprot_key((unsigned long)gp_remote);
...
static int hardware_init(struct platform_device *pdev)
{
	...
	g_remote_base = remote_pdata->ao_baseaddr;
...
void config_windows_g19(){
	int i = 0;
	gp_remote->time_window[0] = 0x4f;
	gp_remote->time_window[1] = 0x51;
	gp_remote->time_window[2] = 0x3a;
	gp_remote->time_window[3] = 0x3c;
	gp_remote->time_window[4] =0x48;
	gp_remote->time_window[5] =0x4b;
	gp_remote->time_window[8] = 0x56;
	gp_remote->time_window[9] = 0x5a;
	gp_remote->time_window[10] = 0x65;
	gp_remote->time_window[11] = 0x69;
	gp_remote->time_window[6] =0xfff;
	gp_remote->time_window[7] =0xfff;
	for(i = 0 ;i<=11;i++)
		printk("gp_remote->time_window[%d] = %d\n",i,gp_remote->time_window[i]);
...
static int work_mode_config(unsigned int cur_mode)
{
	...
	case REMOTE_WORK_MODE_RCA:
		...
		} else if (cur_mode == REMOTE_WORK_MODE_G19) {
			config_windows_g19();
			gp_remote->fiq_handle_item.handle = remote_g19_bridge_isr;
		}else {
