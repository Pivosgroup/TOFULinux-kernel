...
void axp20_dcdc_voltage(int dcdc, int target) 
{
    ...
    axp_read(gcharger->master, addr, &val);
    idx_cur = val & mask;
    while (idx_cur != idx_to) {
        ...
        val &= ~mask;
        val |= (idx_cur);
        axp_write(gcharger->master, addr, val);
...
{
    uint8_t data;
    axp_read(gcharger->master, 0x2f, &data);
