...
#include <linux/mutex.h>
#include <linux/amports/amvideocap.h>
...
static int __init amvideocap_init(void)
{
    int ret = 0;
    printk("amvideocap_init\n");
    ret = amvideocap_dev_register(0, 0);
    printk("amvideocap_init,%d\n", ret);
    return ret;
}
static void __exit amvideocap_exit(void)
{
    printk("amvideocap_exit");
    amvideocap_dev_unregister();
    return ;
}

module_init(amvideocap_init);
module_exit(amvideocap_exit);
