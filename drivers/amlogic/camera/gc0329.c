    ...
    if (plat_dat&&plat_dat->custom_init_script) {
        ...
        while(1)
        {
            ...
            if (custom_script[i].val==0xff&&custom_script[i].addr==0xff){
                printk("GC0329_write_custom_regs success in initial GC0329.\n");
                break;
            }
            if((i2c_put_byte_add8(client,buf, 2)) < 0){
                printk("fail in initial GC0329 custom_regs. \n");
                return;
            }
            i++;
        }
			...
			const struct i2c_device_id *id)
{
	...
	if(err<0 || err != 0xc0) {
		printk("can not get gc0329 id, id=%x\n", err);
		return  -ENODEV;
	}
	printk("gc0329 id is %x\n", err);
#endif
