...
static int gc2015_frame_rate = 15;
...
void gc2015_init_regs(struct gc2015_device *dev)
{
    ...
    if (plat_dat&&plat_dat->custom_init_script) {
		...
		while(1)
		{
			...
			{
				printk("gc2015_write_custom_regs success in initial gc2015.\n");
				break;
			}
			if((i2c_put_byte_add8(client,buf, 2)) < 0)
			{
				printk("fail in initial gc2015 custom_regs. \n");
				return;
			}
			i++;
		}
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.frame_rate = gc2015_frame_rate;
	...
	printk("gc2015,h=%d, v=%d, frame_rate=%d\n", 
		gc2015_h_active, gc2015_v_active, gc2015_frame_rate);
