...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.fmt_info.h_active = sp2518_h_active;
	para.fmt_info.v_active = sp2518_v_active;
...
static int sp2518_open(struct file *file)
{
	...
	fh->fmt      = &formats[0];
	fh->width    = 800;//640//zyy
	fh->height   = 600;//1200;//480
	fh->stream_on = 0 ;
			...
			const struct i2c_device_id *id)
{
	...
	chid_id=i2c_get_byte_add8(client,0x02);//chip id is 0x83
	plat_dat->device_uninit();
	printk("sp2518 chid_id=0x%x\n", chid_id);
	if(chid_id != 0x53)
	{
