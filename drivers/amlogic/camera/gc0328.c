...
void GC0328_init_regs(struct gc0328_device *dev)
{
    ...
    if (plat_dat&&plat_dat->custom_init_script) {
        custom_script = (aml_camera_i2c_fig1_t*)plat_dat->custom_init_script;
        i=0;
        while(1)
        {
            ...
            if (custom_script[i].val==0xff&&custom_script[i].addr==0xff){
                printk("GC0328_write_custom_regs success in initial GC0328.\n");
                break;
            }
            if((i2c_put_byte_add8(client,buf, 2)) < 0){
                printk("fail in initial GC0328 custom_regs. \n");
                return;
            }
            i++;
        }
