...
void NT99340_init_regs(struct nt99340_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    int i=0;
    ...
    if (plat_dat&&plat_dat->custom_init_script) {
		...
		while(1)
		{
			...
			{
				printk("NT99340_write_custom_regs success in initial NT99340.\n");
				break;
			}
			if((i2c_put_byte(client,custom_script[i].addr, custom_script[i].val)) < 0)
			{
				printk("fail in initial NT99340 custom_regs. \n");
				return;
			}
			i++;
		}
