...
void SP0A19_init_regs(struct sp0a19_device *dev)
{
    ...
    while(1)
    {
        buf[0] = SP0A19_script[i].addr;//(unsigned char)((SP0A19_script[i].addr >> 8) & 0xff);
        //buf[1] = (unsigned char)(SP0A19_script[i].addr & 0xff);
    ...
    if (plat_dat&&plat_dat->custom_init_script) {
        ...
        while(1)
        {
            ...
            if (custom_script[i].val==0xff&&custom_script[i].addr==0xff){
                printk("SP0a19_write_custom_regs success in initial sp0a19.\n");
                break;
            }
            if((i2c_put_byte_add8(client,buf, 2)) < 0){
                printk("fail in initial sp0a19custom_regs. \n");
                return;
            }
            i++;
        }
