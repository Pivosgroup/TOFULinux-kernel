    ...
    if (plat_dat&&plat_dat->custom_init_script) {
        ...
        while(1)
        {
            if (custom_script[i].val==0xff&&custom_script[i].addr==0xffff){
                printk("OV3660_write_custom_regs success in initial OV3660.\n");
                break;
            }
            do {
                if((i2c_put_byte(client, custom_script[i].addr, custom_script[i].val)) < 0){
                	printk("fail in initial OV3660 custom_regs. \n");
                	return;
                }
            }while (i2c_get_byte(client, custom_script[i].addr) != custom_script[i].val);
            i++;
        }
    }
