	...
	if (plat_dat&&plat_dat->custom_init_script) {
		...
		while(1) {
			if (custom_script[i].val==0xff&&custom_script[i].addr==0xffff) {
				printk("OV5642_write_custom_regs success in initial OV5642.\n");
				break;
			}
			if((i2c_put_byte(client,custom_script[i].addr, custom_script[i].val)) < 0)
			{
				printk("fail in initial OV5642 custom_regs. \n");
				return;
			}
			i++;
		}
...
static int set_flip(struct ov5642_device *dev)
{
	...
	temp &= ~(dev->platform_dev_data.mirror_flip << 6 );
	temp |= (dev->platform_dev_data.vertical_flip << 5);
	printk("0x3818 dst is %x\n", temp);
	i2c_put_byte(client, 0x3818, temp);
	temp = i2c_get_byte(client, 0x3621);
	printk("0x3621 src is %x\n", temp);
	temp |= dev->platform_dev_data.mirror_flip << 5;
        	...
        	const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		t->platform_dev_data.mirror_flip = plat_dat->mirror_flip;
		t->platform_dev_data.vertical_flip = plat_dat->vertical_flip;
