...
void vdin_set_meas_mux(unsigned int offset, enum tvin_port_e port_)
{
	...
	WRITE_CBUS_REG_BITS(VPP_VDO_MEAS_CTRL,
	...
	WRITE_CBUS_REG_BITS(VPP_VDO_MEAS_CTRL, 1, VDO_MEAS_RST_BIT, VDO_MEAS_RST_WID);
	WR_BITS(VDIN_MEAS_CTRL0,0, MEAS_RST_BIT, MEAS_RST_WID);
	WRITE_CBUS_REG_BITS(VPP_VDO_MEAS_CTRL,0, VDO_MEAS_RST_BIT, VDO_MEAS_RST_WID);
}
	...
	switch ((port)>>8)
	{
		...
		case 0x80: // dvin
			...
			WR_BITS(VDIN_ASFIFO_CTRL3, 0xe2, VDI7_ASFIFO_CTRL_BIT, VDI7_ASFIFO_CTRL_WID);
                        break;
		case 0x100://viu in mybe need modify base on truth
...
inline void vdin_set_hscale(unsigned int offset, unsigned int src_w, unsigned int dst_w)
{
	...
	WRITE_CBUS_REG_BITS(VDIN_SC_MISC_CTRL + offset, 0, PRE_HSCL_EN_BIT, PRE_HSCL_EN_WID);
	WRITE_CBUS_REG_BITS(VDIN_SC_MISC_CTRL + offset, 0, HSCL_EN_BIT, HSCL_EN_WID);
	//write horz filter coefs
	WRITE_CBUS_REG (VDIN_SCALE_COEF_IDX + offset, 0x0100);
	for (i = 0; i < 33; i++)
	{
		WRITE_CBUS_REG(VDIN_SCALE_COEF + offset, filt_coef0[i]); //bicubic
	}

	horz_phase_step = (src_w << 20) / dst_w;
	horz_phase_step = (horz_phase_step << 4);

	WRITE_CBUS_REG(VDIN_WIDTHM1I_WIDTHM1O + offset, ((src_w - 1) << WIDTHM1I_BIT) |
			(dst_w  - 1)
		      );

	WRITE_CBUS_REG(VDIN_HSC_PHASE_STEP + offset, horz_phase_step);
	WRITE_CBUS_REG(VDIN_HSC_INI_CTRL + offset, (1 << HSCL_RPT_P0_NUM_BIT) |//hsc_p0_num
			( 4 << HSCL_INI_RCV_NUM_BIT) |//hsc_ini_rcv_num
			( 0 << HSCL_INI_PHASE_BIT)//hsc_ini_phase
		      );

	WRITE_CBUS_REG(VDIN_SC_MISC_CTRL + offset,
...
inline void vdin_set_hvscale(struct vdin_dev_s *devp)
{
        ...
        if (((devp->scaler4w < devp->h_active) && (devp->scaler4w != 0)) ||
            ...
            if ((devp->h_active > TVIN_MAX_HACTIVE)) {
