...
{   
    int pos=0;
    pos += snprintf(buf+pos, PAGE_SIZE, "3D status: %s\r\n", hdmitx_device.tx_3d_st);
...
static ssize_t store_config(struct device * dev, struct device_attribute *attr, const char * buf, size_t count)
{
    if(strncmp(buf, "force", 5)==0){
        ...
        if(len < 16)    // store 3d config
            strcpy(hdmitx_device.tx_3d_st, buf);
        else {
            strncpy(hdmitx_device.tx_3d_st, buf, 15);
            hdmitx_device.tx_3d_st[15] = '\0';
        }
    }
...
static int amhdmitx_probe(struct platform_device *pdev)
{
    ...
    if(hdmi_pdata && hdmi_pdata->vend_data)
        hdmitx_device.vendor_data = hdmi_pdata->vend_data;
