...
#endif
{
    ...
    hdmi_print(1,"Reg0x196 = 0x%x  Reg0x80 = 0x%x  Reg0x26 = 0x%x\n", hdmi_rd_reg(TX_HDCP_ST_EDID_STATUS), hdmi_rd_reg(TX_CORE_EDID_CONFIG_MORE), hdmi_rd_reg(TX_HDCP_EDID_CONFIG));
...
//#ifdef AML_A3
    ...
    hdmi_wr_only_reg(OTHER_BASE_ADDR + HDMI_OTHER_INTR_STAT_CLR, 0x7); //clear HPD falling interrupt in hdmi module
//#endif
    hdmi_print(1,"Reg0x196 = 0x%x  Reg0x80 = 0x%x  Reg0x26 = 0x%x\n", hdmi_rd_reg(TX_HDCP_ST_EDID_STATUS), hdmi_rd_reg(TX_CORE_EDID_CONFIG_MORE), hdmi_rd_reg(TX_HDCP_EDID_CONFIG));
...
//    Wr(PREG_PAD_GPIO2_EN_N, Rd(PREG_PAD_GPIO2_EN_N)|(1<<0)); //GPIOA_0 as input
    aml_write_reg32(P_PERIPHS_PIN_MUX_1, aml_read_reg32(P_PERIPHS_PIN_MUX_1)&~(1 << 22));
    //GPIOC_10 0x2012[10]
    aml_write_reg32(P_PREG_PAD_GPIO1_EN_N, aml_read_reg32(P_PREG_PAD_GPIO1_EN_N)|(1<<25)); //GPIOC_10 as input
}    
...
//    level = Rd(PREG_PAD_GPIO2_I)&0x1; //read GPIOA_0
    level = !!(aml_read_reg32(P_PREG_PAD_GPIO1_I)&(1<<25)); //read GPIOC_10
    return level;
}
...
//#ifdef AML_A3
    ...
    hdmi_wr_reg(TX_HDCP_EDID_CONFIG, (hdmi_rd_reg(TX_HDCP_EDID_CONFIG) & (1<<6)) | tmp_add_data);
...
static int hdmitx_m3_cntl(hdmitx_dev_t* hdmitx_device, int cmd, unsigned argv)
{
    if(cmd == HDMITX_HWCMD_POWERMODE_SWITCH){
        ...
        aml_write_reg32(P_PREG_PAD_GPIO1_EN_N, aml_read_reg32(P_PREG_PAD_GPIO1_EN_N) & (~(1<<25)));
        aml_write_reg32(P_PREG_PAD_GPIO1_O, aml_read_reg32(P_PREG_PAD_GPIO1_O) & (~(1<<25)));
        msleep(1000);
        aml_write_reg32(P_PERIPHS_PIN_MUX_1, aml_read_reg32(P_PERIPHS_PIN_MUX_1) | (1<<22) );
    }    
...
//\\ TODO
    aml_set_reg32_bits(P_PAD_PULL_UP_REG2, 1, 10, 1);       // Disable GPIOC_10 internal pull-up register  --jihongzhou
    aml_write_reg32(P_HHI_HDMI_CLK_CNTL, aml_read_reg32(P_HHI_HDMI_CLK_CNTL)| (1 << 8));
    HDMI_DEBUG();
    aml_write_reg32(P_HDMI_CNTL_PORT, aml_read_reg32(P_HDMI_CNTL_PORT)|(1<<15)); //APB3 err_en
