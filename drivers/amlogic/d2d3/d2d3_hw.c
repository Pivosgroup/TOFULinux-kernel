...
{		
	...
	/*adjust the offset of d2p */
        Wr_reg_bits(D2D3_D2P_PARAM_1,d2p_offset,D2P_OFFSET_BIT,D2P_OFFSET_WID);
	/*if depth >0 the curve will be protruded, else will be concave*/
	Wr(D2D3_MBDG_PARAM_3,(max_dtp<<24)|(min_dtp<<16)|(max_dtp<<8)|min_dtp);
	/*adjust the gain of mbdg*/
	Wr_reg_bits(D2D3_DBLD_MG_PARAM,mg1,DB_G1_MG_BIT,DB_G1_MG_WID);
	Wr_reg_bits(D2D3_DBLD_MG_PARAM,mg2,DB_G2_MG_BIT,DB_G2_MG_WID);
...
inline void d2d3_enable_path(bool enable, d2d3_param_t *parm)
{
        ...
        //dbr source
        Wr_reg_bits(VPP_INPUT_CTRL,0,VD1_LEAVE_MOD_BIT,VD1_LEAVE_MOD_WID); //vd1_interleave_mode:  0->no_interleave;
        if(!enable)
        {
                //disable dpg path
                Wr_reg_bits(D2D3_INTF_CTRL0,1,26,2);
                //disable dbr path
                Wr_reg_bits(D2D3_INTF_CTRL0,0,4,2); //d2d3_v1_sel: 1 to select VDR, 2 to select VPP scaler
                Wr_reg_bits(VPP_INPUT_CTRL,1,6,3); //vd1_l_sel: 1->vd1(vdr);2->vd2;3->d2d3_l;4->d2d3_r
        ...
        {  
                ...
                Wr_reg_bits(D2D3_INTF_CTRL0,nr_splitter,NRWR_DOUT_SPLITTER_BIT,NRWR_DOUT_SPLITTER_WID);      
                Wr_reg_bits(D2D3_INTF_CTRL0,dpg_gofield,V0_GOFLD_SEL_BIT,V0_GOFLD_SEL_WID);
                //enable dpg read nrw
                Wr_reg_bits(D2D3_INTF_CTRL0,dpg_path,D2D3_V0_SEL_BIT,D2D3_V0_SEL_WID);

                Wr_reg_bits(D2D3_INTF_CTRL0,dbr_gofield,V1_GOFLD_SEL_BIT,V1_GOFLD_SEL_WID);
                Wr_reg_bits(D2D3_INTF_CTRL0,dbr_path,D2D3_V1_SEL_BIT,D2D3_V1_SEL_WID);
                if(D2D3_DBR_MUX_VDR == dbr_path){
                        /*vd1_left select d2d3 left input */
                        Wr_reg_bits(VPP_INPUT_CTRL,3,VD1_L_SEL_BIT,VD1_L_SEL_WID); 
                        /*turn on vpp 3d scale */
                        Wr_reg_bits(VPP_VSC_PHASE_CTRL,2,DOUBLE_LINE_MODE_BIT,DOUBLE_LINE_MODE_WID);
                }
                else{
                        /*turn off vpp 3d scale */
                        Wr_reg_bits(VPP_VSC_PHASE_CTRL,0,DOUBLE_LINE_MODE_BIT,DOUBLE_LINE_MODE_WID);
                        /*vd1_left select vdr input*/
                        Wr_reg_bits(VPP_INPUT_CTRL,1,VD1_L_SEL_BIT,VD1_L_SEL_WID);
                }
                ...
                unsigned int scale_y, d2d3_dbr_mode_t dbr_mode,d2d3_scu18_param_t sSCU18Param)
{
        ...
        //dbr source
        Wr_reg_bits(D2D3_INTF_LENGTH,size_x-1,V1_LINE_LENGTHM1_BIT,V1_LINE_LENGTHM1_WID); 

        Wr_reg_bits(D2D3_GLB_CTRL,1,DBR_EN_BIT,DBR_EN_WID); //Set dbr_en = 1

        //0x02:{szx_m1(16),szy_m1(0)} 
        Wr(D2D3_DBR_OUTPIC_SIZE,((size_x-1)<<16) | (size_y-1)); //image size
        ...
        //d2p neg = 1
        Wr_reg_bits(D2D3_D2P_PARAM_1,1,D2P_NEG_BIT,D2P_NEG_WID);
        //smode
        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_smode,D2P_SMODE_BIT,D2P_SMODE_WID);
        //d2p out mode
        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_out_mode,D2P_OUT_MODE_BIT,D2P_OUT_MODE_WID);

        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_1dtolr,D2P_1DTOLR_BIT,D2P_1DTOLR_WID);

        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_lr_switch,D2P_LR_SWITCH_BIT,D2P_LR_SWITCH_WID); 
        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_lar,D2P_LAR_BIT,D2P_LAR_WID);
        Wr_reg_bits(D2D3_D2P_PARAM_1,0,D2P_WRAP_EN_BIT,D2P_WRAP_EN_WID);

        Wr_reg_bits(D2D3_D2P_PARAM_1,dbr_d2p_lomode,D2P_LOMODE_BIT,D2P_LOMODE_WID);

        //set bound default left 8 pixel&right 8 pixel
        Wr_reg_bits(D2D3_D2P_PARAM_1,8,D2P_BRDWID_BIT,D2P_BRDWID_WID);

        dwTemp = dbr_ddd_lomode | (dbr_ddd_out_mode<<2) | (dbr_ddd_hhalf<<4) | 
                (0<<6) | (0 <<7); 
        Wr(D2D3_DBR_DDD_CTRL,dwTemp);

        Wr_reg_bits(D2D3_DBR_LRDMX_CTRL,1,LR_MERGE_BIT,LR_MERGE_WID); //set lr_merge 

        if(sSCU18Param.dbr_scu18_step_en == 0)
        {
                scale_x_act = (dbr_ddd_hhalf) ? scale_x-1 : scale_x;
                Wr_reg_bits(D2D3_SCALER_CTRL,scale_x_act|(scale_y<<2),4,8-4);  //Set SCD18 factor:
        }
        else
        {  
                ...
                Wr(D2D3_SCU18_INPIC_SIZE,((sSCU18Param.dbr_scu18_isize_y)<<16) | (sSCU18Param.dbr_scu18_isize_x));
                Wr(D2D3_SCU18_STEP,((1<<16)|(scu18_hstep<<8)|scu18_vstep));
        }
        Wr_reg_bits(D2D3_SCALER_CTRL,dbr_scu18_rep_en,8,1);  //Set SCD18 factor:
        Wr_reg_bits(D2D3_SCALER_CTRL,sSCU18Param.dbr_scu18_iniph_h|(sSCU18Param.dbr_scu18_iniph_v<<8),16,16); //set phase
        ...
        Wr_reg_bits(D2D3_DRMIF_CTRL,1,DR_REQ_EN_BIT,DR_REQ_EN_WID); //dr_req_en=1
}
...
{        
        Wr_reg_bits(D2D3_INTF_LENGTH,size_x-1,V0_LINE_LENGTHM1_BIT,V0_LINE_LENGTHM1_WID); //use to generate eol 

        Wr_reg_bits(D2D3_GLB_CTRL,1,DPG_EN_BIT,DPG_EN_WID); //Set dpg_en = 1

        Wr(D2D3_DPG_INPIC_SIZE,((size_x-1)<<16) | (size_y-1)); //set image size of DPG  

        //scu18       
        Wr_reg_bits(D2D3_SCALER_CTRL,scale_x|(scale_y<<2),0,4);  
        Wr_reg_bits(D2D3_SCALER_CTRL,0,11,12-11); 
        Wr(D2D3_DGEN_WIN_HOR,(0<<16) | (xsize_depth-1)); 
        Wr(D2D3_DGEN_WIN_VER,(0<<16) | (ysize_depth-1)); //set Windows of DPG

        //=== Configure the DPG Models ===
        //cpdg
        Wr_reg_bits(D2D3_GLB_CTRL,1,2,1);
        Wr_reg_bits(D2D3_CG_PARAM_2,ysize_depth,16,16); //set cg_vpos_thr:    szyScaled
        //mpdg
        Wr_reg_bits(D2D3_GLB_CTRL,1,3,1);
        Wr_reg_bits(D2D3_MBDG_PARAM_0,xsize_depth/2,16,24-16); 
        Wr_reg_bits(D2D3_MBDG_PARAM_1,ysize_depth/2,16,24-16); 
        Wr(D2D3_MBDG_PARAM_2,((xsize_depth/2-1)) | ((xsize_depth/2-1)<<8) | ((ysize_depth/2-1)<<16) | ((ysize_depth/2-1)<<24)); 
        ...
        //dwmif
        Wr(D2D3_DWMIF_HPOS,(0x0)|((xsize_depth-1)<<16)); 
        Wr(D2D3_DWMIF_VPOS,(0x0)|((ysize_depth-1)<<16)); 
        Wr(D2D3_DWMIF_SIZE,(xsize_depth-1)|((ysize_depth-1)<<16)); 
        Wr_reg_bits(D2D3_DWMIF_CTRL,1,8,1); //dw_req_en = 1;
...
void d2d3_config(d2d3_dev_t *d2d3_devp,d2d3_param_t *parm)
{
        ...
        Wr_reg_bits(D2D3_GLB_CTRL,1,0,1); 
        Wr_reg_bits(D2D3_GLB_CTRL,1,1,1);
...
{
        if(enable){
                Wr_reg_bits(D2D3_GLB_CTRL,0,CLK_CTRL_BIT,CLK_CTRL_WID);
                //disable d2d3 clock
                Wr_reg_bits(D2D3_INTF_CTRL0,1,ON_CLK_D2D3_BIT,ON_CLK_D2D3_WID); 
        }else {
                //soft reset d2d3 unit
        	Wr_reg_bits(D2D3_GLB_CTRL,1,SW_RST_NOBUF_BIT,SW_RST_NOBUF_WID);
                ...
                Wr_reg_bits(D2D3_GLB_CTRL,0x155,CLK_CTRL_BIT,CLK_CTRL_WID);
                //disable d2d3 clock
                Wr_reg_bits(D2D3_INTF_CTRL0,0,ON_CLK_D2D3_BIT,ON_CLK_D2D3_WID); 
        }
}
