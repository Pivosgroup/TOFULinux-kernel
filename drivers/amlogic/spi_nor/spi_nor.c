...
static int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	while (len) {
		...
		if(spi_nor->secure_protect){
			if((addr >= spi_nor->securestorage_info->start_pos)&&(addr < spi_nor->securestorage_info->end_pos)){
				addr += mtd->erasesize;
				len -= mtd->erasesize;
				continue;
			}
...
#ifdef CONFIG_SPI_NOR_SECURE_STORAGE
	if(spi_nor->secure_protect){
		if(((from + len)>spi_nor->securestorage_info->start_pos) &&((from + len)<=spi_nor->securestorage_info->end_pos)){
			//printk("%s:%d,from:%llx,len:%x\n",__func__,__LINE__,from,len);
			return -EINVAL;
		}
		else if(((from+len) > spi_nor->securestorage_info->end_pos)
			&&(from>=spi_nor->securestorage_info->start_pos)
			&&(from<spi_nor->securestorage_info->end_pos)){
			return -EINVAL;
		}
		else if(((from+len) > spi_nor->securestorage_info->end_pos) 
			&&(from < spi_nor->securestorage_info->start_pos)
			&&(len>(spi_nor->securestorage_info->start_pos-from))){
				//printk("prohibit read\n");
			return -EINVAL;
		}
	}
#endif
...
#ifdef CONFIG_SPI_NOR_SECURE_STORAGE
	if(spi_nor->secure_protect){
		if(((to + len)>spi_nor->securestorage_info->start_pos) &&((to + len)<=spi_nor->securestorage_info->end_pos)){
			return -EINVAL;
		}
		else if(((to+len) > spi_nor->securestorage_info->end_pos)
			&&(to>=spi_nor->securestorage_info->start_pos)
			&&(to<spi_nor->securestorage_info->end_pos)){
			return -EINVAL;
		}
		else if(((to+len) > spi_nor->securestorage_info->end_pos) 
			&&(to < spi_nor->securestorage_info->start_pos)
			&&(len>(spi_nor->securestorage_info->start_pos-to))){
				//printk("prohibit write\n");
			return -1;
		}
	}
#endif
...
static int spi_nor_probe(struct spi_device *spi)
{
...
#ifdef CONFIG_SPI_NOR_SECURE_STORAGE
	uint64_t offset;
	int securestorage_flag;
	struct mtd_partition *parts_start;
#endif
	...
	spi_nor->mtd.erase = spi_nor_erase;
	spi_nor->mtd.read = spi_nor_read;
	spi_nor->mtd.write = spi_nor_write;
...
#ifdef CONFIG_SPI_NOR_SECURE_STORAGE
	parts_start = parts;
	securestorage_flag = 0;
	for(i=0;i<nr;i++){
		offset=parts_start->offset + parts_start->size;
		if(offset > SPI_SECURESTORAGE_OFFSET){
			securestorage_flag = 1;
			if(parts_start->offset >= (SPI_SECURESTORAGE_OFFSET + SPI_SECURESTORAGE_AREA_COUNT*SPI_SECURESTORAGE_AREA_SIZE)){
				securestorage_flag = 0;
			}
			break;
		}
	...
	if(securestorage_flag == 0){
		int spi_securestorage_probe(void *keypara);
		spi_securestorage_probe(spi_nor);
	}
	else{
		printk("%s:%d: %s and spi secure storage position conflict\n",__func__,__LINE__,parts_start->name);
	}
#endif
	return add_mtd_partitions(&spi_nor->mtd, parts, nr);
#else
#ifdef CONFIG_SPI_NOR_SECURE_STORAGE
	int spi_securestorage_probe(void *keypara);
	spi_securestorage_probe(spi_nor);
#endif
	return add_mtd_device(&spi_nor->mtd) == 1 ? -ENODEV : 0;
#endif
	}
}
...
static int spi_nor_remove(struct spi_device *spi)
{
	...
	if (mtd_has_partitions() && spi_nor->partitioned)
		status = del_mtd_partitions(&spi_nor->mtd);
	else
		status = del_mtd_device(&spi_nor->mtd);
	if (status == 0)
		kfree(spi_nor);
	return 0;
}
