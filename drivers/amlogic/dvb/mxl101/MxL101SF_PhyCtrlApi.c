...
MXL_STATUS MxL101SF_API_GetChannelOffset(PMXL_TUNER_CHAN_OFFSET_T ChanOffsetPtr)
{
  ...
  status |= Ctrl_ReadRegister(0x9B, &rawData); 
  status |= Ctrl_ReadRegister(V6_FREQ_OFFSET_LSB_REG, &rawData);
  freqOffset = rawData;
  status |= Ctrl_ReadRegister(V6_FREQ_OFFSET_MSB_REG, &rawData);
...
MXL_STATUS MxL101SF_API_CheckChannel(PMXL_TUNER_CHECK_CHAN_REQ_T ChanScanCtrlPtr)
{
  ...
  MxL101SF_API_TuneRF((PMXL_RF_TUNE_CFG_T)&mxlChanCfg);
 ...
 // Reset IRQ status
  ...
  {
    status |= MxL101SF_API_GetAGCLock(&demodLockStatus);
    ...
    status |= MxL101SF_API_DemodGetCpLockStatus(&demodLockStatus);
  ...
  if (demodLockStatus.Status == MXL_LOCKED) 
  {
    ...
    {
      status |= MxL101SF_API_DemodGetTpsLock(&demodLockStatus);
    ...
    if (demodLockStatus.Status == MXL_LOCKED) 
    {
      ...
      {
        status |= MxL101SF_API_DemodGetRsLockStatus(&demodLockStatus);
