...
struct aml_dmx_ts_config {
	bool ts_config_valid;
...
static struct aml_dmx_ts_config g_ts_config[TS_SRC_MAX];

/*FEC_A*/
static pinmux_item_t fec_a_pins[] = {
	{
		.reg = PINMUX_REG(3),
		.setmask = 0x3f
	},
...
static pinmux_set_t fec_a_pinmux_set = {
	.chip_select = NULL,
	.pinmux = &fec_a_pins[0]
};

/*FEC_C*/
static pinmux_item_t fec_c_pins[] = {
	{
		.reg = PINMUX_REG(6),
		.setmask = 0x1f << 19
	},
...
static pinmux_set_t fec_c_pinmux_set = {
	.chip_select = NULL,
	.pinmux = &fec_c_pins[0]
};

/*FEC_B*/
static pinmux_item_t fec_b_pins[] = {
	{
		.reg = PINMUX_REG(3),
		.setmask = 0x3f << 6
	},
...
static pinmux_set_t fec_b_pinmux_set = {
	.chip_select = NULL,
	.pinmux = &fec_b_pins[0]
};
...
static void sec_data_notify(struct aml_dmx *dmx, u16 sec_num, u16 buf_num)
{
	...
	if(!sec_filter_match(dmx, f, p)) {
		...
		if(i>=FILTER_COUNT) {
			pr_dbg("section data donot match any filter:buf_num %d, sec_num %d: \
...
static void software_match_section(struct aml_dmx *dmx, u16 buf_num)
{
	...
	for(i=0; i<FILTER_COUNT; i++) {
		...
		if(sec_filter_match(dmx, f, p)) {
			...
			if (pid == -1){
				pid = dmx->channel[f->chan_id].pid;
				sec_num = i;
			} else {
			//	printk("[software match]Muli-filter match this section, will skip this section\n");
				return;
			}
		}
	}
	...
	if (pid != -1) {
	//	printk("[software match]dispatch section to filter %d, pid %d\n", sec_num, pid);
		sec_data_notify(dmx, sec_num, buf_num);
	} else {
...
static void process_sub(struct aml_dmx *dmx)
{
	...
	if(len1)
		dma_sync_single_for_cpu(NULL, buffer1, len1, DMA_FROM_DEVICE);
	if(len2)
		dma_sync_single_for_cpu(NULL, buffer2, len2, DMA_FROM_DEVICE);
...
static u32 get_invert(int src)
{
	...
	if(g_ts_config[ts_index].ts_config_valid){
		invert_error = g_ts_config[ts_index].invert_error;
		invert_data = g_ts_config[ts_index].invert_data;
		invert_sync = g_ts_config[ts_index].invert_sync;
		invert_valid = g_ts_config[ts_index].invert_valid;
		invert_clk = g_ts_config[ts_index].invert_clk;
	}
#else

	switch(src) {
		case AM_TS_SRC_S_TS0:
#ifdef CONFIG_AMLOGIC_TS0_INVERT_S2P1_FEC_ERROR
			invert_error = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS0_INVERT_S2P1_FEC_DATA
			invert_data = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS0_INVERT_S2P1_FEC_SYNC
			invert_sync = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS0_INVERT_S2P1_FEC_VALID
			invert_valid = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS0_INVERT_S2P1_FEC_CLK
			invert_clk = 1;
#endif
		break;
		case AM_TS_SRC_S_TS1:
#ifdef CONFIG_AMLOGIC_TS1_INVERT_S2P1_FEC_ERROR
			invert_error = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS1_INVERT_S2P1_FEC_DATA
			invert_data = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS1_INVERT_S2P1_FEC_SYNC
			invert_sync = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS1_INVERT_S2P1_FEC_VALID
			invert_valid = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS1_INVERT_S2P1_FEC_CLK
			invert_clk = 1;
#endif
		break;
		case AM_TS_SRC_S_TS2:
#ifdef CONFIG_AMLOGIC_TS2_INVERT_S2P1_FEC_ERROR
			invert_error = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS2_INVERT_S2P1_FEC_DATA
			invert_data = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS2_INVERT_S2P1_FEC_SYNC
			invert_sync = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS2_INVERT_S2P1_FEC_VALID
			invert_valid = 1;
#endif
#ifdef CONFIG_AMLOGIC_TS2_INVERT_S2P1_FEC_CLK
			invert_clk = 1;
#endif
		break;
	}
	...
	return (invert_error<<INVERT_S2P0_FEC_ERROR)|
		(invert_data<<INVERT_S2P0_FEC_DATA)     |
		(invert_sync<<INVERT_S2P0_FEC_SYNC)     |
		(invert_valid<<INVERT_S2P0_FEC_VALID)   |
		(invert_clk<<INVERT_S2P0_FEC_CLK);
}

static int get_fec_clk(aml_ts_source_t src)
{
	int ts_index = 0;
	int fec_clk = 0;
	...
	if(g_ts_config[ts_index].ts_config_valid){
		fec_clk = g_ts_config[ts_index].fec_clk;
	}
#else
#endif
...
{
	int out_src, des_in, en_des, fec_s, fec_clk, hiu ,dec_clk_en;
	int src, tso_src;
	u32 invert;
...
static int dmx_enable(struct aml_dmx *dmx)
{
	...
	switch(dmx->source) {
		case AM_TS_SRC_TS0:
			fec_sel = 0;
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)	
			fec_clk = get_fec_clk(AM_TS_SRC_TS0);
#else
			fec_clk = 1;
#endif
			record  = record?1:0;
		...
		case AM_TS_SRC_TS1:
			fec_sel = 1;
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)	
			fec_clk = get_fec_clk(AM_TS_SRC_TS1);
#else			
			fec_clk = 0;
#endif
			record  = record?1:0;
		...
		case AM_TS_SRC_TS2:
			fec_sel = 2;
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)	
			fec_clk = get_fec_clk(AM_TS_SRC_TS2);
#else			
			fec_clk = 0;
#endif
			record  = record?1:0;
		...
		case AM_TS_SRC_S_TS2:
			fec_sel = 6;
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)	
			fec_clk = get_fec_clk(dmx->source);
#else			
			fec_clk = 1;
#endif
			record  = record?1:0;
			set_stb = 1;
			invert  = get_invert(dmx->source);
	...
	if(dmx->chan_count) {
		if(set_stb){
			...
			v &= ~((3<<S2P0_FEC_SERIAL_SEL)|(1<<INVERT_S2P0_FEC_CLK)|(1<<INVERT_S2P0_FEC_DATA)|(1<<INVERT_S2P0_FEC_ERROR)|(1<<INVERT_S2P0_FEC_SYNC)|(1<<INVERT_S2P0_FEC_VALID));
			v |= (fec_s<<S2P0_FEC_SERIAL_SEL)|invert;
...
int dmx_get_ts_serial(aml_ts_source_t src)
{
	int ts_index = 0;
	int serial_ts = 1;
	...
	if(g_ts_config[ts_index].ts_config_valid)
		serial_ts = g_ts_config[ts_index].serial_ts;
...
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)
			if(g_ts_config[0].ts_config_valid){
				if(g_ts_config[0].serial_ts){
...
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)
			if(g_ts_config[1].ts_config_valid){
				if(g_ts_config[1].serial_ts){
...
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)
			if(g_ts_config[2].ts_config_valid){
				if(g_ts_config[2].serial_ts){
...
int aml_tso_hw_set_source(struct aml_dvb *dvb, dmx_source_t src)
{
	...
	switch(src) {
		case DMX_SOURCE_FRONT0:
			...
			if(g_ts_config[0].ts_config_valid){
				if(g_ts_config[0].serial_ts){
					dvb->tso_source = AM_TS_SRC_S_TS0;
				}
				else{
					dvb->tso_source = AM_TS_SRC_TS0;
				}
			}
#else
		...
		case DMX_SOURCE_FRONT1:
			...
			if(g_ts_config[1].ts_config_valid){
				if(g_ts_config[1].serial_ts){
					dvb->tso_source = AM_TS_SRC_S_TS1;
				}
				else{
					dvb->tso_source = AM_TS_SRC_TS1;
				}
			}
#else
		...
		case DMX_SOURCE_FRONT2:
			...
			if(g_ts_config[2].ts_config_valid){
				if(g_ts_config[2].serial_ts){
					dvb->tso_source = AM_TS_SRC_S_TS2;
				}
				else{
					dvb->tso_source = AM_TS_SRC_TS2;
				}
			}
#else
		...
		break;
		case DMX_SOURCE_DVR0:
			dvb->tso_source = AM_TS_SRC_HIU;
		break;
		case DMX_SOURCE_FRONT0+100:
			dvb->tso_source = AM_TS_SRC_DMX0;
		break;
		case DMX_SOURCE_FRONT1+100:
			dvb->tso_source = AM_TS_SRC_DMX1;
		break;
		case DMX_SOURCE_FRONT2+100:
			dvb->tso_source = AM_TS_SRC_DMX2;
		break;
		default:
			pr_error("illegal demux source %d\n", src);
			ret = -EINVAL;
...
static ssize_t aml_dmx_show_dynamic_config(struct class *class, struct class_attribute *attr, char *buf)
{
	ssize_t ret = 0;
...
static ssize_t aml_dmx_store_dynamic_config(struct class *class, struct class_attribute *attr, const char *buf, size_t size)
{
	int i, ts_index = 0;
	char *s, *p;
	...
	while ((s = strstr(p, "|")) != NULL) {
		*s = '\0';
		pr_info("%s\n", p);
		if(i % 9 == 0){
			ts_index = simple_strtol(p, NULL, 0);
			g_ts_config[ts_index].ts_config_valid = true;
		}
		else if(i % 9 == 1){
			g_ts_config[ts_index].enable_ts = simple_strtol(p, NULL, 0);
			/*set and clear pinmux.normally, the work should do in board source file*/
			if(ts_index == 0){
				pinmux_set(&fec_a_pinmux_set);
			}
			else if(ts_index == 1){				
				pinmux_set(&fec_c_pinmux_set);
			}
			else if(ts_index == 2){				
				pinmux_set(&fec_b_pinmux_set);
			}			
		}
		else if(i % 9 == 2){
			g_ts_config[ts_index].serial_ts = simple_strtol(p, NULL, 0);
		}		
		else if(i % 9 == 3){
			g_ts_config[ts_index].invert_clk = simple_strtol(p, NULL, 0);
		}
		else if(i % 9 == 4){
			g_ts_config[ts_index].invert_data = simple_strtol(p, NULL, 0);
		}
		else if(i % 9 == 5){
			g_ts_config[ts_index].invert_error = simple_strtol(p, NULL, 0);
		}
		else if(i % 9 == 6){
			g_ts_config[ts_index].invert_valid = simple_strtol(p, NULL, 0);
		}
		else if(i % 9 == 7){
			g_ts_config[ts_index].invert_sync = simple_strtol(p, NULL, 0);
		}
		else if(i % 9 == 8){
			g_ts_config[ts_index].fec_clk = simple_strtol(p, NULL, 0);
		}
...
#if defined (CONFIG_AMLOGIC_DYNAMIC_FEANDDMX_CONFIG)		
	__ATTR(aml_dmx_dynamic_config,  S_IRUGO | S_IWUSR, aml_dmx_show_dynamic_config, aml_dmx_store_dynamic_config),
#endif
