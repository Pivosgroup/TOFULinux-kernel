...
static unsigned int c_fUHF_min = 312000;
...
unsigned long samba_init(void)
{
	...
	error = Standard_readRegister(Afatech_SAMBA_demodulator, Processor_OFDM, 0x1dc, &isCATV);
	if (error) goto exit;

	if(isCATV == 1){

		error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh8_en, reg_top_gpioh8_en_pos, reg_top_gpioh8_en_len, 1);
		if (error) goto exit;
		error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh8_on, reg_top_gpioh8_on_pos, reg_top_gpioh8_on_len, 1);
		if (error) goto exit;
	}
...
unsigned long samba_setfreq(unsigned int bw_kHz, unsigned int rf_freq_kHz)
{
	...
	if(isCATV == 1){
		//control V band filter
		if(rf_freq_kHz<176143)
			error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 0);
		else
			error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 1);
		if (error) goto exit;
	}

	error = samba_sethalffreq(rf_freq_kHz);
...
unsigned long samba_sethalffreq(unsigned int rf_freq_kHz)
{
	unsigned long error = 0;
	unsigned char val[3];
	...
	val[0] = samba_get_lna_cap_sel(rf_freq_kHz/2);

	/*----- set_lo_freq -----*/
	tmp = samba_get_lo_freq(rf_freq_kHz/2);
	val[1] = (unsigned char) (tmp & 0xFF);
	val[2] = (unsigned char) ((tmp >> 8) & 0xFF);

	/*----- write -----*/
	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, half_lnac_lna_cap_sel, 1, val);
	if (error) goto exit;	

	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, half_lo_freq_7_0, 1, &val[1]);
	if (error) goto exit;

	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, half_lo_freq_15_8, 1, &val[2]);
	if (error) goto exit;
