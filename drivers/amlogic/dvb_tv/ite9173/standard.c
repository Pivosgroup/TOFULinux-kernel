...
static unsigned char CATV_VBandChannel_RegisterSet_Map[63] = {
    7, 2, 7, 3, 13, 8, 5, 20, 17, 19,       // 93.143 ~ 147.143
    16, 19, 15, 17, 10, 3, 7, 5, 2, 0,      //153.143 ~ 207.143
    19, 18, 15, 12, 11, 9, 8, 2, 0, 1,      //213.143 ~ 267.143
	0, 19, 18, 16, 15, 13, 12, 20, 17, 11,  //273.143 ~ 327.143
	18, 0, 5, 14, 3, 19, 12, 4, 11, 0,      //333.143 ~ 387.143
	3, 5, 8, 20, 14, 15, 14, 0, 16, 13,     //393.143 ~ 447.143
	20, 18, 20,								//453.143 ~ 465.143
};

static unsigned char CATV_UBandChannel_RegisterSet_Map[50] = {
    16, 14, 14, 13, 13, 1, 0, 19, 18, 9,    //473.143 ~ 527.143
    18, 15, 14, 16, 13, 20, 12, 1, 0, 18,   //533.143 ~ 587.143
    17, 16, 15, 14, 14, 14, 13, 20, 6, 19,  //593.143 ~ 647.143
	1, 16, 15, 15, 14, 20, 7, 20, 12, 18,   //653.143 ~ 707.143
	11, 16, 0, 1, 1, 20, 20, 20, 17, 19,    //713.143 ~ 767.143
};
...
Dword Standard_setFrequency (
	...
	if(isSetSDRAMClock){
		error = Standard_readRegister(demodulator, Processor_OFDM, 0x1dc, &isCATV);
		if (error) goto exit;
		...
		if(isCATV == 1){
			...
			if(frequency < 469000){
				// v-band
				if(frequency > 93143)
					channel_no = (Byte)((frequency - 93143)/6000) ;
				else
					channel_no = 0;
				...
				if(frequency < 473143)
					...
					channel_no = (Byte)((frequency - 473143)/6000) ;
			}else{
				channel_no = 49;
			}
			// find nearest Channel
			if((channel_no < 62) && (frequency > 93143)){
				if(frequency < 469000){// v-band
					if((frequency < 465143)){
						if((frequency - ((channel_no)*6000 + 93143)) > (((channel_no+1)*6000 + 93143) - frequency))
							channel_no++;

						if(channel_no > 62)//V band channel no max = 62
							channel_no = 62;
					}else{
						channel_no = 62;
					}
					...
					if(frequency > 473143){
						if((frequency - ((channel_no)*6000 + 473143)) > (((channel_no+1)*6000 + 473143) - frequency))
							channel_no++;
						
						if(channel_no > 49) //U band channel no max = 49
							channel_no = 49;
					}else{
						channel_no = 0;
					}
			...
			if(frequency < 469000){
				register_set = CATV_VBandChannel_RegisterSet_Map[channel_no];
			}else{
				register_set = CATV_UBandChannel_RegisterSet_Map[channel_no];
			}
		}else{
			if(frequency < 400000){
				// v-band
...
Dword Standard_initialize (
	...
	printk("[-----------------ite9173]chip_version is %x\n",chip_version);
	if (error) goto exit;
...
Dword Standard_isMpeg2Locked (
	...
	*locked = False;
	error = Standard_readRegisterBits (demodulator, Processor_OFDM, r_mp2if_sync_byte_locked_A, mp2if_sync_byte_locked_A_pos, mp2if_sync_byte_locked_A_len, &temp);
	if (error) goto exit;
	lock = lock | temp;
	
	error = Standard_readRegisterBits (demodulator, Processor_OFDM, r_mp2if_sync_byte_locked_B, mp2if_sync_byte_locked_B_pos, mp2if_sync_byte_locked_B_len, &temp);
	if (error) goto exit;
	lock = lock | temp;

	error = Standard_readRegisterBits (demodulator, Processor_OFDM, r_mp2if_sync_byte_locked_C, mp2if_sync_byte_locked_C_pos, mp2if_sync_byte_locked_C_len, &temp);
	if (error) goto exit;
	lock = lock | temp;
...
Dword Standard_setOneSeg (
	...
	error = Standard_writeRegister(demodulator, Processor_OFDM, var_one_seg , isoneseg);
	
	return error;
}


Dword Standard_setLayerFilter (
    IN  Demodulator*    demodulator,
           IN  Byte            enableLayerA,
           IN  Byte            enableLayerB,
           IN  Byte            enableLayerC
){
 
           Dword error = Error_NO_ERROR;
    Byte temp = 0;
           error = Standard_writeRegister(demodulator, Processor_OFDM, var_one_seg , 0xff);
    ...
    if(enableLayerC)
                     temp |=1;
    error = Standard_writeRegisterBits(demodulator, Processor_OFDM, p_reg_p_layer_filter , 0, 3, temp);
