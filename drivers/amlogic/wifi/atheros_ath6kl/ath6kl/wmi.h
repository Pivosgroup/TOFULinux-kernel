...
struct wmi {
	...
	struct list_head mgmt_tx_frame;
...
#define GET_PAD_BEFORE_DATA_START(_v)                   \
	((_v) & WMI_DATA_HDR_PAD_BEFORE_DATA_MASK)

#define WMI_DATA_HDR_GET_PAD_BEFORE_DATA_START(h)       \
	(((h)->info3 >> WMI_DATA_HDR_PAD_BEFORE_DATA_SHIFT) & \
...
#define WMI_DATA_HDR_IS_TRIGGER(h)       \
	((((h)->info3 >> WMI_DATA_HDR_TRIGGER_SHIFT) &    \
...
enum wmi_cmd_id {
	...
	WMI_SET_HT_OP_CMDID,
	WMI_SET_TX_SELECT_RATES_CMDID,		/* 0xF020 */
	...
	WMI_SET_BTCOEX_DEBUG_CMDID,		/* 0xF030 */
	WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMDID,
	WMI_GET_BTCOEX_STATS_CMDID,
	WMI_GET_BTCOEX_CONFIG_CMDID,

	WMI_SET_DFS_ENABLE_CMDID,		/* 0xF034 */
	...
	WMI_SET_PROMISCUOUS_MODE_CMDID,		/* 0xF040*/
	...
	/* More P2P Cmds */
	WMI_P2P_GO_NEG_REQ_RSP_CMDID,		/* 0xF050 */
...
struct wmi_bss_bias_info{
        u8                  numBss;
        struct wmi_bss_bias bssBias[1];
};

struct wmi_lowerssi_scan_params{
...
union info_type{
    ...
    struct wmi_bss_bias_info        bssBiasInfo; /* WMI_SET_HOST_BIAS */
    struct wmi_lowerssi_scan_params lrScanParams;
};

struct wmi_set_roam_ctrl_cmd{
    union info_type info;
    u8   roamCtrlType ;
}__packed;
...
struct wmi_set_tx_sgi_param_cmd {
	u32 mask[WMI_MAX_RATE_MASK];
	u8 per_threshold;
} __packed;
...
/* the following filter options are available if the WOW extensions are 
 * installed in the target otherwise the feature setting has no effect  
 * */
#define WOW_FILTER_OPTION_MASK(option)			(1 << (option))
#define WOW_FILTER_SSID							WOW_FILTER_OPTION_MASK(0)
...
int ath6kl_wmi_set_lpreamble_cmd(struct ath6kl_vif *vif, u8 status,
				 u8 preamble_policy);
...
/* AP mode */
int ath6kl_wmi_ap_profile_commit(struct ath6kl_vif *vifp, struct wmi_connect_cmd *p);

int ath6kl_wmi_ap_set_mlme(struct ath6kl_vif *vifp, u8 cmd, const u8 *mac, u16 reason);
...
int ath6kl_wmi_set_rx_frame_format_cmd(struct ath6kl_vif *vif, u8 rx_meta_version,
...
int ath6kl_wmi_set_roam_ctrl_cmd_for_lowerrssi(struct ath6kl_vif *vif, 
                      u16  lowrssi_scan_period,
                      u16  lowrssi_scan_threshold,
                      u16  lowrssi_roam_threshold,
                      u8   roam_rssi_floor);
...
int ath6kl_wmi_add_wow_pattern_cmd(struct ath6kl_vif *vif,
			    struct wmi_add_wow_pattern_cmd *cmd, u8 *pattern,
...
int ath6kl_wmi_set_green_tx_params(struct ath6kl_vif *vif,
				struct wmi_green_tx_params *params);

int ath6kl_wmi_smps_config(struct ath6kl_vif *vif,
				struct wmi_config_smps_cmd *options);

int ath6kl_wmi_smps_enable(struct ath6kl_vif *vif,
				struct wmi_config_enable_cmd *options);

int ath6kl_wmi_lpl_enable_cmd(struct ath6kl_vif *vif,
