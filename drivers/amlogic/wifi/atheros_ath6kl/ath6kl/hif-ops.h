...
{
	if(ar->hif_ops->scat_req_rw)
		return ar->hif_ops->scat_req_rw(ar, scat_req);
}

static inline void ath6kl_hif_cleanup_scatter(struct ath6kl *ar)
{
	if(ar->hif_ops->cleanup_scatter)
		return ar->hif_ops->cleanup_scatter(ar);
}

static inline void ath6kl_hif_suspend(struct ath6kl *ar)
{
	if(ar->hif_ops->suspend)
		ar->hif_ops->suspend(ar);
}

static inline void ath6kl_hif_resume(struct ath6kl *ar)
{
	if(ar->hif_ops->resume) 
		ar->hif_ops->resume(ar);	
}
...
static inline int ath6kl_hif_get_stat(struct ath6kl *ar, u8 *buf,
					int buf_len)
{
	if(ar->hif_ops->get_stat)
		return ar->hif_ops->get_stat(ar, buf, buf_len);
	else
		return 0;
}

static inline void ath6kl_hif_set_max_sche(struct ath6kl *ar, 
					u32 max_sche_tx, u32 max_sche_rx)
{
	if(ar->hif_ops->set_max_sche)
		ar->hif_ops->set_max_sche(ar, max_sche_tx, max_sche_rx);
}
#endif
