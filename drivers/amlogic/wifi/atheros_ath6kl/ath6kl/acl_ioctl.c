...
int ath6kl_wmi_set_param_cmd(struct net_device *netdev, void *data)
{
	...
	acl_wcmd_t     *req = NULL;
	unsigned long      req_len = sizeof(acl_wcmd_t);
	int val;
	...
	{    
		...
		ret = ath6kl_wmi_cmd_send(vif, skb, WMI_AP_ACL_POLICY_CMDID,
					  NO_SYNC_WMIFLAG);
	...
	{
		status = ath6kl_wmi_set_ap_num_sta_cmd(vif, req->val);
	}
done:
    vfree(req);
	up(&vif->ar->sem);
...
int ath6kl_wmi_get_param_cmd(struct net_device *netdev, void *data)
{
	...
	req = (acl_wcmd_t*)vmalloc(req_len);
    if(!req)
        return ENOMEM;
...
static u8 mlme_mac_cmp_wild(u8 *acl_mac, u8 *sta_mac, u8 wild)
{
    ...
    for(i=0;i<ATH_MAC_LEN;i++) {
        if (wild & (1<<i)) continue;
        if(acl_mac[i] != sta_mac[i]) return 1;
    }
    return 0;
}
int ath6kl_wmi_addmac_cmd(struct net_device *netdev, void *data)
{
	...
	for(i = 0; i < AP_ACL_SIZE; i++) {
		if (mlme_mac_cmp_wild(vif->acl_db.acl_mac[i], sa->sa_data, 0)==0) {
			assigned_idx = i;
		}
	}
	...
	if(assigned_idx == -1) {//not in list,find new entry
		for(i = 0; i < AP_ACL_SIZE; i++) {
			if((vif->acl_db.index & (1<<i)) == 0) {//not used item
				assigned_idx = i;				
				break;
			}
		}
	}
	printk("%s[%d]assigned_idx=%d\n\r",__func__,__LINE__,assigned_idx);
	...
	{    
		...
		cmd->wildcard = 0; 
		ret = ath6kl_wmi_cmd_send(vif, skb, WMI_AP_ACL_MAC_LIST_CMDID,
					  NO_SYNC_WMIFLAG);
					  
		//update local acl db
		{
			memcpy(vif->acl_db.acl_mac[assigned_idx], sa->sa_data, ATH_MAC_LEN);
			vif->acl_db.index = vif->acl_db.index | (1 << assigned_idx);
...
int ath6kl_wmi_delmac_cmd(struct net_device *netdev, void *data)
{
	...
	{    
		...
		//update local acl db
		{
			memset(vif->acl_db.acl_mac[assigned_idx], 0x00, ATH_MAC_LEN);
			vif->acl_db.index = vif->acl_db.index & ~(1 << assigned_idx);
			vif->acl_db.wildcard[assigned_idx] = 0;
		}					  
		up(&vif->ar->sem);	
...
int ath6kl_wmi_getmac_cmd(struct net_device *netdev, void *data)
{
	...
	{    
		cnt = (int *)macList;
		macAddr = macList + sizeof(*cnt);   /* MacAddr Count, then MacAddr one by one. */
		len -= sizeof(*cnt);
		*cnt = 0;
		for(i = 0; i < AP_ACL_SIZE; i++) {
			if((vif->acl_db.index & (1<<i)) != 0) {//used item
				...
				memcpy((u8*)macAddr, (u8*)&vif->acl_db.acl_mac[i][0], 6);
...
int ath6kl_wmi_set_mlme_cmd(struct net_device *netdev, void *data)
{
	...
	switch (mlme.im_op) {
		case IEEE80211_MLME_DISASSOC:
			status = ath6kl_wmi_ap_set_mlme(vif, WMI_AP_DISASSOC,
									  mlme.im_macaddr, mlme.im_reason);		  
			break;
		case IEEE80211_MLME_DEAUTH:
			status = ath6kl_wmi_ap_set_mlme(vif, WMI_AP_DEAUTH,
									  mlme.im_macaddr, mlme.im_reason);									  
			break;
		default:
			break;
