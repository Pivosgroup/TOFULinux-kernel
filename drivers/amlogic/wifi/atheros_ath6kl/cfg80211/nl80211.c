...
static const struct nla_policy nl80211_policy[NL80211_ATTR_MAX+1] = {
	...
	[NL80211_ATTR_IE_ASSOC_RESP] = { .type = NLA_BINARY,
					 .len = IEEE80211_MAX_DATA_LEN },
        [NL80211_ATTR_DISABLE_11B_RATES] = { .type = NLA_U8 },
        [NL80211_ATTR_SET_WMM] = { .type = NLA_U8 },
        [NL80211_ATTR_AP_SET_UAPSD] = { .type = NLA_U8 },
        [NL80211_ATTR_SET_TX_SGI_MASK] = { .type = NLA_BINARY,
					   .len = 8 },
	[NL80211_ATTR_SET_TX_SGI_PER_THRESHOLD] = { .type = NLA_U8 },
};
...
static int nl80211_disable_11b_rates(struct sk_buff *skb, struct genl_info *info)
{
        struct cfg80211_registered_device *rdev = info->user_ptr[0];
        struct net_device *dev = info->user_ptr[1];
        struct wireless_dev *wdev = dev->ieee80211_ptr;
        ...
        if (!info->attrs[NL80211_ATTR_DISABLE_11B_RATES])
                return -EINVAL;
	value = nla_get_u8(info->attrs[NL80211_ATTR_DISABLE_11B_RATES]);

        wdev_lock(wdev);
        if (!rdev->ops->disable_11b_rates) {
                err = -EOPNOTSUPP;
                goto out;
        }

        err = rdev->ops->disable_11b_rates(&rdev->wiphy, dev, value);
 out:
        wdev_unlock(wdev);
        return err;
}

static int nl80211_set_wmm(struct sk_buff *skb, struct genl_info *info)
{
        struct cfg80211_registered_device *rdev = info->user_ptr[0];
        struct net_device *dev = info->user_ptr[1];
        struct wireless_dev *wdev = dev->ieee80211_ptr;
        ...
        if (!info->attrs[NL80211_ATTR_SET_WMM])
                return -EINVAL;
	value = nla_get_u8(info->attrs[NL80211_ATTR_SET_WMM]);
        ...
        err = rdev->ops->set_wmm(&rdev->wiphy, dev, value);
 out:
        wdev_unlock(wdev);
        return err;
}

static int nl80211_ap_set_uapsd(struct sk_buff *skb, struct genl_info *info)
{
        struct cfg80211_registered_device *rdev = info->user_ptr[0];
        struct net_device *dev = info->user_ptr[1];
        struct wireless_dev *wdev = dev->ieee80211_ptr;
        ...
        if (!info->attrs[NL80211_ATTR_AP_SET_UAPSD])
                return -EINVAL;
	value = nla_get_u8(info->attrs[NL80211_ATTR_AP_SET_UAPSD]);

        wdev_lock(wdev);
        if (!rdev->ops->ap_set_uapsd) {
                err = -EOPNOTSUPP;
                goto out;
        }

        err = rdev->ops->ap_set_uapsd(&rdev->wiphy, dev, value);
 out:
        wdev_unlock(wdev);
        return err;
}
...
static int nl80211_set_tx_sgi(struct sk_buff *skb, struct genl_info *info)
{
        struct cfg80211_registered_device *rdev = info->user_ptr[0];
        struct net_device *dev = info->user_ptr[1];
        struct wireless_dev *wdev = dev->ieee80211_ptr;
	...
	u32* mask;
        ...
        if (!info->attrs[NL80211_ATTR_SET_TX_SGI_MASK])
                return -EINVAL;
	mask = nla_data(info->attrs[NL80211_ATTR_SET_TX_SGI_MASK]);

        if (!info->attrs[NL80211_ATTR_SET_TX_SGI_PER_THRESHOLD])
                return -EINVAL;
	threshold = nla_get_u8(info->attrs[NL80211_ATTR_SET_TX_SGI_PER_THRESHOLD]);

        wdev_lock(wdev);
        if (!rdev->ops->set_tx_sgi) {
                err = -EOPNOTSUPP;
                goto out;
        }

        err = rdev->ops->set_tx_sgi(&rdev->wiphy, dev, mask, threshold);
 out:
        wdev_unlock(wdev);
        return err;
}
...
static struct genl_ops nl80211_ops[] = {
	...
	{
		.cmd = NL80211_CMD_DISABLE_11B_RATES,
		.doit = nl80211_disable_11b_rates,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
	...
	{
		.cmd = NL80211_CMD_SET_WMM,
		.doit = nl80211_set_wmm,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
	...
	{
		.cmd = NL80211_CMD_AP_SET_UAPSD,
		.doit = nl80211_ap_set_uapsd,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
	...
	{
		.cmd = NL80211_CMD_SET_TX_SGI,
		.doit = nl80211_set_tx_sgi,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
