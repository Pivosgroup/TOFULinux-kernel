...
EXPORT_SYMBOL(ieee80211_get_response_rate_ath6kl);

int ieee80211_channel_to_frequency_ath6kl(int chan, enum ieee80211_band band)
{
...
EXPORT_SYMBOL(bridge_tunnel_header_ath6kl);
...
EXPORT_SYMBOL(ieee80211_hdrlen_ath6kl);
...
unsigned int ieee80211_get_hdrlen_from_skb_ath6kl(const struct sk_buff *skb)
{
	...
	if (unlikely(skb->len < 10))
		return 0;
	hdrlen = ieee80211_hdrlen_ath6kl(hdr->frame_control);
	if (unlikely(hdrlen > skb->len))
		return 0;
	return hdrlen;
}
EXPORT_SYMBOL(ieee80211_get_hdrlen_from_skb_ath6kl);
...
EXPORT_SYMBOL(ieee80211_data_to_8023_ath6kl);
			     ...
			     enum nl80211_iftype iftype, u8 *bssid, bool qos)
{
	...
	if (ethertype == ETH_P_AARP || ethertype == ETH_P_IPX) {
		encaps_data = bridge_tunnel_header_ath6kl;
...
EXPORT_SYMBOL(ieee80211_data_from_8023_ath6kl);
			      ...
			      bool has_80211_header)
{
	...
	if (has_80211_header) {
		err = ieee80211_data_to_8023_ath6kl(skb, addr, iftype);
		if (err)
			goto out;
...
EXPORT_SYMBOL(ieee80211_amsdu_to_8023s_ath6kl);
...
EXPORT_SYMBOL(cfg80211_classify8021d_ath6kl);
...
EXPORT_SYMBOL(ieee80211_bss_get_ie_ath6kl);
