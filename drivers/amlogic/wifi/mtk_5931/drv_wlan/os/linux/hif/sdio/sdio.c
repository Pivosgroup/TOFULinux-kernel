...
#endif
#ifdef CONFIG_MT5931_OOB_INTR_ONLY
extern int sdio_io_rw_direct(struct memory_card *card, int write, unsigned fn,
	unsigned addr, u8 in, u8* out);

static int mtk_sdio_claim_irq(struct sdio_func *func)
{
	int ret;
	unsigned char reg;
...
static int mtk_sdio_release_irq(struct sdio_func *func)
{
	int ret;
	unsigned char reg;
...
static void mtk_sdio_irq_worker(struct work_struct *work)
{
    P_GL_HIF_INFO_T prHifInfo = NULL;
    //printk(KERN_DEBUG "%s\n", __FUNCTION__);
	prHifInfo = container_of(work, GL_HIF_INFO_T,
                irq_work);
    sdio_claim_host(prHifInfo->func);
    mtk_sdio_interrupt(prHifInfo->func);
    sdio_release_host(prHifInfo->func);
    enable_irq(INT_GPIO_4);
    return;
}

static irqreturn_t mtk_extern_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    P_GL_HIF_INFO_T prHifInfo = (P_GL_HIF_INFO_T)dev_id;

    //printk(KERN_DEBUG "%s\n", __FUNCTION__);

    disable_irq_nosync(INT_GPIO_4);

	schedule_work(&prHifInfo->irq_work);
...
#ifdef CONFIG_MT5931_OOB_INTR_ONLY
    sdio_claim_host(prHifInfo->func);
    ret = mtk_sdio_claim_irq(prHifInfo->func);
    sdio_release_host(prHifInfo->func);
    
    INIT_WORK(&prHifInfo->irq_work, mtk_sdio_irq_worker);
    disable_irq_nosync(INT_GPIO_4);
    if (request_irq(INT_GPIO_4, (irq_handler_t) mtk_extern_interrupt, 0, "mtk5931", prHifInfo)) {
		printk("request INT_GPIO_4 irq error!!!\n");
		return -1;
	}
#else
    sdio_claim_host(prHifInfo->func);
    ...
    )
{
    ...
    prHifInfo = &prGlueInfo->rHifInfo;
#ifdef CONFIG_MT5931_OOB_INTR_ONLY
    free_irq(INT_GPIO_4, prHifInfo);

    sdio_claim_host(prHifInfo->func);
    ret = mtk_sdio_release_irq(prHifInfo->func);
    sdio_release_host(prHifInfo->func);
#else
    sdio_claim_host(prHifInfo->func);
