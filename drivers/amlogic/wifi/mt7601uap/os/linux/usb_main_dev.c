...
static void rt2870_late_resume(struct early_suspend *early)
{
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("we need to load firmware\n"));
		VIRTUAL_IF_UP(pAd);
	}
...
void RTUnregisterEarlySuspend(POS_COOKIE pOs_cookie)
{
        ...
        if (pOs_cookie->early_suspend.suspend)
                unregister_early_suspend(&pOs_cookie->early_suspend);
	...
	struct usb_interface *intf)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s, We has already register earlysuspend, just check isFirmwareLoaded\n", __func__));
	return 0;
#endif /* CONFIG_HAS_EARLYSUSPEND */
