...
loadfw_protect:
	...
	DataBuffer = RTUSB_URB_ALLOC_BUFFER(pObj->pUsb_Dev, 4096, &DataDMA);
	...
	while (1)
	{
		...
		SentLen = (ILMLen - CurLen) >= 3840 ? 3840 : (ILMLen - CurLen);
	...
	while (1)
	{
		...
		SentLen = (DLMLen - CurLen) >= 3840 ? 3840 : (DLMLen - CurLen);
...
error2:
	...
	RTUSB_URB_FREE_BUFFER(pObj->pUsb_Dev, 4096, 
								DataBuffer, DataDMA);
