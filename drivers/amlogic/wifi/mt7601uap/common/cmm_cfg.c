	...
	switch(cmd)
	{
       ...
       /* Set some preconfigured value before interface up*/
		...
		/* interface up */
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("================> UP: VIRTUAL_IF_NUM == %d\n", VIRTUAL_IF_NUM(pAd)));
		...
		/* interface down */
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("================> DOWN: VIRTUAL_IF_NUM == %d\n", VIRTUAL_IF_NUM(pAd)));
//			VIRTUAL_IF_DEC(pAd);
			if (VIRTUAL_IF_NUM(pAd) == 1) {
				pInfConf->rt28xx_close(pAd->net_dev);
				VIRTUAL_IF_DEC(pAd);
			}
			else if (VIRTUAL_IF_NUM(pAd) > 1){
				VIRTUAL_IF_DEC(pAd);
			}
		}
			break;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
		case CMD_RTPRIV_IOCTL_SET_SUSPEND_FLAG:
		{
			POS_COOKIE pOS_cookie;
			pOS_cookie = (POS_COOKIE) pAd->OS_Cookie;
			pOS_cookie->In_suspend = TRUE;
			printk("==================================> CMD_RTPRIV_IOCTL_SET_SUSPEND_FLAG %d\n", pOS_cookie->In_suspend);
		}
			break;

		case CMD_RTPRIV_IOCTL_LOAD_FIRMWARE_CHECK:
                /* get load firmware or not */
                {
                        POS_COOKIE pOS_cookie;
                        pOS_cookie = (POS_COOKIE) pAd->OS_Cookie;
                        if (isMCUNeedToLoadFIrmware(pAd))
                                pOS_cookie->isFirmwareNotLoaded = TRUE;//need to load firmware case
                        else
                                pOS_cookie->isFirmwareNotLoaded = FALSE;//firmware is loaded case
                }

			break;
		case CMD_RTPRIV_IOCTL_OS_COOKIE_GET:
		/* get main net_dev */
		{
			VOID **ppOS_cookie = (VOID **)pData;
			*ppOS_cookie = (VOID *)(pAd->OS_Cookie);
		}
			break;
#endif
