...
#if defined(CONFIG_ARCH_MESON2) || defined(CONFIG_ARCH_MESON1)
...
#else
#ifdef CONFIG_BT_SLEEP
static unsigned int rts_control_pin = 0;
#endif
...
#ifdef CONFIG_BT_SLEEP
    struct aml_uart_platform *uart_data = pdev->dev.platform_data;  
    //printk("rts_control_pin:%d\n", rts_control_pin);
    if(rts_control_pin)
        gpio_out(rts_control_pin, 0);
#endif
    printk(KERN_INFO "%s\n", __func__);
    return 0;
}
...
static int am_uart_resume(struct platform_device * pdev)
{
    ...
    for(i=0;i<UART_NR;i++)
    {
        ...
        change_speed(up, up->baud);
    }
#ifdef CONFIG_BT_SLEEP
    //printk("rts_control_pin:%d\n", rts_control_pin);
    if(rts_control_pin)
        gpio_out(rts_control_pin, 1);
#endif
...
#ifdef CONFIG_BT_SLEEP
    rts_control_pin = uart_data->rts_control_pin;
#endif
       for(i=0;i<UART_NR;i++)
       {
           if(uart_data->pinmux_uart[i]==NULL)
