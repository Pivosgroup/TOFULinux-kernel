...
static ssize_t error_hanlde(struct class *class, struct class_attribute *attr,	const char *buf, size_t count);
...
//    __ATTR(part_struct,  S_IRUGO | S_IWUSR, show_logic_block_table,    show_address_map_table),
    ...
    __ATTR(error_handle,  S_IRUGO | S_IWUSR , NULL,    error_hanlde),
...
int e_w_r_test(struct aml_nftl_blk_t *aml_nftl_blk)
{
	struct aml_nftl_part_t * part = NULL;
	int ret =0;
    part = aml_nftl_blk->aml_nftl_part;

	ret = nftl_error_handle_test(part);
