...
#define QF9700_RX_MAX_QUEUE_MEMORY (60 * 1518)
#define	QF9700_TX_QLEN(dev) (((dev)->udev->speed == USB_SPEED_HIGH) ? \
			(QF9700_RX_MAX_QUEUE_MEMORY/(dev)->hard_mtu) : 4)
/* ------------------------------------------------------------------------------------------ */
...
static void qf_write_async_callback(struct urb *urb, struct pt_regs *regs)
#else
...
int qf9700_suspend(struct usb_interface *intf, pm_message_t message)
{
	return usbnet_suspend(intf,message);
}

int qf9700_resume (struct usb_interface *intf)
{
	struct sk_buff          *skb;
	struct urb              *res;
	int                     retval;
	...
	if (!--dev->suspend_count) {
		...
		if (test_bit(EVENT_DEV_OPEN, &dev->flags)) {
			if (!(dev->txq.qlen >= QF9700_TX_QLEN(dev)))
...
static struct usb_driver qf9700_driver = {
	...
	.suspend = qf9700_suspend,
	.resume = qf9700_resume,
};
