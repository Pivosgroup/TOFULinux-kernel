...
static int  coodec_status = CLOSED;
*/
/********************************/
...
static struct rt5631_init_reg init_list[] = {
    ...
    {RT5631_ALC_CTRL_1      , 0x0a0f},
    {RT5631_ALC_CTRL_2      , 0x0000},
    {RT5631_ALC_CTRL_3      , 0xe080},      //Target level -4.5db; noise gate thd 39db
    //4-17 [e]
#endif//ALC_FUNC
};
...
hweq_t hweq_param[]={
    ...
    {//HFREQ
        {     0x0,    0x2,     0x3,     0x5,     0x6,    0x7,     0x8,     0x9,     0xb,     0xc,     0xd,     0xe,     0xf,   0x11,   0x12},
        {0x1BBC,0xC9A4,0x1BBC,0x2997,0x142D,0xFCB6,0xEF01,0x1BBC,0xE835,0x0FEC,0xC66E,0x1A29,0x1CEE,0x8002,0x0007},
        0x4014
    },
    {//SPK_FR
        {     0x0,     0x1,    0x2,     0x3,     0x4,     0x5,    0x6,     0x7,     0x8,     0x9,    0xb,     0xd,     0xe,     0xf,   0x11,   0x12},
        {0x1CD4,0xF405,0xe0bf,0x0f9e,0xfa19,0x07CA,0x12AF,0xF805,0xE904,0x1C10,0x1C8B,0xc5e1,0x1afb,0x1d46,0x8002,0x0007},
        0x4090
    },
};
#define RT5631_HWEQ_LEN ARRAY_SIZE(hweq_param)

static int rt5631_hw_eq(struct snd_soc_codec *codec, int index)
{
    int i;

    if(codec == NULL ||  index >= RT5631_HWEQ_LEN)
        return -EINVAL;

    for(i = 0; i < EQ_REG_NUM; i++) {
        if(hweq_param[index].value[i])
            rt5631_index_write(codec, hweq_param[index].reg[i],
                    hweq_param[index].value[i]);
        else
            break;
    }
    snd_soc_write(codec, RT5631_EQ_CTRL, hweq_param[index].ctrl);
...
static int rt5631_call_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
    struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);

    ucontrol->value.integer.value[0] = rt5631->call_mode;

    return 0;
}

static int rt5631_call_put(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
    struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);

    rt5631->call_mode = ucontrol->value.integer.value[0];
    return 0;
}
...
static const struct snd_kcontrol_new rt5631_snd_controls[] = {
    ...
    /* Call Mode */
    SOC_SINGLE_EXT("Call Mode Switch", 0, 0, 1, 0,
        rt5631_call_get, rt5631_call_put),
};
    ...
    struct snd_kcontrol *kcontrol, int event)
{
    ...
    mic1_gain_power = (snd_soc_read(codec, RT5631_PWR_MANAG_ADD2) & RT5631_PWR_MIC1_BOOT_GAIN) >> RT5631_PWR_MIC1_BOOT_GAIN_BIT;
    mic2_gain_power = (snd_soc_read(codec, RT5631_PWR_MANAG_ADD2) & RT5631_PWR_MIC2_BOOT_GAIN) >> RT5631_PWR_MIC2_BOOT_GAIN_BIT;

    switch (event) {
    case SND_SOC_DAPM_POST_PMD:
            //no copy ADC channel
            snd_soc_update_bits(codec, RT5631_INT_ST_IRQ_CTRL_2, 0xc000, 0x0000);
        break;
    case SND_SOC_DAPM_POST_PMU:
            ...
            {
                snd_soc_update_bits(codec, RT5631_INT_ST_IRQ_CTRL_2, 0xc000, 0x4000);
            }
            else if(mic2_gain_power)//use mic2,copy ADC right to left
            {
                snd_soc_update_bits(codec, RT5631_INT_ST_IRQ_CTRL_2, 0xc000, 0x8000);
            }
            else//no copy ADC channel
            {
                snd_soc_update_bits(codec, RT5631_INT_ST_IRQ_CTRL_2, 0xc000, 0x0000);
            }
...
#else
    SND_SOC_DAPM_PGA_E("MIC1 Boost", RT5631_PWR_MANAG_ADD2, RT5631_PWR_MIC1_BOOT_GAIN_BIT, 0, NULL, 0,
            mic_event, SND_SOC_DAPM_POST_PMD | SND_SOC_DAPM_POST_PMU),
    SND_SOC_DAPM_PGA_E("MIC2 Boost", RT5631_PWR_MANAG_ADD2, RT5631_PWR_MIC2_BOOT_GAIN_BIT, 0, NULL, 0,
            mic_event, SND_SOC_DAPM_POST_PMD | SND_SOC_DAPM_POST_PMU),
#endif
...
#endif
#if 0
static int rt5631_codec_digital_mute(struct snd_pcm_substream *substream,int mute)
{
	...
	/**************adjust the volume from large to small**********/
	snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x4f4f);
	snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x5555);

	snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x5f5f);

	snd_soc_write(codec,RT5631_SPK_OUT_VOL,0xe7e7);
	msleep(80);
		...
		if(!hp_pluged){
			...
			snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x5555);
			snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x4f4f);
			snd_soc_write(codec,RT5631_SPK_OUT_VOL,0x4848);
										...
										struct snd_soc_dai * dai)
{
	...
	int hp_pluged;
	rt5631_snd_pdata = pdev->platform_data;
	//printk("***Entered %s:%s,type %s\n", __FILE__,__func__,(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)?"P":"C");
	/* if codec are in using by packback/caputure, not do shutdown */
	if((coodec_status == CLOSED)||  \
		((substream->stream == SNDRV_PCM_STREAM_PLAYBACK)&&(coodec_status&CAPTURE_ENABLE))|| \
		((substream->stream == SNDRV_PCM_STREAM_CAPTURE)&&(coodec_status&PLAYBACK_ENABLE)) )


	{
		//printk("codec already shutdown or used by adc/dac ,status %d,device %s\n", coodec_status,(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)?"P":"C");
		coodec_status &= ((substream->stream == SNDRV_PCM_STREAM_CAPTURE)?(~CAPTURE_ENABLE):(~PLAYBACK_ENABLE));
		return 0;
	}
	...
	mutex_unlock(&lock);
	coodec_status = CLOSED;

	return 0;
}

static int rt5631_codec_startup(struct snd_pcm_substream * substream,
										struct snd_soc_dai * dai)
{
	...
	v_z=snd_soc_read(codec,RT5631_INT_ST_IRQ_CTRL_2);

	if((v_z & 0x0001)== 0x0001)
	{
		printk("**************rt5631******over current************\n");

	}
	if(ref_count){
		if(coodec_status)
		{
			//printk("codec already started ,status %d,device %s\n",coodec_status,(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)?"P":"C");
		...
		snd_soc_write(codec, RT5631_PWR_MANAG_ADD2, 0xfc3e);
		...
		{
			snd_soc_write(codec, RT5631_PWR_MANAG_ADD4, 0xc000);
		}

		rt5631_codec_digital_mute(substream, 0);
		mutex_unlock(&lock);
		coodec_status |=  (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)?PLAYBACK_ENABLE:CAPTURE_ENABLE;
	}else{
...
struct snd_soc_dai_ops rt5631_ops = {
	//.startup = rt5631_codec_startup,
	//.shutdown = rt5631_codec_shutdown,
