...
static int __devinit pcm2bt_platform_probe(struct platform_device *pdev)
{
    int ret;
...
static int __devexit pcm2bt_platform_remove(struct platform_device *pdev)
{   
...
static struct platform_driver pcm2bt_platform_driver = {
	...
	.remove = __devexit_p(pcm2bt_platform_remove),
};
