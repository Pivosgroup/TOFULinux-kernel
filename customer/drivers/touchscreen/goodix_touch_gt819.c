...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	uint8_t cfg_info_len[4] = {sizeof(cfg_info_group1)/sizeof(cfg_info_group1[0]), 
...
COORDINATE_POLL:
	...
	if(touch_num)
	{
		for(index=0; index<touch_num; index++)
		{
            ...
            //printk("ORI: input_x = %d,input_y = %d, input_w = %d\n", input_x, input_y, input_w);
...
static void gt81xx_hardware_reset(struct touch_pdata *pdata)
{
    gpio_out(RST_PORT, 0);
    msleep(10);
    gpio_out(RST_PORT, 1);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts_com->owner = "gt81xx";
	ts_com->hardware_reset = gt81xx_hardware_reset;
