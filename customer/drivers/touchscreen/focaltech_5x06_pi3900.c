...
static int ft5x0x_printk_enable_flag=1;
...
static DEVICE_ATTR(ft5x0xPrintFlag, S_IRUGO, ft5x0x_read, ft5x0x_write);
...
{
//#include "1009_ft_app.i"
// #include "0x3A_20121018_ft_app.i"
//  #include "IT_WH_1024x600_V11_20130218_app.i"
//  #include "IT_BEITAI_YAYI_PI3100_V11_20130304_app.i"
 #include "IT_BEITAI_YAYI_PI3100_V14_20130308_app.i"
//#include "IT_BEITAI_YAYI_PI3100_V16_20130418_app.i"
//#include "IT_BEITAI_YAYI_PI3100_V17_20130419_app.i"
};
    	...
    	for(i=0;i<10;i++)
    		{
			 ...
			 if (reg_val[0] == 0x79 && reg_val[1] == 0x3)
			 	{
			 	 printk("[TSP] Step 3: succes = 0x%x\n",i);
				 break;
			 	}
			 else
			 	{
				 printk("[TSP] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
			 	 printk("[TSP] Step 3: error = 0x%x\n",i);
			 	}
    		}
		if(i==10)
			{
			printk("[TSP] Step 3: return error = 0x%x\n",i);
        	return ERR_READID;
			}
        //i_is_new_protocol = 1;
    }
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
   ...
   printk("--wzy debug tp_fw_ver is %x,host_fm_ver is %x \n",uc_tp_fm_ver,uc_host_fm_ver);
#if 1 
...
{
	u8 buf[FT5X0X_EVENT_MAX * 6 - 2 + 2 + 1 + 1] = {0};
	int ret, i;

	ret = ft5x0x_i2c_rxdata(buf, FT5X0X_EVENT_MAX * 6 - 2 + 2 + 1);
	if (ret < 0) {
...
static void ft5x0x_report_mt_event(struct input_dev *input, struct ts_event *event, int event_num)
{
	...
	for (i=0; i<event_num; i++) {
		...
		input_mt_sync(input);
		//ft5x0x_dbg("point_%d: %d, %d, event = %d\n",event->id, event->x,event->y, event_num);
		event++;
	}
	input_sync(input);
}
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("===== ft5x06 TP test start =====\n");
	err = ft5x_i2c_test(client);
	if(!err){
		printk("!!! ft5x06 TP is not exist !!!\n");
		err = -ENODEV;
		goto exit_check_functionality_failed;
	}
	printk("===== ft5x06 TP test ok=======\n");
...
int update_sensitivity_reg(int in)
{ 
	return 0;
    switch(in)
    {
	case 1:
	  ft5x0x_write_reg(0x80,0x20);
	break;
        case 0:
	  ft5x0x_write_reg(0x80,0x16);
	break;
	default:
	  ft5x0x_write_reg(0x80,0x20);
        break;
    ...
    char tmp = 0;
    ft5x0x_read_reg(0x80,&tmp);
    printk("reg read from 0x80 is %d \n",tmp);
    return 0;	
}
EXPORT_SYMBOL(update_sensitivity_reg);
