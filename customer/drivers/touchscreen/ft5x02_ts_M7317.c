...
#define CALIBRATE_SEQ 1
static int g_need_calibrate = CALIBRATE_SEQ;
struct ts_event {
...
extern int ft5x02_calibrate(struct i2c_client * client);
...
int ft5x02_write_reg(struct i2c_client * client, u8 regaddr, u8 regvalue)
{
  ...
  int i,err;
  for (i=0; i<10; i++) {
	  err = ft5x02_i2c_Write(client, buf, sizeof(buf));
	  if (err < 0) {
      printk("%s: I2C write error! addr=%x, value=%x\n", __func__, regaddr, regvalue);
      return err;
    }
    udelay(200);
	  err = ft5x02_i2c_Read(client, buf, 1, &read_value, 1);
	  if (err < 0) {
      printk("%s: I2C read back error! addr=%x\n", __func__, regaddr);
      return err;
	  }
  
    if (read_value == regvalue) {
      break;
    }
    else {
      printk("%s: addr=%x, write in =%x, read out=%x, retry %d\n", __func__, regaddr, regvalue, read_value, i);
      udelay(200);
      continue;
    }
  }
...
static void ft5x02_report_value(struct ft5x02_ts_data *data)
{
    ...
    {   
         ...
         if(event->au8_touch_event[i]== 0 || event->au8_touch_event[i] == 2) 
        {
            ...
            input_report_abs(data->input_dev, ABS_MT_POSITION_X, 800 - event->au16_x[i]);
    ...
    if(event->touch_point == 0){
        ...
        if (g_need_calibrate) {
            g_need_calibrate--;
            ft5x02_calibrate(data->client);
          	printk("ft5202 calibrate after first finger up!\n");
        }
    }    
	input_sync(data->input_dev);
...
static void ft5x02_ts_suspend(struct early_suspend *handler)
{
	...
	g_need_calibrate = CALIBRATE_SEQ;
...
static void ft5x02_ts_resume(struct early_suspend *handler)
{
	...
	msleep(10);
	ft5x02_write_reg(ts->client,0x00,0x40);
	msleep(150);
  ft5x02_write_reg(ts->client,0x00,0x00);		
  msleep(150);
#if 0
	ft5x02_write_reg(ts->client,0xa5,0x03);
	msleep(20);
	ts->pdata->power(0);
	msleep(20);
	ts->pdata->power(1);
...
//	ft5x02_ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	ft5x02_ts->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
...
#if 1
  ft5x02_calibrate(client);
	printk("ft5202 calibrate completed!\n");

#else
	printk("reset ft5202 with %d\n", buf[0]);
  if (buf[0] <= '2') {
  	ft5x02_write_reg(client,0xa5,0x03);
  	msleep(20);
  }
  if (buf[0] == '2') {
  	ts->pdata->power(0);
