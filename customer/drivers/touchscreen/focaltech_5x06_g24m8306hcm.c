...
static unsigned char  ft5x06_fw_ver;
...
static DEVICE_ATTR(ft5x0xPrintFlag, S_IRUGO|S_IWUSR, ft5x0x_read, ft5x0x_write);
...
{
			//#include "ft_app_g22.ft"
			#include "ft_app_g24m8306hcm-1024x768.ft"
			//#include "ft_Ver0x09_app.ft"
			//#include "TP770_0xXX_FT5406_Ver0x10_20130108_app.ft"
...
static int ft5x0x_get_event(struct ts_event *event)
{
	...
	for (i=0; i<ret; i++) {
	  ...
	  /*event->y = (s16)(buf[3+i*6] & 0x0F)<<8 | (s16)buf[4+i*6];   //EDT 2012/11/19 by ETML0700E8DHA ->800*1280 TO 1280*800 ¥¿±Û90  
		event->x =1280-((s16)(buf[5+i*6] & 0x0F)<<8 | (s16)buf[6+i*6]);   //EDT 2012/11/19 by ETML0700E8DHA ->800*1280 TO 1280*800 ¥¿±Û90  
		
		*/
    event->z = 200;
    event->w = 1;
   // printk("prevent->x1= %d prevent->y1= %d  ocaltechPdata2->xpol %d [%d %d] %d \n",event->x,event->y, focaltechPdata2->xpol,focaltechPdata2->screen_max_x, focaltechPdata2->screen_max_y,i );
...
static void ft5x0x_ts_resume(struct early_suspend *handler)
{
	  ...
	  for(i=0;i<10;i++){
   		 ...
   		 printk("[FST] Firmware  version >>>>>>>>>>>>>> = 0x%x,  i =0x%x \n", uc_reg_value,i);  
	     if(uc_reg_value == ft5x06_fw_ver) break;
   	 msleep(500);
	}
}
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	this_client = client;
       ...
       for(retry=0;retry <= 2; retry++)
       {
             ...
             ret =ft5x0x_read_reg(0,&uc_reg_value);
               if (ret >= 0)
                       break;
               printk( "FT5X06 I2C TEST FAILED!Please check the HARDWARE connect\n");
       }
	   if(ret <= 0)
	  {
