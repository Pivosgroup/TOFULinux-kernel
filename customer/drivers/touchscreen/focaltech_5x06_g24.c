...
static DEVICE_ATTR(ft5x0xPrintFlag, S_IRUGO|S_IWUSR|S_IWGRP, ft5x0x_read, ft5x0x_write);
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    u8  bt_ecc;
    int  j,temp,lenght,packet_number, i = 0, i_ret = 0;
    u32  config_add;
	if(focaltechPdata2->power){
        	focaltechPdata2->power(0);
        	msleep(50);
...
//    mdelay(35);  
	...
	printk("[TSP] Step 2: i_ret = 0x%x i=0x%x\n",i_ret, i);
        delay_qt_ms(5);
    }while(i_ret <= 0 && i < 5 );
...
//	delay_qt_ms(5);
    ...
    for (i = 0; i<6; i++)
    {
        temp = 0x1ff00 + i;
			packet_buf[0] = 0xbf;
			packet_buf[1] = (u8)config_add>>16;
        packet_buf[2] = (FTS_BYTE)((temp & 0xffff)>>8);
        packet_buf[3] = (FTS_BYTE)(temp & 0xff);
        temp =1;
        packet_buf[4] = (FTS_BYTE)(temp>>8);
        packet_buf[5] = (FTS_BYTE)temp;
        packet_buf[6] = pbt_buf[sizeof(CTPM_FW)-8 + i]; 
        //bt_ecc ^= packet_buf[6];
 		printk("\n the last eight bytes  is 0x%x \n", packet_buf[6]);
...
//	disable_irq(IRQ_EINT(6));
   ...
   /***wait CTP to bootup normally***/
    ...
    msleep(200); 
    ft5x0x_read_reg(FT5X0X_REG_FIRMID, &reg_version);
    printk("[TSP] firmware version = 0x%2x\n", reg_version);
   // ft5x0x_read_reg(FT5X0X_REG_REPORT_RATE, &reg_value);
   // printk("[TSP]firmware report rate = %dHz\n", reg_value*10);
   // ft5x0x_read_reg(FT5X0X_REG_THRES, &reg_value);
   // printk("[TSP]firmware threshold = %d\n", reg_value * 4);
   // ft5x0x_read_reg(FT5X0X_REG_NOISE_MODE, &reg_value);
   // printk("[TSP]nosie mode = 0x%2x\n", reg_value);
    ...
    if (fts_ctpm_get_upg_ver() != reg_version)  
    {
        ...
        if (err == 0)
        {
            ...
            msleep(300);
            ft5x0x_read_reg(FT5X0X_REG_FIRMID, &reg_value);
...
deferred_module_init(ft5x0x_ts_init);
#else
