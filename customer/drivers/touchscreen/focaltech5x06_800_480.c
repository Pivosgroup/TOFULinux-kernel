...
static uint32_t time_interval = 30;
static uint32_t step_size = 8;
static ssize_t step_size_store(struct class *cla, struct class_attribute *attr, char *buf, size_t count)
{
    unsigned int tmp;
    int ret;
...
static ssize_t step_size_show(struct class *cla, struct class_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", step_size);
}

static ssize_t time_interval_store(struct class *cla, struct class_attribute *attr, char *buf, size_t count)
{
    unsigned int tmp;
    int ret;
    ret = sscanf(buf, "%u", &tmp);
...
static ssize_t time_interval_show(struct class *cla, struct class_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", time_interval);
}

static struct class_attribute focaltech_ts_class_attrs[] = {
    __ATTR(time_interval, S_IRWXU, time_interval_show, time_interval_store),
    __ATTR(step_size, S_IRWXU, step_size_show, step_size_store),
    __ATTR_NULL,
};

static struct class focaltech_ts_class = {
    .name = "focaltech_ts",
    .class_attrs = focaltech_ts_class_attrs,
};
...
*/
static DEVICE_ATTR(ft5x0xPrintFlag, 0644, ft5x0x_read, ft5x0x_write);
static DEVICE_ATTR(effect, 0644, ft5x0x_read, ft5x0x_write);
static DEVICE_ATTR(upgrade, 0644, ft5x0x_read, ft5x0x_write);
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    FTS_BYTE reg_val[2] = {0}, read_id = 0;
    FTS_DWRD i = 0;
    ...
    if (focaltechPdata2->chip_id == 0) {
	      focaltechPdata2->step12_delay = 30;
        focaltechPdata2->step23_delay = 0;
        read_id = 0x3;
    }
    else {
	      focaltechPdata2->step12_delay = 10;
        focaltechPdata2->step23_delay = 100;
        read_id = focaltechPdata2->chip_id;
    }
    ...
    delay_qt_ms(focaltechPdata2->step12_delay);   
    ...
    delay_qt_ms(focaltechPdata2->step23_delay);
...
//	delay_qt_ms(5);
    ...
    if (reg_val[0] == 0x79 && reg_val[1] == read_id)
    {
...
static inline uint32_t abs_t(uint32_t a, uint32_t b)
{
    if (a >= b) {
        return a - b;    
    }
...
{
	int i;
	uint32_t dx, dy, curr_time;
	
	curr_time = jiffies_to_msecs(jiffies - last_t);
	for (i=0; i<event_num; i++) {
        dx = abs_t(last_x, event->x);
        dy = abs_t(last_y, event->y);
        if (dx*dx + dy*dy >= step_size * step_size || curr_time >= time_interval) {
...
static int __init ft5x0x_ts_init(void)
{
	...
	printk("ret=%d\n",ret);
	class_register(&focaltech_ts_class);
	return ret;
