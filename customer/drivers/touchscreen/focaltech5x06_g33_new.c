...
static	unsigned char uc_reg_value; 
...
#define CTPM_FW       (focaltechPdata2->fw_data)
#define CTPM_FW_SIZE  (focaltechPdata2->fw_size)
    	...
    	for(i=0;i<10;i++)
    		{
			 ...
			 if (reg_val[0] == 0x79 && reg_val[1] == 0x3)
			 	{
			 	 printk("[TSP] Step 3: succes = 0x%x  ID1 = 0x%x,ID2 = 0x%x\n",i,reg_val[0],reg_val[1]);
				 break;
			 	}
			 else
			 	{
			 	 printk("[TSP] Step 3: error = 0x%x ID1 = 0x%x,ID2 = 0x%x\n",i,reg_val[0],reg_val[1]);
			 	}
    		}
		if(i==10)
			{
			printk("[TSP] Step 3: return error = 0x%x ID1 = 0x%x,ID2 = 0x%x\n",i,reg_val[0],reg_val[1]);
        	return ERR_READID;
			}
        //i_is_new_protocol = 1;
    }
...
//	ts =  container_of(handler, struct ft5x0x_ts_data, early_suspend);
    disable_irq(focaltechPdata2->irq);
    msleep(20);
...
static void ft5x0x_ts_resume(struct early_suspend *handler)
{
	...
	unsigned char uc_tp_fm_ver,i;
	unsigned char uc_host_fm_ver = fts_ctpm_get_upg_ver();
	for(i=0; i<100; i++) {
		if(focaltechPdata2->power) {
			focaltechPdata2->power(0);
			msleep(50);
		...
		uc_tp_fm_ver = ft5x0x_read_fw_ver();
		if (uc_tp_fm_ver!=0xa6 && (uc_tp_fm_ver==uc_host_fm_ver || uc_tp_fm_ver==uc_reg_value))
			break;
		printk("[FST] Firmware  version = 0x%x, i = 0x%x \n", uc_tp_fm_ver,i);
		msleep(100); 
	}
    enable_irq(focaltechPdata2->irq);
