/*
 * drivers/input/touchscreen/gsl3680.c
 *
...
//#define GLL_DEBUG
//#define GSL_TIMER
static u32 gsl3680_debug_mask = 1;


#ifdef CONFIG_TOUCHSCREEN_ITO_KEYS
#if defined(CONFIG_TOUCHSCREEN_GSL3680_MARY)
#define BTN1_X_MIN 0
...
struct gsl_ts {
	...
	struct gsl3680_platform_data *pdata;
...
static int gsl3680_chip_init(void)
{
...
static int gsl3680_ts_suspend(void)
{
	gpio_set_status(PAD_GPIOA_6, gpio_status_out);
	gpio_out(PAD_GPIOA_6, 0);
	mdelay(5); 	
...
static int gsl3680_ts_resume(void)
{
	gpio_set_status(PAD_GPIOA_6, gpio_status_out);
  	gpio_out(PAD_GPIOA_6, 1);
	mdelay(20); 
...
static struct gsl3680_platform_data gsl3680_dragon_pdata = {
	.ts_name = "gsl3680",
	.dis_min_x = 0,
	.dis_max_x = 1024,
	.dis_min_y = 0,
	...
	.chip_init = gsl3680_chip_init,
	.suspend = gsl3680_ts_suspend,
	.resume = gsl3680_ts_resume,
};
...
static ssize_t gsl3680_debug_enable_show(
	struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	return sprintf(buf, "0x%x", gsl3680_debug_mask);
}

static ssize_t gsl3680_debug_enable_store(
	struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count)
{
	if (buf[0] >= '0' && buf[0] <= '9')
	{
		gsl3680_debug_mask = (buf[0] - '0');
	}
	else if (buf[0] >= 'a' && buf[0] <= 'f')
	{
		gsl3680_debug_mask = 0x0A + (buf[0] - 'a');
	}
	else
	{
		gsl3680_debug_mask = 0;
	}
	return count;
}

//static DEVICE_ATTR(debug_enable, 0666, gsl3680_debug_enable_show, gsl3680_debug_enable_store);
...
static void gsl_load_fw(struct i2c_client *client)
{
    ...
    source_len = ARRAY_SIZE(GSL3680_CFG);
		
    for (source_line = 0; source_line < source_len; source_line++)
    {
        /* init page trans, set the page val */
        if (GSL_PAGE_REG == GSL3680_CFG[source_line].offset)
	 {
            fw2buf(cur, &GSL3680_CFG[source_line].val);
            gsl_write_interface(client, GSL_PAGE_REG, buf, 4);
            send_flag = 1;
        }
	 else 
	 {
            if (1 == send_flag % (DMA_TRANS_LEN < 0x20 ? DMA_TRANS_LEN : 0x20))
                buf[0] = (u8)GSL3680_CFG[source_line].offset;
...
static void process_gsl3680_data(struct gsl_ts *ts)
{
	u8 id, touches;
	u16 x, y;
	int i = 0;
...
{
	struct gsl_ts *ts;
	struct gsl3680_platform_data *pdata = &gsl3680_dragon_pdata;
	int rc;
	...
	if (rc < 0) {
		dev_err(&client->dev, "GSL3680-D0 init failed\n");
...
deferred_module_init(gsl_ts_init);
#else
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("GSL3680 touchscreen controller driver");
