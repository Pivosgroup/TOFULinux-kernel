...
static int ft5x02_set_other_param(struct i2c_client *client)
{
	...
	err = ft5x02_write_reg(client, FT5X02_REG_THDIFF, g_param_ft5x02.ft5x02_THDIFF>>4);
	if (err < 0) {
...
static int ft5x02_get_other_param(struct i2c_client *client)
{
	...
	if (err < 0) {
		...
		DBG("THDIFF=%02x\n", value<<4);
		if((value<<4) != g_param_ft5x02.ft5x02_THDIFF)
		return -1;
	}
...
int ft5x02_calibrate(struct i2c_client *client)
{
	ft5x02_write_reg(client, FT5x02_REG_DEVICE_MODE, FT5x02_FACTORYMODE_VALUE);
	msleep(100);
	ft5x02_write_reg(client, FT5x02_REG_DEVICE_MODE, FT5x02_WORKMODE_VALUE);
	return 0;
}
...
RETURN_WORK:	
	...
	if (err < 0) {
		...
		if(xvalue != g_param_ft5x02.ft5x02_FACE_DETECT_LAST_TIME)
			goto ERR_EXIT;
	}
...
RETURN_WORK:	
	...
	/*set face detect num*/
	err = ft5x02_set_face_detect_num(client, g_param_ft5x02.ft5x02_FACE_DETECT_NUM);
	if (err < 0) {
		printk("set face detect num failed\n");
		goto ERR_EXIT;
	}
