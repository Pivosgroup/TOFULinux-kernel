...
int ft5x0x_write_reg(u8 addr, u8 para);
int ft5x0x_read_reg(u8 addr, u8 *pdata);
...
EXPORT_SYMBOL(ft5x0x_write_reg);
...
#ifdef CONFIG_ROTATE_180
		event->x = input->absinfo[ABS_MT_POSITION_X].maximum - event->x;
		event->y = input->absinfo[ABS_MT_POSITION_Y].maximum - event->y;
#endif
	if(focaltechPdata2->screen_max_y > focaltechPdata2->screen_max_x)
	{
		input_report_abs(input, ABS_MT_POSITION_X, focaltechPdata2->screen_max_x - event->y);
		input_report_abs(input, ABS_MT_POSITION_Y, event->x);
	}
	else
	{
