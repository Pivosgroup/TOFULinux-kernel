...
#include "skyworth_gslX680_m1701.h"
...
#define IRQ_PORT			INT_GPIO_0//中断引脚配置
...
#define MAX_KEY_NUM     (sizeof(key_array)/sizeof(key_array[0]))//总共有4个key
...
static struct gsl_ts_data devices[] = {
	{
		...
		.z_index = 5,
		.id_index = 7,
		.data_reg = GSL_DATA_REG,//gsl数据寄存器
		.status_reg = GSL_STATUS_REG,//gsl状态寄存器
		.update_data = 0x4,
		.touch_bytes = 4,
		.touch_meta_data = 4,
		.finger_size = 70,
...
struct gsl_ts {
	...
	struct input_dev *input;//工作队列，用于中断上半部
	struct work_struct work;//工作队列，用于中断下半部
...
static int gslX680_chip_init(void)
{
	...
	gpio_irq_set(PAD_GPIOA_16, GPIO_IRQ(INT_GPIO_0-INT_GPIO_0, GPIO_IRQ_RISING));//管脚GPIOA16，上升沿有效
	msleep(20);
	return 0;
}
...
static void reset_chip(struct i2c_client *client)//重置清零
{
	u8 buf[4] = {0x00};
	u8 tmp = 0x88;
...
static void check_mem_data(struct i2c_client *client)//i2c的mem检查，具体作用分析I2C架构
{
	char write_buf;
	char read_buf[4]  = {0};
...
static void process_gslX680_data(struct gsl_ts *ts)
{
	...
	for(i= 0;i < (touches > MAX_FINGERS ? MAX_FINGERS : touches);i ++)
	{
		...
		{
			record_point(x, y , id);
			report_data(ts, x_new, y_new, 10, id);	//这个确定是10到底是什么意思	
			id_state_flag[id] = 1;
		}
	}
	for(i=1;i<=MAX_CONTACTS;i++)
...
schedule:
	enable_irq(ts->irq);//以前这里是被屏蔽掉了，只是往寄存器里写数据
	//控制特殊寄存器的
...
{	
	struct gsl_ts *ts = dev_id;//在这里需要强制转换(struct gsl_ts * )
	xxx++;
	if (ts->is_suspended == true) 
		return IRQ_HANDLED;		//当中断处理程序正确调用，且确实是他对应的设备产生了中断，返回这个
		//在这里貌似有点不妥
	//print_info("==========GSLX680 Interrupt============\n");				 

	disable_irq_nosync(ts->irq);//不需要等待中断处理完，直接屏蔽中断源。
	//原来的程序在这里没有中断屏蔽中断，这样做得原因可能是怕被打断。

	if (!work_pending(&ts->work)) //gsl_ts_xy_worker这才是实际工作的函数
	{
		queue_work(ts->wq, &ts->work);//下半部中断处理
	}
...
static void gsl_timer_handle(unsigned long data)
{
	...
	disable_irq_nosync(ts->irq);	//中断屏蔽，这里改动的原因还有待分析
...
static int gsl_ts_init_ts(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	if (ts->device_id == 0) {
		...
		ts->dd->touch_index = 0;//在这里有点模糊，稍后回来研究?
	}
	...
	input_device = input_allocate_device();//分配一个input 子系统设备
	if (!input_device) {
		rc = -ENOMEM;
		goto error_alloc_dev;//分配失败则释放掉ts->邋touch_data
	}
	
	ts->input = input_device;
	input_device->name = GSLX680_I2C_NAME;//名称
	//这里是否应该加一个设备在系统中的物理路径
	//例如memcpy(input_device->phys, "input/ts", 8);
	input_device->id.bustype = BUS_I2C;//总线的类型在input_type中
	input_device->dev.parent = &client->dev;
	input_set_drvdata(input_device, ts);//这个应该是把申请的input和ts关联起来

#ifdef REPORT_DATA_ANDROID_4_0
	__set_bit(EV_ABS, input_device->evbit);//支持绝对坐标等等
	...
	input_set_abs_params(input_device,ABS_MT_POSITION_X, 0, SCREEN_MAX_X, 0, 0);//1280
	input_set_abs_params(input_device,ABS_MT_POSITION_Y, 0, SCREEN_MAX_Y, 0, 0);//800
	input_set_abs_params(input_device,ABS_MT_TOUCH_MAJOR, 0, PRESS_MAX, 0, 0);//住接触面的长轴
	input_set_abs_params(input_device,ABS_MT_WIDTH_MAJOR, 0, 200, 0, 0);//手指触摸面积的大小

	client->irq = IRQ_PORT,//中断INT_GPIO_0
	ts->irq = client->irq;//这个中断号在probe的时候用于中断注册
		//printk("444444444444444444444444444\n");
	ts->wq = create_singlethread_workqueue("kworkqueue_ts");//单线程的工作队列
	if (!ts->wq) {
	...
	INIT_WORK(&ts->work, gsl_ts_xy_worker);//创建一个工作
	//初始化工作队列，这个比较重要，中断触发后，调用队列中的goodix_ts_work_func函数，计算上报坐标值； 
	rc = input_register_device(input_device);//注册input的设备
	if (rc)
			...
			const struct i2c_device_id *id)
{
	...
	rc=  request_irq(client->irq, gsl_ts_irq, IRQF_DISABLED, client->name, ts);//设置I2C的中断请求函数
	//快速中断处理程序，快速中断处理时屏蔽所有中断
	if (rc < 0) {
	...
	ts->early_suspend.suspend = gsl_ts_early_suspend;//先于suspend之前suspend 
...
static const struct i2c_device_id gsl_ts_id[] = {
	{GSLX680_I2C_NAME, 0},//这个名字必须与BOARD_INFO的匹配才会调用probe函数
	{}
};
MODULE_DEVICE_TABLE(i2c, gsl_ts_id);

/*
 i2c_driver gsl_ts_driver这个结构对应了驱动方法，如果驱动设备支持好
 几个设备，那么这里面就要包含这些设备的IDi2c_driver，
 .id_table	= gsl_ts_id，id_table是struct i2c_device_id结构的。
*/
static struct i2c_driver gsl_ts_driver = {
	...
	.suspend	= gsl_ts_suspend,//同下
	.resume	= gsl_ts_resume,//因为定义了early，在这里resume其实可以不要
#endif
	.probe		= gsl_ts_probe,
