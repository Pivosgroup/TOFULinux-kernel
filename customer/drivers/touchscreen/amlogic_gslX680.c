...
#include "amlogic_gslX680_ys.h"
#include "amlogic_gslX680_sg.h"
...
static void gsl_load_fw_sg(struct i2c_client *client)
{
	u8 buf[DMA_TRANS_LEN*4 + 1] = {0};
	u8 send_flag = 1;
	u8 *cur = buf + 1;
	u32 source_line = 0;
	u32 source_len = ARRAY_SIZE(GSLX680_FW_sg);

	printk("=============gsl_load_fw_sg start==============\n");

	for (source_line = 0; source_line < source_len; source_line++) 
	{
		/* init page trans, set the page val */
		if (GSL_PAGE_REG == GSLX680_FW_sg[source_line].offset)
		{
			fw2buf(cur, &GSLX680_FW_sg[source_line].val);
		...
		else 
		{
			if (1 == send_flag % (DMA_TRANS_LEN < 0x20 ? DMA_TRANS_LEN : 0x20))
	    			buf[0] = (u8)GSLX680_FW_sg[source_line].offset;
	...
	printk("=============gsl_load_fw_sg end==============\n");

}

static void gsl_load_fw_ys(struct i2c_client *client)
{
	u8 buf[DMA_TRANS_LEN*4 + 1] = {0};
	u8 send_flag = 1;
	u8 *cur = buf + 1;
	u32 source_line = 0;
	u32 source_len = ARRAY_SIZE(GSLX680_FW_ys);

	printk("=============gsl_load_fw_ys start==============\n");

	for (source_line = 0; source_line < source_len; source_line++) 
	{
		/* init page trans, set the page val */
		if (GSL_PAGE_REG == GSLX680_FW_ys[source_line].offset)
		{
			fw2buf(cur, &GSLX680_FW_ys[source_line].val);
			gsl_write_interface(client, GSL_PAGE_REG, buf, 4);
			send_flag = 1;
		}
		else 
		{
			if (1 == send_flag % (DMA_TRANS_LEN < 0x20 ? DMA_TRANS_LEN : 0x20))
	    			buf[0] = (u8)GSLX680_FW_ys[source_line].offset;
	...
	printk("=============gsl_load_fw_ys end==============\n");

}

static void gsl_load_fw(struct i2c_client *client)
{
	//get the gpio pin data
	/*
			GPIOC_6  	GPIOC_5
		ys	    0			    0
		cg	    1			    1
	*/
	...
	int32_t data_gpioc5, data_gpioc6;

	data_gpioc5 = gpio_in_get(PAD_GPIOC_5);
	data_gpioc6 = gpio_in_get(PAD_GPIOC_6);

	printk("\n*******************************************\n");
	printk("get data_gpioc5 : %d, data_gpioc6:%d\n", data_gpioc5, data_gpioc6);

	if((data_gpioc5 == 1) && (data_gpioc6 == 1))
		gsl_load_fw_sg(client);
	else
		gsl_load_fw_ys(client);
