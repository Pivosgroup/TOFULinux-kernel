...
{
	  char *tmp_buf = NULL, *pre_tmp = NULL;
	  int i = 0,count = 0;
    if (!strcmp(attr->attr.name, "ft5x0xPrintFlag")) {
        ...
        return sizeof(ft5x0x_printk_enable_flag);
    	   ...
    	   if (pre_tmp == NULL) pre_tmp = (FTS_BYTE *)kmalloc(20*sizeof(FTS_BYTE), GFP_KERNEL); 
         if (pre_tmp == NULL) {
             printk("Insufficient memory in upgrade!\n");   
             return -ERR_REV_FILE_FAIL;
         }  
         tmp_buf = pre_tmp;            
         *tmp_buf++ = ft5x0x_read_fw_info(FT5X0X_REG_PERIODACTIVE);         
         *tmp_buf++ = ft5x0x_read_fw_info(FT5X0X_REG_FIRMID);
         *tmp_buf++ = ft5x0x_read_fw_info(FT5X0X_REG_CIPHER);	
         *tmp_buf++ = ft5x0x_read_fw_info(FT5X0X_REG_FT5201ID);     
             	   	       	               
         count = tmp_buf-pre_tmp;                  
         for (i=0; i<count; i++) {
         	   tmp_buf--;     
             snprintf(buf++,sizeof(FTS_BYTE)+1,"%x",*tmp_buf/16);                 
             snprintf(buf++,sizeof(FTS_BYTE)+1,"%x",*tmp_buf%16);          
         } 
...
static ssize_t ft5x0x_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strcmp(attr->attr.name, "ft5x0xPrintFlag")) {
		...
		if (buf[0] == '2') {
			...
			if(focaltechPdata2->reset){
				focaltechPdata2->reset(0);
				msleep(15);
			}
