...
struct cali cali_arry[CALI_NUM];
#define SAMPLES 4
...
static ssize_t DMT_delay_acc_store( struct device *dev, struct device_attribute *attr,char const *buf, size_t count)
{
	...
	if (false == get_value_as_int64(buf, count, &data))
		return -EINVAL;
	atomic_set(&devdata.delay, (unsigned int) (data >> 1));
	dmtprintk(KERN_INFO "Driver attribute set delay =%lld\n",data);
	return count;
}

static struct device_attribute DMT_attributes[] = {
	__ATTR(enable, 0664, DMT_enable_acc_show, DMT_enable_acc_store),
	__ATTR(delay,  0664, DMT_delay_acc_show,  DMT_delay_acc_store),
	__ATTR_NULL,
};
...
*/
#define G_CONST DEFAULT_SENSITIVITY
struct sample gsensor_apply_cali(struct sample data)
{
    int i = 0;
    for (i = 0; i < CALI_NUM; i++) {
        if (cali_arry[i].valid == 0) break;
        if (data.x > 0)
            data.x = data.x - cali_arry[i].xoffset_p;
        else
            data.x = data.x - cali_arry[i].xoffset_n;
        if (data.y > 0)
            data.y = data.y - cali_arry[i].yoffset_p;
        else
            data.y = data.y - cali_arry[i].yoffset_n;
        if (data.z > G_CONST)
            data.z = data.z - cali_arry[i].zoffset_p;
        else
            data.z = data.z - cali_arry[i].zoffset_n;
    }
    return data;
}
...
{	
    ...
    int current_num = 0;
    int gsensor_check_num = 0;
    struct cali _cali_arry[CALI_NUM];
    int xoffset1 = 0;
    int yoffset1 = 0;
    int zoffset1 = 0;
    int num1x = 0;
    ...
    int num2y = 0;
    int num2z = 0;
    memset(_cali_arry, 0, sizeof(_cali_arry));
    while(current_num < CALI_NUM) {
		device_i2c_read_xyz(devdata.client, xyz);
        data.x = xyz[0];
        data.y = xyz[1];
        data.z = xyz[2];

        for (i = 0; i < CALI_NUM; i++) {
            if (_cali_arry[i].valid == 0) break;

            if (data.x > 0)
                data.x = data.x - _cali_arry[i].xoffset_p;
            else
                data.x = data.x - _cali_arry[i].xoffset_n;
            if (data.y > 0)
                data.y = data.y - _cali_arry[i].yoffset_p;
            else
                data.y = data.y - _cali_arry[i].yoffset_n;
            if (data.z > G_CONST)
                data.z = data.z - _cali_arry[i].zoffset_p;
            else
                data.z = data.z - _cali_arry[i].zoffset_n;
  	}
	/* calculate averages */
		
        if (gsensor_check_num++ < SAMPLE_NUM) {
            if (data.x > 0) {
                xoffset1 += data.x;
                num1x++;
            } else {
            ...
            if(data.z < 0)
            {//If z is less than 0, compare it with -G_CONST to compute the offset.
                if (data.z > -G_CONST) {
                    zoffset1 += (data.z + G_CONST);
                    num1z++;
                } else {
                    zoffset2 += (data.z + G_CONST);
                    num2z++;
                }
            }
            else
        ...
        gsensor_check_num = 0;
        if (num1x > 0)
            xoffset1 = (xoffset1 + (num1x>>1))/num1x;
        else
            xoffset1 = 0;
        if (num1y > 0)
            yoffset1 = (yoffset1 + (num1y>>1))/num1y;
        else
            yoffset1 = 0;
        ...
        if (num1z > 0)
            ...
            zoffset1 = 0;
        if (num2x > 0)
            xoffset2 = (xoffset2 - (num2x>>1))/num2x;
        else
            xoffset2 = 0;
        if (num2y > 0)
            yoffset2 = (yoffset2 - (num2y>>1))/num2y;
        else
            yoffset2 = 0;
        if (num2z > 0)
            zoffset2 = (zoffset2 - (num2z>>1))/num2z;
        else
            zoffset2 = 0;
        _cali_arry[current_num].xoffset_p  = xoffset1;
        _cali_arry[current_num].xoffset_n = xoffset2;
        _cali_arry[current_num].yoffset_p  = yoffset1;
        _cali_arry[current_num].yoffset_n = yoffset2;
        _cali_arry[current_num].zoffset_p  = zoffset1;
        _cali_arry[current_num].zoffset_n = zoffset2;
        _cali_arry[current_num++].valid  = 1;
        num1x = num2x = num1y = num2y = num1z = num2z = 0;
        xoffset1 = xoffset2 = yoffset1 = yoffset2 = zoffset1 = zoffset2 = 0;
        {
            int ii = 0;
            for(;ii < CALI_NUM ; ii++)
            {
                printk("(%d %d %d), (%d, %d, %d), %d\n",
                _cali_arry[ii].xoffset_p, _cali_arry[ii].yoffset_p, _cali_arry[ii].zoffset_p,
                _cali_arry[ii].xoffset_n, _cali_arry[ii].yoffset_n, _cali_arry[ii].zoffset_n,
                _cali_arry[ii].valid);
	}
        }
  	}
    memcpy(cali_arry, _cali_arry, sizeof(_cali_arry));
	return 0;
}
...
struct miscdevice dmard10_device =
{
...
static void DMT_work_func(struct work_struct *fakework)
{
    ...
    data = gsensor_apply_cali(data);
	PRINT_X_Y_Z(xyz[0], xyz[1], xyz[2]);
	PRINT_X_Y_Z(offset.u.x, offset.u.y, offset.u.z);
	input_report_abs(devdata.input, ABS_X, data.x);
	input_report_abs(devdata.input, ABS_Y, -data.y);
	input_report_abs(devdata.input, ABS_Z, -data.z);
	input_sync(devdata.input);
...
static int __devinit device_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    ret = misc_register(&dmard10_device);
  	if(ret < 0)
	{
		dmtprintk(KERN_INFO "%s, add misc device error, ret %d\n", __func__, ret);
		return ret;
	}
...
void gsensor_write_offset_to_file(void)
{
	...
	fp = filp_open(OffsetFileName, O_RDWR | O_CREAT, 0664);
	if(IS_ERR(fp))
	{
	...
	else
	{
		...
		fp->f_op->write(fp,cali_arry,sizeof(cali_arry), &fp->f_pos);
 		filp_close(fp,NULL);
	}
...
void gsensor_read_offset_from_file(void)
{
	...
	if(IS_ERR(fp))
	{
		...
		fp->f_op->read(fp,cali_arry,sizeof(cali_arry), &fp->f_pos);
		offset.u.z=uz;
		filp_close(fp,NULL);
	}
	set_fs(orgfs);
