...
static struct battery_parameter g17_battery = {
    ...
    .pmu_battery_rdc = 91,     ////150,
    .pmu_battery_cap = 3710,    ////8000,
...
static int led_control(int flag)
{
	switch(flag){
...
static pinmux_item_t hi253_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t hi253_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &hi253_pins[0]
};
static void hi253_v4l2_init(void)
{
    pinmux_set(&hi253_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock

    msleep(20);
    printk("hi253_v4l2_init \n");
    //camera reset to low
    //Camera_PWR_DWN
    ...
    //msleep(20);
    //camera reset to high	
}
static void hi253_v4l2_uninit(void)
{
printk("hi253_v4l2_uninit \n");
    gpio_out(PAD_GPIOZ_0, 1);    // reset IO
    msleep(20);
    //Camera_PWR_DWN
    gpio_out(PAD_GPIOE_10, 1);
...
void hi253_flash_on(void)
{
	printk("hi253 flash on\n");
...
void hi253_flash_off(void)
{
	printk("hi253 flash off\n");
	//gpio_out(PAD_GPIOD_2, 0);
	//gpio_out(PAD_GPIOAO_6, 0);  
	gpio_out(PAD_GPIOC_4,0);
}
void hi253_flash_torch(void)                                                                   
{                                                                                             
    printk("flash off\n");                                                                    
    gpio_out(PAD_GPIOAO_6, 1); 
    gpio_out(PAD_GPIOD_2, 0);                                                                 
...
aml_plat_flashlight_data_t hi253_flashlight_data = {
	.flashlight_on	= hi253_flash_on,
	.flashlight_off	= hi253_flash_off,
	.flashlight_torch = hi253_flash_torch,
...
aml_plat_cam_data_t video_hi253_data = {
	...
	.flash_support = 1,
    .flash_ctrl = &hi253_flashlight_data, 
};
#endif
...
static void sp0838_v4l2_early_suspend(void)
{
}

static void sp0838_v4l2_late_resume(void)
{
}
...
aml_plat_cam_data_t video_sp0838_data = {
	...
	.early_suspend = sp0838_v4l2_early_suspend,
	.late_resume = sp0838_v4l2_late_resume,
};
#endif
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GT2005)
...
static void sp2518_v4l2_disable(void)
{
...
static aml_plat_cam_data_t video_sp2518_data = {
    ...
    .device_disable=sp2518_v4l2_disable,
...
static struct i2c_board_info __initdata aml_i2c_bus_info_a[] = {
    ...
    {  
        I2C_BOARD_INFO("hm2057_i2c",  0x24 ),
        .platform_data = (void *)&video_hm2057_data
    },
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_HM5065
    {
        I2C_BOARD_INFO("hm5065_i2c",  0x1f ),
        .platform_data = (void *)&video_hm5605_data
    },
#endif
...
#endif
#ifdef CONFIG_GSl1680_CAPACITIVE_TOUCHSCREEN_GT7260
    {
...
static struct i2c_board_info __initdata aml_i2c_bus_info_c[] = {
...
void print_pinmux_reg()
{
	//GPIOD_7 bit 23
	printk("GPIOD_3 INPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_I, 19, 1));
	printk("GPIOD_3 OUTPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_O, 19, 1));
	printk("GPIOD_3 RWPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_EN_N, 19, 1));
	
	gpio_set_status(PAD_GPIOD_3, gpio_status_out);
	...
	*/
	...
	//GPIOD_7 bit 23
	printk("GPIOD_7 INPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_I, 23, 1));
	printk("GPIOD_7 OUTPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_O, 23, 1));
	printk("GPIOD_7 RWPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_EN_N, 23, 1));

	printk("GPIOD_8 INPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_I, 24, 1));
	printk("GPIOD_8 OUTPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_O, 24, 1));
	printk("GPIOD_8 RWPUT:%d\n", aml_get_reg32_bits(P_PREG_PAD_GPIO2_EN_N, 24, 1));

	//gpio_set_status(PAD_GPIOD_7, gpio_status_out);
	ret = gpio_out(PAD_GPIOD_7, 0);
	printk("set GPIOD_7(%d) ret:%d\n", PAD_GPIOD_7, ret);

	//gpio_set_status(PAD_GPIOD_8, gpio_status_out);
	gpio_out(PAD_GPIOD_8, 1);
	printk("set GPIOD_8(%d) ret:%d\n", PAD_GPIOD_8, ret);
	...
	#if 0
	//added by zam for read gpio regs
	printk("P_PIN_MUX_REG_0:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_0));
	printk("P_PIN_MUX_REG_1:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_1));
	printk("P_PIN_MUX_REG_2:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_2));
	printk("P_PIN_MUX_REG_3:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_3));
	printk("P_PIN_MUX_REG_4:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_4));
	printk("P_PIN_MUX_REG_5:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_5));
	printk("P_PIN_MUX_REG_6:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_6));
	printk("P_PIN_MUX_REG_7:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_7));
	printk("P_PIN_MUX_REG_8:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_8));
	printk("P_PIN_MUX_REG_9:0x%x\n", aml_read_reg32(P_PERIPHS_PIN_MUX_9));
	#endif
#endif
}

EXPORT_SYMBOL(print_pinmux_reg);
...
MACHINE_START(MESON6_GT7260, "Amlogic Meson 6 gt7260 board")
