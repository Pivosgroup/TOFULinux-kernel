...
extern int m6g30N87A_lcd_init();
#endif
//#define CONFIG_AMLOGIC_MICRON_NYNIX_NAND_4_CHIPS
...
//	{KEY_MENU,		"menu", CHAN_4, 9, 40},
	{KEY_BACK,		"vol-", CHAN_4, 150, 40},
	{KEY_MENU,		"vol+", CHAN_4, 275, 40},
	{KEY_VOLUMEDOWN,	"vol-", CHAN_4, 150, 40},
	{KEY_VOLUMEUP,		"vol+", CHAN_4, 275, 40},
//	{KEY_BACK,		"back", CHAN_4, 390, 40},
//	{KEY_HOME,		"home", CHAN_4, 500, 40},
};
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_N87A
#include <linux/ft5x06_n87a.h>
...
{
	gpio_set_status(GPIO_FT_WAKEUP, gpio_status_out);
	gpio_out(GPIO_FT_WAKEUP, on);
}
...
static struct aml_camera_i2c_fig1_s gc0308_custom_init_script[] = {
	{0xfe,0x00},
	{0x14,0x12},  //13-h--flip-12; 10-v--flip-11
	{0xff,0xff},
};
...
static int gc2035_init(void)
{
	...
	gpio_out(PAD_GPIOE_10, 1);				// set camera power high, disable
	msleep(20);

	//gpio_out(PAD_GPIOA_21, 1);					//power on: 2.8V, 1.8V
	//msleep(20);

	gpio_out(PAD_GPIOE_10, 0);				// set camera power low, enable
	msleep(20);
...
static struct aml_camera_i2c_fig1_s gc2035_custom_init_script[] = {
	{0xfe,0x00},
	{0x17,0x17},//0x14   normal , 0x15 h--flip, =0x16  v--flip, =0x17
	{0xff,0xff},
};
...
static struct i2c_board_info __initdata aml_i2c_bus_info_b[] = {
#ifdef CONFIG_SND_SOC_WM8960
    {
		...
		.platform_data = (void *)&wm8960_codec_data,
    },
#endif
...
MACHINE_START(MESON6_G12, "Amlogic Meson6 Yuke N87A customer platform")
