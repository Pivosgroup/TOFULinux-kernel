/*
 * arch/arm/mach-meson6/board-meson6-g25.c
 *
...
#include "board-m6g25d-M8000NBD.h"
...
#define CONFIG_VDDAO_NORMAL_VOLTAGE 1100
...
static int g25dlvds_pmu_call_back(void *para)
{
    // add your call back implement here, more information please see file 'axp-mfd.h'    
}

void g25d_led_control(int flag)
{
    if (flag == 1)
    {
        //aml_clr_reg32_mask(P_AO_RTI_PULL_UP_REG, (1 << 10));
        aml_clr_reg32_mask(0xfa004004,(1 << 31));
    ...
    }else if (flag == 2)
    {
		//aml_clr_reg32_mask(P_AO_RTI_PULL_UP_REG, (1 << 10));
		aml_clr_reg32_mask(0xfa004004,(1 << 31));
        gpio_out(PAD_GPIOAO_10, 1);       
    }else if (flag == 3)
    {
		//aml_clr_reg32_mask(0xda004004, (1 << 31));
		//disable jtag tdi pinmux
		aml_clr_reg32_mask(0xfa004004,(1 << 31));
		
	}else if (flag == 4)
	{
		aml_set_reg32_mask(0xfa004004,(1 << 31));       		
	}
}
...
static struct axp_supply_init_data axp_sply_init_data = {
	 ...
	 */
	.led_control = g25d_led_control,
    .soft_limit_to99 =  0,
    .para            =  NULL,                       // para will pass to g24_pmu_call_back
    .pmu_call_back   =  g25dlvds_pmu_call_back,
    .board_battery   = &g25dlvds_battery,
};
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_M8000
...
static int sp2518_back_have_inited = 0;
#endif
...
static int sp0838_have_inited = 0;
static int sp0838_init(void)
{
...
static struct aml_camera_i2c_fig1_s sp0838_custom_init_script[] = {
//    {0x14,0x10  | (GC0308_FLIP<<1) | (GC0308_MIRROR<<0)},
    {0xff,0xff},
};

static aml_plat_cam_data_t video_sp0838_data = {
    .name="video-sp0838",
    .video_nr=1,//1,
    ...
    .custom_init_script = sp0838_custom_init_script,
};
#endif
...
{
	sp2518_back_have_inited=1;
	sp2518_init();
}
static void sp2518_v4l2_uninit(void)
{
	sp2518_back_have_inited=0;
    printk( "amlogic camera driver: sp2518_back_v4l2_uninit. \n");
    gpio_out(PAD_GPIOE_10, 1);
    msleep(5);
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_SP0838)
    if (sp0838_have_inited == 0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
...
struct aml_camera_i2c_fig1_s custom_sp2518_back_script[] = {  
	{0xff,0xff}, 
};
...
aml_plat_cam_data_t video_sp2518_data = {
	...
	//.custom_init_script = custom_sp2518_script,
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_SP2518
    {
        /*sp2518_back i2c address is 0x78/0x79*/
...
//set status
	...
	gpio_set_status(PAD_GPIOX_11,gpio_status_out);
...
{
	DBG_LINE_INFO();
    gpio_set_status(PAD_GPIOX_11,gpio_status_out);//set wifi_en gpio mode out
    if(is_on){
...
MACHINE_START(MESON6_G25_M8000NBD, "Amlogic Meson6 g25d customer platform")
