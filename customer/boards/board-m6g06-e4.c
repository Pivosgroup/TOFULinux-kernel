...
static struct meson_cs_pdata_t vcck_pdata = {
    ...
    .voltage_step_table = {
        1356000, 1330000, 1303000, 1278000,
        1252000, 1226000, 1201000, 1176000,
        1150000, 1124000, 1098000, 1073000,
        1047000, 1022000, 996000, 971000,
    },
    .default_uV = 1356000,
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G06_E4
...
#ifdef CONFIG_IPS_BC8601_TOUCHSCREEN
#include <linux/ips_bc8601.h>
#define IPS_IRQ	INT_GPIO_0

static int ips_init_irq(void)
{
	gpio_set_status(PAD_GPIOA_16, gpio_status_in);
	gpio_irq_set(170, GPIO_IRQ( (IPS_IRQ-INT_GPIO_0), GPIO_IRQ_FALLING));
	return 0;
}

static int ips_reset(void)
{
...
static struct ips_platform_data ips_pdata = {
	.irq = IPS_IRQ,
	.init_irq = ips_init_irq,
	.power = NULL,
	.reset = ips_reset,
	.screen_max_x=1280,
	.screen_max_y=800,
...
{
	printk( "amlogic camera driver: ov5640_v4l2_uninit. \n");
    gpio_out(PAD_GPIOE_10, 1);    // set camera power disable
    msleep(5);
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
static int ov5640_v4l2_probe(struct i2c_adapter *adapter)
{
    int ret = 0;
    unsigned char reg[2];
    ov5640_v4l2_init();    
...
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_HM5065)
static void hm5065_init(void)
{
    pinmux_set(&hm5065_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 0, 7); //XTAL div 2
...
static int hm5065_v4l2_probe(struct i2c_adapter *adapter)
{
    int ret = 0;
    unsigned char reg[2];
    hm5065_v4l2_init();    
...
void hm5065_flash_on(void)
{
	printk("flash on\n");
	gpio_out(PAD_GPIOD_2,1);
}
void hm5065_flash_off(void)
{
	printk("flash off\n");
	gpio_out(PAD_GPIOD_2, 0);
}
aml_plat_flashlight_data_t hm5065_flashlight_data = {
	.flashlight_on	= hm5065_flash_on,
	.flashlight_off	= hm5065_flash_off,
};
static aml_plat_cam_data_t video_hm5065_data = {
    ...
    .device_uninit=hm5065_v4l2_uninit,
		.device_probe = hm5065_v4l2_probe,
    .flash_support = 1,
    .flash_ctrl = &hm5065_flashlight_data,
};
#endif /* CONFIG_VIDEO_AMLOGIC_CAPTURE_HM5065 */
...
static void hi253_v4l2_init(void)
{
    ...
    msleep(20);
	// reset low
	printk( "amlogic camera driver: hi253_v4l2_init. \n");
    //camera reset to low
    //Camera_PWR_DWN
...
static int hi253_v4l2_probe(struct i2c_adapter *adapter)
{
    int ret = 0;
    unsigned char reg = 0;
    hi253_v4l2_init();    
    reg = aml_i2c_get_byte_add8(adapter, 0x20, 0x04);
    pr_info("hi253 dev id is %d\n", reg);
    if (reg == 0x92)
        ret = 1;
    hi253_v4l2_uninit();
    return ret;
...
static struct aml_camera_i2c_fig1_s hi253_custom_init_script[] = {
	{0x11,0x91},  //0x10  11
	{0xff,0xff},
};

static aml_plat_cam_data_t video_hi253_data = {
	.name="video-hi253",
	.video_nr=1,   //    1
	.device_init= hi253_v4l2_init,
	.device_uninit=hi253_v4l2_uninit,
	.custom_init_script = hi253_custom_init_script,
	.device_probe = hi253_v4l2_probe,
};
#endif
...
#endif
#ifdef CONFIG_SENSORS_LTR501
    {
        I2C_BOARD_INFO("LTR501", 0x46>>1),
...
static struct i2c_board_info  __initdata aml_dynamical_i2c_bus_info_a[] = {
    ...
    {
        I2C_BOARD_INFO("hm5065_i2c", 0x3e >> 1),
...
#ifdef CONFIG_SENSORS_MMA8452
static struct i2c_board_info __initdata aml_i2c_bus_info_b_mma8452[] = {
	{
		I2C_BOARD_INFO(MMA8452_I2C_NAME,  MMA8452_I2C_ADDR),
	},
};
...
MACHINE_START(MESON6_G06_e4, "Amlogic Meson6 g06 customer platform")
