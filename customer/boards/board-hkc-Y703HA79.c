...
//#include "board-rocoref.h"
#include "board-hkc-Y703H.h"
...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 86,     ////150,
    .pmu_battery_cap = 3600,    ////8000,
...
static struct regulator_init_data axp_regl_init_data[] = {
    ...
    [vcc_buck2] = {
        .constraints = { /* default 1.24V */
            ...
            .initial_state = PM_SUSPEND_STANDBY,
            .state_standby = {
                .uV = 1540 * 1000,  //axp_cfg.dcdc2_vol * 1000,
                .enabled = 1,
            }
        },
 ...
 * in an lcd on the IM2 for example!).
 */
#define GPIO_AML_PMU_IRQ    ((GPIOAO_bank_bit0_11(2)) << 16 | GPIOAO_bit_bit0_11(2))
static int aml_pmu_init_irq(void)
{
    gpio_direction_input(GPIO_AML_PMU_IRQ);
    gpio_irq_set(PAD_GPIOAO_2, GPIO_IRQ(2, GPIO_IRQ_RISING));
    return 0;
}

static struct battery_parameter g24_battery = {
    .pmu_twi_id             = 2,
    .pmu_irq_id             = AML_PMU_IRQ_NUM,
    .pmu_twi_addr           = AML1212_ADDR,
    .pmu_battery_rdc        = 90,  
    .pmu_battery_cap        = 2855,   
    .pmu_battery_technology = POWER_SUPPLY_TECHNOLOGY_LiFe,
    .pmu_battery_name       = "PTI PL336078",
    .pmu_init_chgvol        = 4200000,	    //set initial charing target voltage
    .pmu_init_chgend_rate   = 10,	        //set initial charing end current rate
    .pmu_init_chg_enabled   = 1,	        //set initial charing enabled
    .pmu_init_adc_freq      = 100,	        //set initial adc frequency
    .pmu_init_adc_freqc     = 100,	        //set initial coulomb adc coufrequency
    ...
    .pmu_usbcur             = 900,
#else
    .pmu_init_chgcur        = 1500000,		//set initial charging current limite
    .pmu_suspend_chgcur     = 1500000,	    //set suspend charging current limite
    .pmu_resume_chgcur      = 1500000,	    //set resume charging current limite
    .pmu_shutdown_chgcur    = 1500000,		//set shutdown charging current limite
    .pmu_usbcur_limit       = 1,
    .pmu_usbcur             = 500,
#endif
    .pmu_usbvol_limit       = 1,
...
static void aml1212_call_back(void *para)
{
    // call back function for PMU work, need implement according your application    
}


static struct aml_pmu_init aml_pmu_init_data = {
    .soft_limit_to99      = 0,
    .charge_timeout_retry = 0,
    .vbus_dcin_short_connect = 0,
    .board_battery     = &g24_battery,
    .para              = NULL,                      // not used right now
    .aml_pmu_call_back = aml1212_call_back,
};
...
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
    ...
    0x0f000d,0x10000c,0x11000b,0x11000b, 
};
...
//add camera front 0308 and back 2035 of lsj 20130514
...
static struct aml_camera_i2c_fig1_s gc0308_custom_init_script[] = {
    {0x14,0x10},//0x11},  //0x10  11
    {0xff,0xff},
};

static aml_plat_cam_data_t video_gc0308_data = {
    .name="video-gc0308",
    .video_nr=0,//   caihua.zhao mofidy for 1,back camera gc2035 to 0,
...
static void gc0309_v4l2_late_resume(void)
{
#if 0
    pinmux_set(&gc0309_pinmux_set);
...
static aml_plat_cam_data_t video_gc0309_data = {
    .name="video-gc0309",
    .video_nr=1,//   caihua.zhao mofidy for 1,back camera gc2035 to 0,
    .device_init= gc0309_v4l2_init,
    .device_uninit=gc0309_v4l2_uninit,
    //.early_suspend = gc0309_v4l2_early_suspend,
    //.late_resume = gc0309_v4l2_late_resume,
    .custom_init_script = gc0309_custom_init_script,
};
	CONFIG_CAM_DEV(gc0309, gc0309_i2c, CAM_GC0309);
#endif
//add camera sp2518 of lsj 20130723
...
static int sp2518_init(void)
{
   ...
   //gpio_out(PAD_GPIOA_21, 1);    //power on: 2.8V, 1.8V
    ...
    msleep(20);
          gpio_out(PAD_GPIOE_10, 1);    // set camera pwdn low, enable
    msleep(20);
	  gpio_out(PAD_GPIOE_10, 0);    // set camera pwdn low, enable
    msleep(20);
...
/*  
  pinmux_set(&sp2518_pinmux_set);
    ...
    msleep(20);
*/
    printk("sp2518_init OK!!!!\n");
    return 0;
}

static int sp2518_v4l2_init(void)
{
    sp2518_have_inited=1;
    sp2518_init();
}
static int sp2518_v4l2_uninit(void)
{
    sp2518_have_inited=0;
    printk( "amlogic camera driver: sp2518_v4l2_uninit. \n");
    gpio_out(PAD_GPIOE_10, 1);    // set camera power disable
    msleep(5);
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2655)
    if (ov2655_have_inited == 0)
...
static pinmux_item_t sp0a19_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t sp0a19_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &sp0a19_pins[0]
};

static int sp0a19_init(void)
{

    gpio_out(PAD_GPIOE_10, 1); 
    pinmux_set(&sp0a19_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
...
static int sp0a19_v4l2_init(void)
{
    sp0a19_have_inited=1;
    sp0a19_init();
}
static int sp0a19_v4l2_uninit(void)
{
...
static struct aml_camera_i2c_fig1_s sp0a19_custom_init_script[] = {
    {0xff,0xff},
};

static aml_plat_cam_data_t video_sp0a19_data = {
    .name="video-sp0a19",
    .video_nr=1,//   caihua.zhao mofidy for 1,back camera gc2035 to 0,
    ...
    //.custom_init_script = sp0a19_custom_init_script,
};
#endif
...
static void sp0a19_init(void)
{
    ...
    // reset low
    printk( "amlogic camera driver: sp0a19_v4l2_init. \n");
    gpio_out(PAD_GPIOZ_0, 0);

    // set camera power disanable
    gpio_out(PAD_GPIOE_10, 1);//10
...
{
    sp0a19_have_inited=0;
    gpio_out(PAD_GPIOE_10, 1);    // set camera power disable,10
    msleep(5);

        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}

static void sp0a19_v4l2_disable(void)
{
...
static aml_plat_cam_data_t video_sp0a19_data = {
    ...
    .early_suspend = sp0a19_v4l2_early_suspend,
    .late_resume = sp0a19_v4l2_late_resume,
    .device_disable=sp0a19_v4l2_disable,
};
#endif
*/
...
CONFIG_CAM_DEV(gt2005, gt2005_i2c, CAM_GT2005);
#endif
...
#ifdef CONFIG_GT813_827_828_CAPACITIVE_TOUCHSCREEN_M9308        //// add wang in 20130620 for gt813TP
#include <linux/gt813_827_828_g24m9308cm.h>				
#define GPIO_GOODIX_PENIRQ 	PAD_GPIOA_16 
#define GPIO_GOODIX_RST 	PAD_GPIOC_3
static struct goodix_i2c_rmi_platform_data goodix_ts_pdata = {
    .gpio_rst = GPIO_GOODIX_RST,
    ...
    .xpol = 0,
    .ypol = 1,
    .xmax = 1024, //800   for test
    .ymax = 600,  //480
    //.config_info_len = ARRAY_SIZE(ts_config_data),
    //.config_info = ts_config_data,
};
#endif
#ifdef CONFIG_GOODIX_GT813_827_828_CAPACITIVE_TOUCHSCREEN		
#include <linux/gt813_827_828.h>				
#define GPIO_GOODIX_PENIRQ 	PAD_GPIOA_16 
#define GPIO_GOODIX_RST 	PAD_GPIOC_3
static struct goodix_i2c_rmi_platform_data goodix_ts_pdata = {
    .gpio_rst = GPIO_GOODIX_RST,
...
#endif
#ifdef CONFIG_GOODIX_GT813_N77A_CAPACITIVE_TOUCHSCREEN
#include <linux/gt813_827_828.h>
#define GPIO_GOODIX_PENIRQ 	PAD_GPIOA_16
#define GPIO_GOODIX_RST		PAD_GPIOA_6
static struct goodix_i2c_rmi_platform_data goodix_ts_pdata = {
	.gpio_rst = GPIO_GOODIX_RST,
	.gpio_irq = GPIO_GOODIX_PENIRQ,
	.irq_edge = 0, /* 1:rising edge, 0:falling edge */
	.swap_xy = 0,
	.xpol = 0,
	.ypol = 1,
	.xmax = 800,
...
static s16 d10_sensor_layout[3][3] = {
...
#endif
#ifdef CONFIG_GSL1680_CAPACITIVE_TOUCHSCREEN_ROCO
    {
        I2C_BOARD_INFO("gsl1680", 0x40),
    },
#endif
#ifdef CONFIG_GSL1680_CAPACITIVE_TOUCHSCREEN_HKC_Y703H
    {
        I2C_BOARD_INFO("gsl1680", 0x40),
    },
#endif
#ifdef CONFIG_GT813_827_828_CAPACITIVE_TOUCHSCREEN_M9308
    {
...
//#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_SP2518
//    {
//        /*sp2518 i2c address is 0x42/0x43*/
//        I2C_BOARD_INFO("sp2518_i2c",  0x60 >> 1),
...
#ifdef CONFIG_AML1212
    {
        I2C_BOARD_INFO("aml1212", AML1212_ADDR),
        .platform_data = &aml_pmu_init_data,
        .addr = AML1212_ADDR, //axp_cfg.pmu_twi_addr,
        .irq = INT_WATCHDOG,	//0 smp irq number base change to 32
    },
#endif
};
...
#ifdef CONFIG_SENSORS_DMARD10
    {
        I2C_BOARD_INFO("dmt", 0x18),
        .platform_data = (void *)d10_sensor_layout,
    },
#endif
...
//set status
	...
	//WIFI_WAKE -->1GPIOX_11   in	:
    	gpio_set_status(PAD_GPIOX_10,gpio_status_in);
	//set pull-up
...
#define USB_A_POW_GPIO_BIT_OFF  0
    ...
    #ifdef CONFIG_AML1212 
        if (aml_pmu_init_data.vbus_dcin_short_connect) {
...
static void usb_battery_charger_detect_cb(int bc_mode)
{
    ...
    if (get_uboot_battery_para_status() == UBOOT_BATTERY_PARA_SUCCESS) {
        limit = get_uboot_battery_para()->pmu_usbcur_limit;
        curr  = get_uboot_battery_para()->pmu_usbcur;
    } else {
        limit = g24_battery.pmu_usbcur_limit;
        curr  = g24_battery.pmu_usbcur;
    }
#else
    limit = g24_battery.pmu_usbcur_limit;
    curr  = g24_battery.pmu_usbcur;
#endif
    printk(KERN_INFO "BC MODE: %d, current limit = %d, current:%d\n", bc_mode, limit, curr);
    if (limit) {
        switch(bc_mode){
        ...
        case USB_BC_MODE_SDP:                   /* PC, limit current by your config         */
			...
			aml_pmu_set_usb_current_limit(900, bc_mode);
#else
            aml_pmu_set_usb_current_limit(curr, bc_mode);
#endif
            break;
...
{ 
  ...
  if (g24_battery.pmu_usbcur_limit) { 
    switch(bc_mode){ 
    case USB_BC_MODE_DCP:         /* Charger, do not limit current      */ 
    case USB_BC_MODE_CDP:         /* PC + Charger, don't limit current also */ 
    case USB_BC_MODE_DISCONNECT:     /* Disconnected            */ 
    ...
    case USB_BC_MODE_SDP:         /* PC, limit current by your config    */ 
      axp_charger_set_usbcur_limit_extern(g24_battery.pmu_usbcur); 
      break; 
...
//    CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO2_EN_N, (1<<8));
	if (is_power) 
		{
		    gpio_set_status(PAD_GPIOX_10,gpio_status_out);
			printk(" usb_wifi_power WIFI  enable! \n");
			gpio_out(PAD_GPIOX_10, 0); 
		
		}
	else		
		{
			printk("usb_wifi_power WIFI  Disenable! \n");
			gpio_set_status(PAD_GPIOX_10,gpio_status_in);
			//gpio_out(PAD_GPIOX_10, 1);    
		}
  return 0;
...
#if defined(CONFIG_AM_TV_OUTPUT2)
extern char* get_vout2_mode_internal(void);
#endif
...
int get_display_mode(void) {
	...
	if(strncmp("panel", get_vout_mode_internal(), 5)){
	  ...
	  #if defined(CONFIG_AM_TV_OUTPUT2)
	  if(strcmp("", get_vout2_mode_internal()) && strncmp("null", get_vout2_mode_internal(), 4)){
	    ret = 1;
	  }
	  #endif
	}
...
MACHINE_START(MESON6_G24, "Amlogic Meson6 roco customer platform")
