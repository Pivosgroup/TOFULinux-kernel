...
#if defined(CONFIG_PIVOS_AM_SNAPSHOT) || defined(CONFIG_PIVOS_AM_SNAPSHOT_MODULE)
#define AM_SNAPSHOT_MEM_SIZE        (1920 * 1088 * B32BpP)
#else
#define AM_SNAPSHOT_MEM_SIZE        0
#endif
#define AM_SNAPSHOT_ADDR_START      U_ALIGN(DI_ADDR_END)
#define AM_SNAPSHOT_ADDR_END        (AM_SNAPSHOT_ADDR_START+AM_SNAPSHOT_MEM_SIZE-1)
...
#define PPMGR_ADDR_START	U_ALIGN(AM_SNAPSHOT_ADDR_END)
...
#if defined(CONFIG_PIVOS_AM_SNAPSHOT) || defined(CONFIG_PIVOS_AM_SNAPSHOT_MODULE)
static struct resource aml_snapshot_resources[] = {
    [0] = {
        .start = AM_SNAPSHOT_ADDR_START,
        .end   = AM_SNAPSHOT_ADDR_END,
        .flags = IORESOURCE_MEM,
    },
};

static struct platform_device aml_snapshot_device = {
    .name           = "aml_snapshot",
    .id             = 0,
    .num_resources  = ARRAY_SIZE(aml_snapshot_resources),
    .resource       = aml_snapshot_resources,
};
#endif
...
#if defined(CONFIG_BCM40183_WIFI)  || defined (CONFIG_BT_DEVICE)
    {
        .reg = PINMUX_REG(4),
        .setmask = 0xf << 10
    },
...
#define USB_B_POW_GPIO_BIT  17
    if(is_power_on){
        printk( "set usb_A port power on (board gpio %d)!\n",USB_A_POW_GPIO_BIT);
        printk( "set usb_B port power on (board gpio %d)!\n",USB_B_POW_GPIO_BIT);
        ...
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2012),0,USB_B_POW_GPIO_BIT,1);//mode
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2013),0,USB_B_POW_GPIO_BIT,1);//out

    }
    else    {
        printk("set usb_A port power off (board gpio %d)!\n",USB_A_POW_GPIO_BIT);
        printk("set usb_B port power off (board gpio %d)!\n",USB_B_POW_GPIO_BIT);

        aml_set_reg32_bits(CBUS_REG_ADDR(0x2012),0,USB_A_POW_GPIO_BIT,1);//mode
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2013),1,USB_A_POW_GPIO_BIT,1);//out
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2012),0,USB_B_POW_GPIO_BIT,1);//mode
        aml_set_reg32_bits(CBUS_REG_ADDR(0x2013),1,USB_B_POW_GPIO_BIT,1);//out
    }
}
...
{
    CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO2_EN_N,(1<<17));   //GPIOD_1 is USB_PWR_CTL_HOST
    CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO2_EN_N,(1<<25));   //GPIOD_9 is USB_PWR_CTL
    if(power_on)
    {
        aml_set_reg32_bits(SECBUS2_REG_ADDR(0), 1, 0, 1);// set TEST_n output mode
        aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 1, 31, 1); // set TEST_n pin H
        CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO2_O,(1<<17));  //GPIOD_1 is USB_PWR_CTL_HOST
        CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO2_O,(1<<25));  //GPIOD_9 is USB_PWR_CTL
    }
    else
    {
        aml_set_reg32_bits(SECBUS2_REG_ADDR(0), 1, 0, 1);// set TEST_n output mode
        aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 0, 31, 1);// set TEST_n pin L
        SET_CBUS_REG_MASK(PREG_PAD_GPIO2_O, (1<<17));   //GPIOD_1 is USB_PWR_CTL_HOST
        SET_CBUS_REG_MASK(PREG_PAD_GPIO2_O, (1<<25));   //GPIOD_9 is USB_PWR_CTL
    }
}
