/*
 * arch/arm/mach-meson6/board-meson6-g26.c
 *
...
static void led_control(flag)
{
    ...
    if (flag == 1)
    {
		...
		gpio_set_status(PAD_GPIOAO_10,gpio_status_out);
        gpio_out(PAD_GPIOAO_10, 1);
    }else if (flag == 0)
    {
		//aml_set_reg32_mask(P_AO_RTI_PULL_UP_REG, (1 << 10));
	...
	for(i = 0; i < 5; i++)
	{
		...
		aml_clr_reg32_mask(0xfa004004,(1 << 31));
		//gpio_set_status(PAD_GPIOAO_10,gpio_status_out);
		gpio_out(PAD_GPIOAO_10, i & 0x1);
	}

#endif
}

static int g_usb_chg_type = USB_BC_MODE_DISCONNECT;
static int get_usb_chg_type(void)
{
	return g_usb_chg_type;
}
static void init_usb_chg_type(int usb_chg_type)
{
	g_usb_chg_type = usb_chg_type;
}
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G06_M777ND
...
static struct ts_platform_data ts_pdata = {
	...
	.power = ts_power,
	.screen_max_x=H_ACTIVE,
	.screen_max_y=V_ACTIVE,
	.swap_xy = 0,
	.xpol = 0,
	.ypol = 0,
};
...
MACHINE_START(MESON6_G26_M777ND, "Amlogic Meson6 G26_M777ND customer platform")
