...
static void rt5616_device_init(void)
{
    ...
    /* GPIOA_18 PULL_UP_REG0 bit18 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 1, 18, 1);
}
...
static int rt5616_mic_detect(void)
{
#define MIC_PLUG     1
#define MIC_UNPLUG   0
    ...
    aml_set_reg32_bits(P_PREG_PAD_GPIO0_EN_N, 1, 18, 1);    // mode
    level = aml_get_reg32_bits(P_PREG_PAD_GPIO0_I, 18, 1);  // value
    if (level == 0) {
        val = MIC_PLUG;
    }
    return val;
