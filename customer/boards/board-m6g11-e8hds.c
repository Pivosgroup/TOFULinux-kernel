/*
 * arch/arm/mach-meson6/board-m6g11-e8hds.c
 *
...
{
	#include "../drivers/touchscreen/firmware/GDM_Release_V2.01H.mgar"
};
...
static struct battery_curve board_battery_curve[] = {
...
static struct battery_parameter  gdm_battery = {
...
#else
    .pmu_init_chgcur = 500000,	//700000	//set initial charging current limite
    ...
    .pmu_bat_curve = board_battery_curve,
};
...
static struct regulator_init_data axp_regl_init_data[] = {
    ...
    [vcc_buck2] = {
        .constraints = { /* default 1.24V */
            ...
            .initial_state = PM_SUSPEND_STANDBY,
            .state_standby = {
                .uV = 1525 * 1000,  //axp_cfg.dcdc2_vol * 1000, 1500 * 1000
                .enabled = 1,
            }
        },
...
static int pmu_call_back(void *para)
{
...
static struct axp_supply_init_data axp_sply_init_data = {
    ...
    .soft_limit_to99 =  0,
    .para            =  NULL,                       // para will pass to pmu_call_back
    .pmu_call_back   =  pmu_call_back,
    .board_battery   = &gdm_battery,
};
...
static struct meson_cs_pdata_t vcck_pdata = {
    ...
    .voltage_step_table = {
        1380000, 1350000, 1330000, 1300000,
...
static int front_camera_have_inited = 0;
static int back_camera_have_inited = 0;
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_PAS6329
static pinmux_item_t pas6329_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t pas6329_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &pas6329_pins[0]
};

static int pas6329_init(void)
{
    pinmux_set(&pas6329_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    ...
    printk("pas6329_init OK!!!!\n");
    return 0;
}

static void pas6329_v4l2_init(void)
{
    front_camera_have_inited=1;
    pas6329_init();
}
static void pas6329_v4l2_uninit(void)
{
    front_camera_have_inited=0;
    printk( "amlogic camera driver: pas6329_v4l2_uninit. \n");
    gpio_out(PAD_GPIOE_11, 1);    // set camera power disable
    msleep(5);
	if(back_camera_have_inited == 0)
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
static void pas6329_v4l2_early_suspend(void)
{

}

static void pas6329_v4l2_late_resume(void)
{

}

static struct aml_camera_i2c_fig1_s pas6329_custom_init_script[] = {
    {0x14, 0x11},
    {0x93, 0x44},
    {0xff, 0xff},
};

static aml_plat_cam_data_t video_pas6329_data = {
    .name="video-pas6329",
    .video_nr=0,//1,
    .device_init= pas6329_v4l2_init,
    .device_uninit=pas6329_v4l2_uninit,
    .early_suspend = pas6329_v4l2_early_suspend,
    .late_resume = pas6329_v4l2_late_resume,
    .custom_init_script = pas6329_custom_init_script,
};
#endif
...
{
    back_camera_have_inited=1;
    gt2005_init();
}
static void gt2005_v4l2_uninit(void)
{
    back_camera_have_inited=0;
	if(front_camera_have_inited == 0)
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_PAS6329
    {
        /*pas6329 i2c address is 0x80/0x81*/
        I2C_BOARD_INFO("pas6329_i2c",  0x80 >> 1),
        .platform_data = (void *)&video_pas6329_data,
    },
#endif
...
#ifdef CONFIG_MG8698S_CAPACITIVE_TOUCHSCREEN
    {
       I2C_BOARD_INFO("mg8698s", 0x44),
       .platform_data = (void *)&mg8698s_data,
	   .irq = MG8698S_IRQ,
    },
#endif
};
...
static struct i2c_board_info __initdata aml_i2c_bus_info_b[] = {
	...
	{
		I2C_BOARD_INFO("bma250",  0x08),  // bma250->0x18/bma222->0x08
	},
#endif
#ifdef CONFIG_SENSORS_MMA8452
	{
...
*WIFI_EN			-->GPIOC_7
*WIFI_PWREN			-->GPIOC_8 E8HD-3D
					-->GPIOD_3 E8HD & E8HD-T3
*BT/WLAN_RST		-->GPIOC_9
*WIFI_WAKE		-->GPIOX_11(GB9663)
				-->GPIOX_15(GB9662)
...
#define USB_A_POW_GPIO_BIT_OFF  0
    //aml_clr_reg32_mask(P_PAD_PULL_UP_REG2,1<<25);
    if(is_power_on) {
...
static void m6g11_e8hds_set_vccx2(int power_on)
{
/*
    if (power_on) {
...
static struct meson_pm_config aml_pm_pdata = {
    ...
    .ddr_clk = 0x00110820,
    .sleepcount = 128,
    .set_vccx2 = m6g11_e8hds_set_vccx2,
...
static void wm8960_device_init(void)
{
    ...
    /* GPIOA_19 PULL_UP_REG0 bit19 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 1, 19, 1);
    /* GPIOA_25 PULL_UP_REG0 bit25 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 0, 25, 1);
}
...
#define HP_UNPLUG   0
    ...
    aml_set_reg32_bits(P_PREG_PAD_GPIO0_EN_N, 1, 19, 1);    // mode
    level = aml_get_reg32_bits(P_PREG_PAD_GPIO0_I, 19, 1);  // value
    //printk("wm8960_hp_detect headphone %d\n", level);
    if (level == 0) {
        /* GPIOA_25 headset dectect */
        aml_set_reg32_bits(P_PREG_PAD_GPIO0_EN_N, 1, 25, 1);
        level = aml_get_reg32_bits(P_PREG_PAD_GPIO0_I, 25, 1);  // 0: headset 1: headphone
        //printk("wm8960_hp_detect headset %d\n", level);
        if (level == 0) {
            val = HS_PLUG;
        } else {
            val = HP_PLUG;
...
MACHINE_START(MESON6_G11_E8HDS, "Amlogic Meson 6 g11 e8hds board")
