...
static struct battery_parameter g39emmc10_battery = {
    .pmu_twi_id = 2,		//AXP20_I2CBUS
    .pmu_irq_id = INT_WATCHDOG,
    .pmu_twi_addr = AXP20_ADDR,
    .pmu_battery_rdc = 108,     ////150,
    .pmu_battery_cap = 3620,    ////8000,
    ...
    .pmu_usbcur = 500,
#else
    .pmu_init_chgcur = 700000,		//set initial charging current limite
    ...
    .pmu_charge_efficiency = 99,
     //add this fot NTC
    .pmu_ntc_enable = 1,
    .pmu_ntc_ts_current = 0x03,
    .pmu_ntc_lowtempvol = 2265,
    .pmu_ntc_hightempvol = 384,
    //end NTC config
    .pmu_bat_curve = {
...
static int g39emmc10_pmu_call_back(void *para)
{
		// printk("now val = %d....\n",axp_get_battery_percent());
     static int t = 1;
     if (axp_get_battery_percent() <= 10) {
         gcharger->interval = msecs_to_jiffies(500);
         if (t % 2) {
     	...
     	{
     		aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 0, 31, 1); // set TEST_n pin L			
     		gcharger->interval = msecs_to_jiffies(2*1000); 
     		}
     //add below to Protect the charger :  stop to charge when input is  more that 2.1A;charging again to charge when input less than 1.1 when system need to charge
     //printk("now DC input is %d\n ",gcharger->iac);
     uint8_t val_Protect;
     if (gcharger->iac >= 2300)
     	{
     		axp_reg_read(0x33, &val_Protect);
     		//printk("1old_val_Protect = %d\n",val_Protect);
     		val_Protect &= 0x7f;
     		//printk("1new_val_Protect = %d\n",val_Protect);     		
     		axp_reg_write(0x33, val_Protect);     		
     		}
     	else if (gcharger->iac <= 1700)
     			{
     				axp_reg_read(0x33, &val_Protect);
     				//printk("2old_val_Protect = %d\n",val_Protect);
     				val_Protect |= 0x80;
     				//printk("2new_val_Protect = %d\n",val_Protect);     		
     				axp_reg_write(0x33, val_Protect);     		
     			}
...
static int g39_led_control(int flag)
{
	switch(flag){
...
static struct axp_supply_init_data axp_sply_init_data = {
	 ...
	 */
    .led_control     =  g39_led_control,
    .soft_limit_to99 =  0,
    .para            =  NULL,                       // para will pass to g39emmc10_pmu_call_back
    .pmu_call_back   =  g39emmc10_pmu_call_back,
    .board_battery   = &g39emmc10_battery,
};
...
//Shuttle. add touchscreen power sequence content.
void tp_power_sequence(void)
{
	//Shuttle. init gpio.
	gpio_set_status(PAD_GPIOA_16, gpio_status_out);//set int,rst pin to output status.
	gpio_set_status(PAD_GPIOC_3, gpio_status_out);
	gpio_out(PAD_GPIOA_16, 0);
	gpio_out(PAD_GPIOC_3, 0);
	mdelay(100);
        gpio_out(PAD_GPIOA_16, 0);//int pull low.
        mdelay(2);
        gpio_out(PAD_GPIOC_3, 1);//rst pull high.
        mdelay(6);
	gpio_set_status(PAD_GPIOA_16, gpio_status_in);//set to input status
}
...
static int gc0329_v4l2_uninit(void)
{
    ...
    printk( "amlogic camera driver:gc0329_uninit. \n");
    gpio_out(PAD_GPIOE_10, 1);    // set camera power disable
    msleep(5);
...
static void gc0329_v4l2_early_suspend(void)
{

}

static void gc0329_v4l2_late_resume(void)
{
...
static aml_plat_cam_data_t video_gc0329_data = {
    ...
    .early_suspend = gc0329_v4l2_early_suspend,
    .late_resume = gc0329_v4l2_late_resume,
...
static void gc2035_init(void)
{
    ...
    // reset low
    printk( "amlogic camera driver: gc2035_v4l2_init. \n");
    gpio_out(PAD_GPIOZ_0, 0);
...
static void gc2035_v4l2_init(void)
{
    gc2035_have_inited=1;
    gc2035_init();
}
static void gc2035_v4l2_uninit(void)
{
    gc2035_have_inited=0;
    gpio_out(PAD_GPIOE_11, 1);    // set camera power disable
    msleep(5);
    #if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0329)
    if(gc0329_have_inited==0)
    #endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}

struct aml_camera_i2c_fig_s custom_gc2035_script[] = {
    {0x0101,0x00},  //02  01(l-r)
    {0xffff,0xff}, 
};
...
static aml_plat_cam_data_t video_gc2035_data = {
	...
	.custom_init_script = custom_gc2035_script,
};
#endif
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0329
	{
 		I2C_BOARD_INFO("gc0329_i2c",  0x62 >> 1),
		.platform_data = (void *)&video_gc0329_data,
	},
#endif
...
#endif
//Shuttle. add touchscreen info to i2c board. 
#ifdef CONFIG_GOODIX_GT928_CAPACITIVE_TOUCHSCREEN
    {
        I2C_BOARD_INFO("Goodix-gt928", 0x5d),
    },
#endif
...
#ifdef CONFIG_SENSORS_KXTJ9
static struct kionix_accel_platform_data kxtj9_data = {
    .min_interval = 5,
    .poll_interval = 200,
    .accel_direction =1,
...
#define CONFIG_ENV_SIZE         (0x8000*2)
...
{
#define HS_PLUG     2    //headset
#define HP_PLUG     1	 //headphone
#define HP_UNPLUG   0
...
*/
    ...
    if (level == 0) {
        /* ADC chan2 headset dectect */
       ret = get_adc_sample(2);
       if (ret < 100) {
          val = HS_PLUG;
       } else {
          val = HP_PLUG;
...
static __init void meson_init_machine(void)
{
    ...
    m6g39emmc10_lcd_init();
#endif
#ifdef CONFIG_GOODIX_GT928_CAPACITIVE_TOUCHSCREEN
	tp_power_sequence(); //Shuttle. call tp power sequence function.
#endif
...
MACHINE_START(MESON6_G39EMMC10, "Amlogic Meson6 g39emmc10 customer platform")
