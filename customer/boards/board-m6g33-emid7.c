...
#define EMID7_USE_X11_FOR_WIFI_WAKEUP 1
...
static void m6ref_hdmi_3v3_ctrl(unsigned int pwr)
{
    if(pwr){
		axp_gpio_set_io(2,0);
        printk("HDMI 3V3 Power On--Refer to board\n");
    }
    else{
		axp_gpio_set_io(2,1);
		axp_gpio_set_value(2,0);
        printk("HDMI 3V3 Power Off--Refer to board\n");
    }
}
...
static struct hdmi_config_platform_data aml_hdmi_pdata = {
    ...
    .hdmi_3v3_ctrl = m6ref_hdmi_3v3_ctrl,
...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 80,     ////150,
    .pmu_battery_cap = 3000,    ////8000,
    ...
    .pmu_usbcur = 500,
#else
    .pmu_init_chgcur = 0,//500000,		//set initial charging current limite
...
static struct regulator_init_data axp_regl_init_data[] = {
    ...
    [vcc_buck2] = {
        .constraints = { /* default 1.24V */
            ...
            .initial_state = PM_SUSPEND_STANDBY,
            .state_standby = {
                .uV = 1550 * 1000,  //axp_cfg.dcdc2_vol * 1000,
                .enabled = 1,
            }
        },
...
u8 usb_otg_flag_emdoor=-1;
EXPORT_SYMBOL(usb_otg_flag_emdoor);

int usb_otg_connected=-1;
EXPORT_SYMBOL(usb_otg_connected);
...
/*
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
	0x01001b, 0x01001b, 0x030019, 0x040018, 
	0x060016, 0x090013, 0x0b0011, 0x0c0010, 
	0x0f000d, 0x11000b, 0x130009, 0x140008, 
	0x160006, 0x180004, 0x1a0002, 0x1b0001, 
};
*/
...
///*
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
	0x030019, 0x040018, 0x050017, 0x060016,
	0x070015, 0x080014, 0x090013, 0x0a0012, 
	0x0b0011, 0x0c0010, 0x0d000f, 0x0e000e, 
	0x0f000d, 0x10000c, 0x11000b, 0x12000a, 
};
//*/
/*
static unsigned int vcck_pwm_table[MESON_CS_MAX_STEPS] = {
        0x040018, 0x040018, 0x040018, 0x040018,
        0x040018, 0x040018, 0x040018, 0x040018,
        0x040018, 0x040018, 0x040018, 0x040018,
        0x040018, 0x040018, 0x040018, 0x040018,
};
...
//    {KEY_VOLUMEDOWN,    "vol-", CHAN_4, 160, 25},
    {KEY_VOLUMEUP,          "vol+", CHAN_4, 99, 25},
...
static pinmux_item_t camera_pins[] = {
	{
		.reg = PINMUX_REG(9),
		.setmask = 1 << 12
	},
...
static pinmux_set_t camera_pinmux_set = {
	.chip_select = NULL,
	.pinmux = &camera_pins[0]
};
...
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0309
static int gc0309_have_inited = 0;
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2659
static int ov2659_have_inited = 0;
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV3660
static int ov3660_have_inited = 0;
#endif
...
static int hi253_have_inited = 0;
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_SP2518
static int sp2518_have_inited = 0;
#endif
...
{
    pinmux_set(&camera_pinmux_set);
	#if 1
	unsigned int clk = clk_util_clk_msr(12) / 1000000; //vid2_pll_clk
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, ((clk / 24) - 1), 0, 7); //set div
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 5, 9, 4); //select vid2_pll_clk as camera clock
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 1); //enable clk
	#else
    ...
    gpio_out(CAM_RST, 0);    // reset IO low
    msleep(20);

    gpio_out(CAM_BACK_PWDN, 1);    // set camera pwdn high, disable
    msleep(20);

    gpio_out(CAM_PWR, 1);    //power on: 2.8V, 1.8V
	msleep(20);
	
    gpio_out(CAM_BACK_PWDN, 0);    // set camera pwdn low, enable
    msleep(20);

    gpio_out(CAM_RST, 1);    // reset IO high
    msleep(20);
...
static int gc2035_v4l2_uninit(void)
{
    ...
    gpio_out(CAM_BACK_PWDN, 1);    // set camera power disable
    msleep(5);
	gpio_out(CAM_RST, 0);    // reset IO low
    msleep(20);
	//gpio_out(CAM_PWR, 0);    //power down: 2.8V, 1.8V
	msleep(20);
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0308)
    if(gc0308_have_inited==0)
#endif
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0309)
    if(gc0309_have_inited==0)
#endif
...
{
	#if 0//Remove by tomy.huang, avoid camera window green
    pinmux_set(&camera_pinmux_set);
...
static aml_plat_cam_data_t video_gc2035_data = {
    .name="video-gc2035",
	//video_nr =1 front camera/video_nr=0 back camera		
    .video_nr=0,
...
static int hi704_have_inited = 0;
...
static void hi704_v4l2_init(void)
{
	...
	// Set reset to high
    gpio_out(CAM_RST, 1); 
	// Set Front Camera 1.8v 2.8v
    gpio_out(CAM_PWR, 1); 
    msleep(10);
    pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	//disable back hi253 Camera pwnd high GPIOE_10#1
	msleep(20);
	gpio_out(CAM_BACK_PWDN, 1); 	
    msleep(10);
    // set camera power pwd high
    gpio_out(CAM_FRONT_PWDN, 1);    
    msleep(10);
	// set camera power pwd low
    gpio_out(CAM_FRONT_PWDN, 0);    // set camera power enable
    msleep(10);
	
    printk("hi704_init OK!!!!\n");
    return 0;
}
static void hi704_v4l2_uninit(void)
{
    ...
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera pwnd disable
    msleep(20);
    
    gpio_out(CAM_PWR, 1);    // Set Front Camera 1.8v 2.8v
    msleep(5);
    
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_HI253)
    if (hi253_have_inited == 0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
static void hi704_v4l2_early_suspend(void)
{
	gpio_out(PAD_GPIOE_11, 1);    // set camera pwnd disable
    msleep(20);
    
    gpio_out(PAD_GPIOA_21, 1);    // Set Front Camera 1.8v 2.8v
}

static void hi704_v4l2_late_resume(void)
{
	// Set Front Camera 1.8v 2.8v
    gpio_out(PAD_GPIOA_21, 1); 
    msleep(10);
    pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    msleep(10);
	//disable Camera pwnd high GPIOE_10#1
...
aml_plat_cam_data_t video_hi704_data = {
	.name="video-hi704",
	#if defined(KERNEL_USE_FRONT_CAMERA)
	.video_nr=0,//1,
	#else
	.video_nr=1,//1,
	#endif
	...
	//modify by emdoor jim.kuang 2012-05-08 for camera green interface
...
static void hi253_v4l2_init(void)
{
	hi253_have_inited=1;
	// Set Back Camera 1.8v 2.8v
    gpio_out(CAM_PWR, 1); 
    msleep(10);	
    ...
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    msleep(20);
	//disable Front Camera pwnd high GPIOE_11#1
	gpio_out(CAM_FRONT_PWDN, 1); 	
    // set camera power high
    gpio_out(CAM_BACK_PWDN, 1);
    msleep(10);
    // set camera power low
    gpio_out(CAM_BACK_PWDN, 0);
	msleep(10);
	// set camera rst High
    gpio_out(CAM_RST, 1);
    msleep(10);
}
static void hi253_v4l2_uninit(void)
{
    hi253_have_inited=0;
    gpio_out(CAM_BACK_PWDN, 1);    // set camera power disable
    msleep(20);
    ...
    gpio_out(CAM_PWR, 1); 
    msleep(5);
    #if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_hi704)
    if(hi704_have_inited==0)
    #endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock

}
static void hi253_v4l2_early_suspend(void)
{
	gpio_out(PAD_GPIOE_10, 0);    // set camera power disable
    msleep(20);
    
    // Set Front Camera 1.8v 2.8v
    gpio_out(PAD_GPIOA_21, 0); 
}

static void hi253_v4l2_late_resume(void)
{
    pinmux_set(&camera_pinmux_set);
...
aml_plat_cam_data_t video_hi253_data = {
	.name="video-hi253",
	//video_nr =1 front camera/video_nr=0 back camera
	.video_nr=0,//1,
	.device_init= hi253_v4l2_init,
	.device_uninit=hi253_v4l2_uninit,
	//modify by emdoor jim.kuang 2012-05-08 for camera green interface
	//.early_suspend = hi253_v4l2_early_suspend,
	//.late_resume = hi253_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2659
static void ov2659_v4l2_init(void)
{
	ov2659_have_inited=1;
	printk("<------------------%s\n", __func__);
	//reset low
	gpio_out(CAM_RST, 0);
	// Set Back Camera 1.8v 2.8v
...
static void ov2659_v4l2_uninit(void)
{
	ov2659_have_inited=0;
	printk("<------------------%s\n", __func__);
    gpio_out(CAM_BACK_PWDN, 1);    // set camera power disable
    msleep(1);
...
static void ov2659_v4l2_early_suspend(void)
{
...
static void ov2659_v4l2_late_resume(void)
{
	pinmux_set(&camera_pinmux_set);
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	msleep(2);
...
aml_plat_cam_data_t video_ov2659_data = {
	.name="video-ov2659",
	//video_nr =1 front camera/video_nr=0 back camera
	.video_nr=0,//1,
	.device_init= ov2659_v4l2_init,
	.device_uninit= ov2659_v4l2_uninit,
//	.early_suspend = ov2659_v4l2_early_suspend,
//	.late_resume = ov2659_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV3660
static void ov3660_v4l2_init(void)
{
	ov3660_have_inited=1;
	printk("<------------------%s\n", __func__);
	//reset low
	gpio_out(CAM_RST, 0);
	// Set Back Camera 1.8v 2.8v
...
static void ov3660_v4l2_uninit(void)
{
	ov3660_have_inited=0;
	printk("<------------------%s\n", __func__);
    gpio_out(CAM_BACK_PWDN, 1);    // set camera  disable
    msleep(1);
...
static void ov3660_v4l2_early_suspend(void)
{
   gpio_out(PAD_GPIOE_11, 0);    // set camera power disable
    msleep(1);
...
static void ov3660_v4l2_late_resume(void)
{
	pinmux_set(&camera_pinmux_set);
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	msleep(2);
...
aml_plat_cam_data_t video_ov3660_data = {
	.name="video-ov3660",
	//video_nr =1 front camera/video_nr=0 back camera
	.video_nr=0,//1,
	.device_init= ov3660_v4l2_init,
	.device_uninit= ov3660_v4l2_uninit,
//	.early_suspend = ov2659_v4l2_early_suspend,
//	.late_resume = ov2659_v4l2_late_resume,
};
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV7675
static void ov7675_v4l2_init(void)
{
   ov7675_have_inited=1;
   printk("<------------------%s\n", __func__);
   //reset low
   gpio_out(CAM_RST, 0);
	// Set Front Camera 1.8v 2.8v power
    gpio_out(CAM_PWR, 1); 
    msleep(1);
    pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
...
static void ov7675_v4l2_uninit(void)
{
...
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2659)
    if (ov2659_have_inited == 0)
#endif
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_OV3660)
			if (ov3660_have_inited == 0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
...
{
    gpio_out(PAD_GPIOE_10, 1);    // set camera pwnd disable
    msleep(20);
...
static int gc0308_v4l2_init(void)
{
	...
	msleep(20);
	// set camera power enable
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera power enable
	msleep(20);
	
    gpio_out(CAM_RST, 0);    // reset IO
	msleep(20);
	
    gpio_out(CAM_RST, 1);    // reset IO
	msleep(20);
...
static int gc0308_v4l2_uninit(void)
{
    ...
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera power disable
    msleep(5);
	// close Camera 1.8v 2.8v
    //gpio_out(CAM_PWR, 0); 
    msleep(10);		
...
static int gc0308_v4l2_probe(struct i2c_adapter *adapter)
{
	int ret = 0;
	unsigned char reg;
	gc0308_v4l2_init();	
...
static aml_plat_cam_data_t video_gc0308_data = {
	...
	.device_probe = gc0308_v4l2_probe,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC0309
static int gc0309_v4l2_init(void)
{
    gpio_out(CAM_PWR, 1); 
    msleep(10);	
	pinmux_set(&camera_pinmux_set);
    ...
    gpio_out(CAM_FRONT_PWDN, 0);    // set camera power enable
	msleep(20);
	gc0309_have_inited=1;
	printk("gc0309_init OK!!!!\n");
	return 0;
}

static int gc0309_v4l2_uninit(void)
{
    gc0309_have_inited=0;
	printk( "amlogic camera driver: gc0308_v4l2_uninit. \n");
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera power disable
    msleep(5);
	// close Camera 1.8v 2.8v
    ...
    if (gt2005_have_inited == 0)
#endif
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_GC2035)
    if (gc2035_have_inited == 0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
static void gc0309_v4l2_early_suspend(void)
{

}
static int gc0309_v4l2_probe(struct i2c_adapter *adapter)
{
	int ret = 0;
	unsigned char reg;
	gc0309_v4l2_init();	
...
static struct aml_camera_i2c_fig1_s gc0309_custom_init_script[] = {
	{0x14,0x11},  //0x10  11
	{0xff,0xff},
};

static aml_plat_cam_data_t video_gc0309_data = {
	.name="video-gc0309",
	//video_nr =1 front camera/video_nr=0 back camera		
    .video_nr=1,//1,
	.device_init= gc0309_v4l2_init,
	.device_uninit=gc0309_v4l2_uninit,
	.custom_init_script = gc0309_custom_init_script,
	.device_probe = gc0309_v4l2_probe,
};
#endif
...
static int gt2005_v4l2_probe(struct i2c_adapter *adapter)
{
	int ret = 0;
	unsigned char reg[2];
	gt2005_v4l2_init();	
...
static aml_plat_cam_data_t video_gt2005_data = {
	...
	.device_probe = gt2005_v4l2_probe,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_SP0A19
static int sp0a19_have_inited = 0;
static void sp0a19_v4l2_init(void)
{
#if defined(EM62V3)
	gpio_out(CAM_RST, 0);
	 msleep(20);
    ...
    msleep(10);
#endif	
    printk("sp0a19_init OK!!!!\n");
    return 0;
}
static void sp0a19_v4l2_uninit(void)
{
    sp0a19_have_inited=0;
    printk( "amlogic camera driver: sp0a19_v4l2_uninit. \n");
#if defined(EM62V3)
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera pwnd disable
    msleep(50);
#else
    ...
    gpio_out(CAM_PWR, 0);    // Set Front Camera 1.8v 2.8v
    msleep(5);
#endif    
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_SP2518)
    if (sp2518_have_inited == 0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}
static void sp0a19_v4l2_early_suspend(void)
{
	gpio_out(PAD_GPIOE_11, 1);    // set camera pwnd disable
    msleep(20);
    
    gpio_out(PAD_GPIOA_25, 1);    // Set Front Camera 1.8v 2.8v
}

static void sp0a19_v4l2_late_resume(void)
{
	// Set Front Camera 1.8v 2.8v
    gpio_out(PAD_GPIOA_25, 1); 
    msleep(10);
    pinmux_set(&camera_pinmux_set);
    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
    msleep(10);
...
aml_plat_cam_data_t video_sp0a19_data = {
	.name="video-sp0a19",
#if defined(KERNEL_USE_FRONT_CAMERA)
	.video_nr=0,//1,
#else
	.video_nr=1,//1,
#endif
	.device_init= sp0a19_v4l2_init,
	.device_uninit=sp0a19_v4l2_uninit,
	//modify by emdoor jim.kuang 2012-05-08 for camera green interface
	//.early_suspend = sp0a19_v4l2_early_suspend,
	//.late_resume = sp0a19_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_SP2518
static void sp2518_v4l2_init(void)
{
	sp2518_have_inited=1;
	// Set Back Camera 1.8v 2.8v
    gpio_out(CAM_PWR, 1); 
    msleep(10);	
...
//    msleep(50);
	// set camera power pwd low
    gpio_out(CAM_BACK_PWDN, 0);    // set camera power enable
    msleep(100);
...
static void sp2518_v4l2_uninit(void)
{
    sp2518_have_inited=0;
    gpio_out(CAM_BACK_PWDN, 1);    // set camera power disable
    msleep(20);
    ...
    gpio_out(CAM_PWR, 0); 
    msleep(5);
    #if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_SP0A19)
    if(sp0a19_have_inited==0)
    #endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock

}
static void sp2518_v4l2_early_suspend(void)
{
	gpio_out(PAD_GPIOE_10, 0);    // set camera power disable
    msleep(20);
...
static void sp2518_v4l2_late_resume(void)
{
    pinmux_set(&camera_pinmux_set);
...
aml_plat_cam_data_t video_sp2518_data = {
	.name="video-sp2518",
	//video_nr =1 front camera/video_nr=0 back camera
	.video_nr=0,//1,
	.device_init= sp2518_v4l2_init,
	.device_uninit=sp2518_v4l2_uninit,
	//modify by emdoor jim.kuang 2012-05-08 for camera green interface
	//.early_suspend = sp2518_v4l2_early_suspend,
	//.late_resume = sp2518_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99250
static int nt99250_have_inited = 0;
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99252
static int nt99252_have_inited = 0;
#endif

#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99340)
static int nt99340_have_inited = 0;
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99250
static void nt99250_v4l2_init(void)
{
   nt99250_have_inited=1;
   printk("<------------------%s\n", __func__); 
   //reset low
   gpio_out(CAM_RST, 0);
   msleep(10);   
...
static void nt99250_v4l2_uninit(void)
{
	nt99250_have_inited=0;
	printk("<------------------%s\n", __func__);
    gpio_out(CAM_FRONT_PWDN, 1);    // set camera pwnd disable
    msleep(10);
...
static void nt99250_v4l2_early_suspend(void)
{
    gpio_out(PAD_GPIOE_10, 1);    // set camera pwnd disable
    msleep(20);
...
static void nt99250_v4l2_late_resume(void)
{
    // Set Front Camera 1.8v 2.8v
    gpio_out(PAD_GPIOA_21, 1); 
    msleep(1);
...
aml_plat_cam_data_t video_nt99250_data = {
	.name="video-nt99250",
	.video_nr=1,
	.device_init= nt99250_v4l2_init,
	.device_uninit=nt99250_v4l2_uninit,
//	.early_suspend = nt99250_v4l2_early_suspend,
//	.late_resume = nt99250_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99252
static void nt99252_v4l2_init(void)
{
   nt99252_have_inited=1;
   printk("<------------------%s\n", __func__); 
   //reset low
   gpio_out(CAM_RST, 0);
   msleep(10);   
...
static void nt99252_v4l2_uninit(void)
{
	nt99252_have_inited=0;
	printk("<------------------%s\n", __func__);
    gpio_out(CAM_BACK_PWDN, 1);    // set camera pwnd disable
    msleep(10);
...
static void nt99252_v4l2_early_suspend(void)
{
    gpio_out(PAD_GPIOE_10, 1);    // set camera pwnd disable
    msleep(20);
...
static void nt99252_v4l2_late_resume(void)
{
    // Set Front Camera 1.8v 2.8v
    gpio_out(PAD_GPIOA_21, 1); 
    msleep(1);
...
aml_plat_cam_data_t video_nt99252_data = {
	.name="video-nt99252",
	.video_nr=0,
	.device_init= nt99252_v4l2_init,
	.device_uninit=nt99252_v4l2_uninit,
//	.early_suspend = nt99252_v4l2_early_suspend,
//	.late_resume = nt99252_v4l2_late_resume,
};
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99340
static void nt99340_v4l2_init(void)
{
	printk("<------------------%s\n", __func__);	
	nt99340_have_inited=1;
	//reset high
	gpio_out(CAM_RST, 0);
	msleep(20);    // set camera power low
	// Set Back Camera 1.8v 2.8v
    gpio_out(CAM_PWR, 1); 
    msleep(10);			
    pinmux_set(&camera_pinmux_set);
...
static void nt99340_v4l2_uninit(void)
{
	nt99340_have_inited=0;
	printk("<------------------%s\n", __func__);
    gpio_out(CAM_BACK_PWDN, 1);    // set camera power disable
    msleep(1);
...
static void nt99340_v4l2_early_suspend(void)
{
   gpio_out(PAD_GPIOE_11, 0);    // set camera power disable
    msleep(1);
...
static void nt99340_v4l2_late_resume(void)
{
	pinmux_set(&camera_pinmux_set);
	aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock
	msleep(2);
...
aml_plat_cam_data_t video_nt99340_data = {
	.name="video-nt99340",
	//video_nr =1 front camera/video_nr=0 back camera
	.video_nr= 0,
	.device_init= nt99340_v4l2_init,
	.device_uninit= nt99340_v4l2_uninit,
//	.early_suspend = nt99340_v4l2_early_suspend,
//	.late_resume = nt99340_v4l2_late_resume,
};
#endif
...
struct ts_platform_data ft5x06_ts_pdata = {
    .irq = FT_IRQ,
    .init_irq = ts_init_irq,
    .get_irq_level = NULL,
    .power = ts_power,
    .Ac_is_connect= IS_AC_connected,
    .screen_max_x=KERNEL_SCREEN_MAX_X, 
    .screen_max_y=KERNEL_SCREEN_MAX_Y, 
#if defined(ft5x06_swap_xy)
	.swap_xy = 1,
#else
    .swap_xy = 0,
#endif    
...
#ifdef CONFIG_MULTI_TP_SUPPORT	
EXPORT_SYMBOL(ft5x06_ts_pdata);
#endif
...
#endif
#ifdef CONFIG_ZT622X_TOUCHSCREEN

#include <linux/zt622x_i2c_ts.h>


#define GPIO_FT_PWR  PAD_GPIOA_23
#define GPIO_FT_RST  PAD_GPIOC_3
#define GPIO_FT_IRQ  PAD_GPIOA_16
#define FT_IRQ	INT_GPIO_0

static int zt622x_ts_init_irq(void)
{
	printk("+++++++++++++++++++%s\n", __func__);
    gpio_set_status(GPIO_FT_IRQ, gpio_status_in);
    gpio_irq_set(170, GPIO_IRQ(FT_IRQ-INT_GPIO_0, GPIO_IRQ_FALLING));
...
void zt622x_reset(void)
{
	printk("+++++++++++++++++++%s\n", __func__);
	gpio_set_status(GPIO_FT_RST, gpio_status_out);
    gpio_out(GPIO_FT_RST, 0);
	msleep(50);
    gpio_out(GPIO_FT_RST, 1);
}

struct zt622x_i2c_ts_platform_data	zt622x_ts_pdata = {
	.xmax = 800,
	.ymax = 480,
	.gpio_irq = GPIO_FT_IRQ,
	.gpio_rst = GPIO_FT_RST,
	.key_num = 0,
	.point = 5,
	.init_irq = zt622x_ts_init_irq,
	.reset = zt622x_reset,	
};

#ifdef CONFIG_MULTI_TP_SUPPORT	//modify by emdoor jim.kuang 2012-07-20
EXPORT_SYMBOL(zt622x_ts_pdata);
#endif
...
static struct i2c_board_info __initdata aml_i2c_bus_info_a[] = {
    ...
    {
        /*gc0308 i2c address is 0x42/0x43*/
        I2C_BOARD_INFO("gc0309_i2c",  0x42 >> 1),
        .platform_data = (void *)&video_gc0309_data,
    },
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GT2005
    {
...
#ifndef CONFIG_MULTI_TP_SUPPORT
...
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN
    {
        ...
        .irq = INT_GPIO_0,
        .platform_data = (void *)&ft5x06_ts_pdata,
    },
#endif
#ifdef CONFIG_ZT622X_TOUCHSCREEN
	{
        I2C_BOARD_INFO(ZET_TS_ID_NAME, 0x76),
        .irq = INT_GPIO_0,
        .platform_data = (void *)&zt622x_ts_pdata,
    },
#endif
...
#ifdef CONFIG_GOODIX_GT811_CAPACITIVE_TOUCHSCREEN
    {
        I2C_BOARD_INFO("Goodix-TS", 0x5d),
    },
#endif
...
#endif
#ifdef CONFIG_GSL1680_CAPACITIVE_TOUCHSCREEN_86V 
    {
...
static struct i2c_board_info  __initdata aml_dynamical_i2c_bus_info_a[] = {
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_GC2035
	{
		...
		{
		//in fact, sp0a19 addr is 0x21,here is 0x22,but it will be changed to 0x21	in probe
			I2C_BOARD_INFO("sp0a19_i2c",  0x44 >> 1),
		...
		{
			/*sp2518 i2c address is 0x30*/
			//in fact, sp2518 addr is 0x30,here is 0x31,but it will be changed to 0x30  in probe
			I2C_BOARD_INFO("sp2518_i2c",  0x62 >> 1),
			.platform_data = (void *)&video_sp2518_data,
		},
#endif
	...
	{
        /*Hi704 i2c address is 0x60*/
		I2C_BOARD_INFO("HI704_i2c",  0x60 >> 1),
		.platform_data = (void *)&video_hi704_data,
	},
#endif
//modify by emdoor jim.kuang 2012-02-28
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_HI253
	{
        /*hi253 i2c address is 0x40*/
		I2C_BOARD_INFO("hi253_i2c",  0x40 >> 1),
		.platform_data = (void *)&video_hi253_data,
	},
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV2659
	{
        /*ov2659 i2c address is 0x30*/
		//in fact, ov2659 addr is 0x30,here is 0x32,but it will be changed to 0x30  in probe		
		I2C_BOARD_INFO("ov2659_i2c",  0x64 >> 1),
		.platform_data = (void *)&video_ov2659_data,
	},
#endif

#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_OV3660
	{
        /*ov3660 i2c address is 0x3c*/
		I2C_BOARD_INFO("ov3660_i2c",  0x78 >> 1),
		.platform_data = (void *)&video_ov3660_data,
	},
#endif
	...
	{
        /*ov7675i2c address is 0x21*/
		//in fact, ov7675 addr is 0x21,here is 0x23,but it will be changed to 0x21  in probe
		I2C_BOARD_INFO("ov7675_i2c",  0x46 >> 1),
		...
		{
			/*nt99250 i2c address is 0x36*/
			I2C_BOARD_INFO("nt99250_i2c",  0x6c >> 1),
			.platform_data = (void *)&video_nt99250_data,
		},
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99252
	
		{
			/*nt99252 i2c address is 0x36*/
			I2C_BOARD_INFO("nt99252_i2c",  0x6c >> 1),
			.platform_data = (void *)&video_nt99252_data,
		},
#endif
#ifdef CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99340
		{
			/*nt99340 i2c address is 0x3b*/
			I2C_BOARD_INFO("nt99340_i2c",  0x76 >> 1),
			.platform_data = (void *)&video_nt99340_data,
		},
#endif
};
#if 0
int aml_camera_dynamical_probe(struct i2c_adapter *adapter)
{
	...
	aml_plat_cam_data_t* plat_cam_data;
	for (i = 0; i < count; i++) {
		board_info = &aml_dynamical_i2c_bus_info_a[i];
		plat_cam_data = (aml_plat_cam_data_t*)(board_info->platform_data);
		if (!plat_cam_data->device_probe) {
			printk("error jdkjaslfdjla\n");
			continue;
		}
		if (plat_cam_data->device_probe(adapter))
...
static int __init aml_camera_dynamical_init(void)
{
	...
	if (adapter) {
		printk("aml_camera_dynamical_init\n");
		aml_camera_dynamical_probe(adapter);
	}
	return 0;
}
...
#endif
#ifdef CONFIG_SENSORS_MMA7660 || defined(CONFIG_SENSORS_MC32X0)
	{
...
static struct mtd_partition normal_partition_info[] = {
    ...
    {
		.name = "factory",	
		.offset = 728*SZ_1M+40*SZ_1M,
		.size = 128*SZ_1M,
	},
	{
		.name = "cache",
		.offset = 856*SZ_1M+40*SZ_1M,
		.size = 600*SZ_1M,
	},
	{
		.name = "userdata",
...
/* BT ON Flag */
static int BT_ON=0;
//Add  by emdoor jim.kuang 2012-06-06 for sdio wifi power on
//GPIOAO_11 control sdio wifi
void sdio_wifi_power_control(int on)
{
			return ;
}
EXPORT_SYMBOL(sdio_wifi_power_control);
...
{
	DBG_LINE_INFO();
		//modify by emdoor jim.kuang 2012-05-24
	    gpio_set_status(PAD_GPIOAO_3,gpio_status_out);//WIFI_PWREN GPIOAO#3	1 enable 0 disable
	    if(is_on){
			//Add by emdoor jim.kuang 2012-06-06 for SDIO wifi power on
			WIFI_ON=1;		
			gpio_out(PAD_GPIOAO_3,1);
...
static void sdio_extern_init(void)
{
...
#if defined(EMID7_USE_X11_FOR_WIFI_WAKEUP)
	//Modify by emdoor jim.kuang for CryStal 2013-05-08
    gpio_out(PAD_GPIOA_25, 1); 
	//Modify by emdoor jim.kuang 2013-05-08
	gpio_set_status(PAD_GPIOX_11,gpio_status_in);
    ...
    gpio_irq_set(PAD_GPIOX_11,GPIO_IRQ(4,GPIO_IRQ_RISING));
   	...
   	#ifdef CONFIG_BCM40183_WIFI //if (extern_check_wifi_type()) { //GB9663
		gpio_set_status(PAD_GPIOX_11,gpio_status_in); 
...
//u8 usb_otg_flag_emdoor=-1;
//EXPORT_SYMBOL(usb_otg_flag_emdoor);
static int usb_charging_timer_counts = 0;
extern int axp_set_charge_current(int chgcur);
extern int axp_set_charging_cur_500ma_with_sys_interface;

static void usb_battery_charger_detect_cb(int bc_mode) 
{ 
  printk(KERN_INFO "####journ---null  usb_battery_charger_detect_cb\n",__func__,bc_mode); 
}


#if 1////defined(USB_CHARGING_ONLY)
#include <linux/hrtimer.h>
static struct hrtimer usb_charging_timer;
static enum hrtimer_restart charging_cur_setting_timer_func(struct hrtimer *timer)
{
	printk("++++++++++ charging_cur_setting_timer_func,reurn now\n");
	usb_charging_timer_counts = 1;
	return HRTIMER_NORESTART;
	
	//axp_clr_bits(gcharger->master, 0x33, BIT(0)|BIT(2)|BIT(3));
	//axp_set_bits(gcharger->master, 0x33, BIT(7)|BIT(1));//500ma
		
	//axp_set_bits(gcharger->master, 0x30, BIT(0)|BIT(1)|BIT(7));//vbus enable,and not limit cur

	//axp_write(gcharger->master,0x30,0xe3);//vbus enable,and not limit cur
	//mdelay(10);
	//axp_write(gcharger->master,0x33,0xc2);//charging cur to 500ma
	//mdelay(10);

	axp_set_charge_current(500000);//500ma,2013-07-19
...
#if defined(CONFIG_AW_AXP20)    /* register charger detect call back  */ 
...
static void usb_wifi_power(int is_power)
{
	...
	if (is_power)
        gpio_out(PAD_GPIOX_11, 0); 
	else
		gpio_out(PAD_GPIOX_11, 1); 
    return 0;
}
...
#else
	// 8723au add by soxiebing	2013-0221 解决tenda 路由器无法上网 和 wifi 信号差的问题
	/* PCM_CLK */
	gpio_set_status(PAD_GPIOX_7,gpio_status_out);
	gpio_out(PAD_GPIOX_7,0);
	/* PCM_FS */
	gpio_set_status(PAD_GPIOX_6,gpio_status_out);
	gpio_out(PAD_GPIOX_6,0);
	/* PCM_OUT */
	gpio_set_status(PAD_GPIOX_4,gpio_status_out);
	gpio_out(PAD_GPIOX_4,0);
	/* PCM_IN */
	gpio_set_status(PAD_GPIOX_5,gpio_status_out);
	gpio_out(PAD_GPIOX_5,0);
...
static int rt5631_pure_hp_detect(void)
{
    int val = RT5631_HP_UNPLUG;
    uint32_t level = 0;
...
static int rt5631_hp_detect(void)
{
    if(get_display_mode() != 0) {   //if !panel, return HP_PLUG
        return RT5631_HP_PLUG;//modify by emdoor jim.kuang 2012-05-04 
    }

    return rt5631_pure_hp_detect();
}
...
/* added by dongming.li */
extern  struct proc_dir_entry *proc_emdoor;
EXPORT_SYMBOL_GPL(proc_emdoor);
struct proc_dir_entry *proc_emdoor = NULL;

static __init void meson_init_machine(void)
{
	// added by dongming.li to create /proc/emdoor
	struct proc_dir_entry *res;
	
	proc_emdoor = proc_mkdir("emdoor", 0);
	/* end added */
    //meson_cache_init();
    //mmc_lp_suspend_init();
    setup_usb_devices();
...
//    mutex_init(&wifi_bt_mutex);
  ...
  extern int m6g04_lcd_init(void);
  m6g04_lcd_init();
#endif


	gpio_out(CAM_FRONT_PWDN, 1);    // set camera power enable	
	gpio_out(CAM_BACK_PWDN,  1);    // set camera power enable	


#if 0//defined(USB_CHARGING_ONLY)
	hrtimer_init(&usb_charging_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	usb_charging_timer.function = charging_cur_setting_timer_func;
	hrtimer_start(&usb_charging_timer,ktime_set(25, 0), HRTIMER_MODE_REL);
#endif
