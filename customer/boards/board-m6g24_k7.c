...
static struct battery_parameter g24_battery = {
    ...
    .pmu_battery_rdc = 57,     ////150,
    .pmu_battery_cap = 11580,    ////8000,
    ...
    .pmu_usbcur = 500,
#else
    .pmu_init_chgcur = 1800000,		//set initial charging current limite
    .pmu_suspend_chgcur = 1800000,	//set suspend charging current limite
    .pmu_resume_chgcur = 1800000,							//set resume charging current limite
    .pmu_shutdown_chgcur = 1800000,		//set shutdown charging current limite
    .pmu_usbcur_limit = 0,
    .pmu_usbcur = 500,
#endif
    .pmu_usbvol_limit = 1,
...
static struct ts_platform_data_ssd253x ts_pdata_ssd253x = {
	.irq_no 			= FT_IRQ,
	.reset_gpio_no	= PAD_GPIOC_3,
	.irq_gpio_no		= PAD_GPIOA_16,
};
#endif
...
static u8 gt82x_config[] = {
    0x0F,0x80,/*config address*/
       0x00,0x0F,0x01,0x10,0x02,0x11,0x03,0x12,0x04,0x13,0x05,0x14,0x06,0x15,0x07,0x16,0x08,0x17,0x09,0x18,0x0A,0x19,0x0B,0x1A,0x0C,0x1B,0x0D,0x1C,0x0E,0x1D,0x13,0x09,0x12,0x08,0x11,0x07,0x10,0x06,0x0F,0x05,0x0E,0x04,0x0D,0x03,0x0C,0x02,0x0B,0x01,0x0A,0x00,0x1F,0x03,0x48,0x10,0x10,0x27,0x00,0x00,0x0A,0x00,0x00,0x0A,0x48,0x30,0x3D,0x03,0x00,0x05,0x00,0x03,0x20,0x05,0x00,0x49,0x49,0x44,0x44,0x28,0x00,0x25,0x19,0x02,0x14,0x10,0x02,0xDA,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x60,0x20,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01
};
...
#endif
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_GEMEI86V_NEW
...
#endif
#ifdef CONFIG_GOODIX_GT811_GEMEI86V_CAPACITIVE_TOUCHSCREEN
#include <linux/gt811_gemei86v.h>
#define GTP_RST_PORT	PAD_GPIOC_3
#define GTP_INT_PORT	PAD_GPIOA_16
#define GTP_INT_IRQ	    INT_GPIO_0

static int gtp_request_io_port(void)
{
...
static int gtp_reset_guitar(int on)
{
	gpio_set_status(GTP_RST_PORT, gpio_status_out);
	gpio_out(GTP_RST_PORT, on);
	//msleep(30);
	//gpio_out(GTP_RST_PORT,0);
	//msleep(20);
	//gpio_out(GTP_RST_PORT, 1);
	//msleep(20);
...
static struct goodix_i2c_rmi_platform_data goodix_ts_pdata = {
	.gpio_rst = GTP_RST_PORT,
	.gpio_irq = GTP_INT_PORT,
	.irq_edge = 1, /* 1:rising edge, 0:falling edge */
	.swap_xy = 1,
	.xpol = 0,
	.ypol = 0,
	.abs_x_max = 800,
	.abs_y_max = 480,
	.irq_init = gtp_request_io_port,
	.reset = gtp_reset_guitar,
...
#endif
#ifdef CONFIG_GSL1680_CAPACITIVE_TOUCHSCREEN_gemei_k10 
    {
