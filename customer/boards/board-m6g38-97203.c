...
static struct battery_parameter g38_97203_battery = {
    .pmu_twi_id = 2,		//AXP20_I2CBUS
    .pmu_irq_id = INT_WATCHDOG,
    .pmu_twi_addr = AXP20_ADDR,
    .pmu_battery_rdc = 107,     ////150,
    .pmu_battery_cap = 3830,    ////8000,
...
static int g38_97203_pmu_call_back(void *para)
{
...
static struct axp_supply_init_data axp_sply_init_data = {
  ...
  //.led_control     =  led_control,
    ...
    .pmu_call_back   =  g38_97203_pmu_call_back,
    .board_battery   = &g38_97203_battery,
};
...
static pinmux_item_t nt99252_pins[] = {
    {
        .reg = PINMUX_REG(9),
        .setmask = 1 << 12
    },
...
static pinmux_set_t nt99252_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &nt99252_pins[0]
};
static void nt99252_init(void)
{
    pinmux_set(&nt99252_pinmux_set);

    aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 1, 8, 5); //select XTAL as camera clock

    // reset low
    printk( "amlogic camera driver: nt99252_v4l2_init. \n");
    gpio_out(PAD_GPIOZ_0, 0);

    // set camera power disanable
    gpio_out(PAD_GPIOE_11, 1);//10
...
static void nt99252_v4l2_uninit(void)
{
    ...
    gpio_out(PAD_GPIOE_11, 1);    // set camera power disable,10
    msleep(5);
#if defined(CONFIG_VIDEO_AMLOGIC_CAPTURE_NT99340)
    //if(nt99340_have_inited==0)
#endif
        aml_set_reg32_bits(P_HHI_GEN_CLK_CNTL, 0, 8, 1); //close clock
}

static void nt99252_v4l2_disable(void)
{
...
static aml_plat_cam_data_t video_nt99252_data = {
    .name="video-nt99252",
    .video_nr=0,   //    1
    ...
    .device_disable=nt99252_v4l2_disable,
};
#endif
...
#endif /* CONFIG_VIDEO_AMLOGIC_CAPTURE_gt2005 */
...
#endif
#ifdef CONFIG_CT360_CAPACITIVE_TOUCHSCREEN
...
static struct ct360_platform_data ts_pdata = {
    .irq = FT_IRQ,
    .init_gpio = ts_init_irq,
    .shutdown = ts_power,
    .x_max=1024, 
    .y_max=768, 
    .swap_xy = 0,
...
static struct i2c_board_info __initdata aml_i2c_bus_info_a[] = {
	...
	{
		I2C_BOARD_INFO("nt99250_i2c", 0x6c >> 1),  //0x6c
...
#endif
#ifdef CONFIG_GSl3680_CAPACITIVE_TOUCHSCREEN_G38_T97203
    {
...
#ifdef CONFIG_CT360_CAPACITIVE_TOUCHSCREEN
    {
        I2C_BOARD_INFO("ct36x_ts", 0x39),//0x39
        .irq = INT_GPIO_0,
        .platform_data = (void *)&ts_pdata,
    },
#endif
...
static void sdio_extern_init(void)
{
    ...
    aml_write_reg32(0xfa004004,0x00000000); //SYS_RST  ->JGTA TMS
    aml_set_reg32_mask(P_PAD_PULL_UP_REG3,0x1<<24); //clk not pull up
     
    gpio_set_status(PAD_GPIOAO_9,gpio_status_out); //RST 
    gpio_set_status(PAD_GPIOX_11,gpio_status_in);

    gpio_out(PAD_GPIOAO_9,1);
    //mtk interrupt
...
static pinmux_item_t rt3261_pinmux[] = {
    /* I2S_MCLK I2S_BCLK I2S_LRCLK I2S_DIN I2S_DOUT */
    {
        .reg = PINMUX_REG(9),
        .setmask = (1 << 7) | (1 << 5) | (1 << 9) | (1 << 11) | (1 << 4),
...
static pinmux_set_t rt3261_pinmux_set = {
    .chip_select = NULL,
    .pinmux = &rt3261_pinmux[0],
};

static void rt3261_device_init(void)
{
    /* audio pinmux */
    pinmux_set(&rt3261_pinmux_set);

    /* GPIOA_19 PULL_UP_REG0 bit19 */
    aml_set_reg32_bits(P_PAD_PULL_UP_REG0, 1, 19, 1);
}

static void rt3261_device_deinit(void)
{
    pinmux_clr(&rt3261_pinmux_set);
}

static int rt3261_hp_detect(void)
{
#define HP_PLUG     1
#define HP_UNPLUG   0
...
static struct rt3261_platform_data rt3261_pdata = {
    .hp_detect      = rt3261_hp_detect,
    .device_init    = rt3261_device_init,
    .device_uninit  = rt3261_device_deinit,

    //.spk_watt       = RT3261_SPK_0_5W,
    .spk_output     = RT3261_SPK_STEREO,
    .mic_input      = RT3261_MIC_DIFFERENTIAL,
};

static struct platform_device aml_rt3261_audio = {
    .name           = "aml_rt3261_audio",
    .id             = 0,
    .resource       = aml_m6_audio_resource,
    .num_resources  = ARRAY_SIZE(aml_m6_audio_resource),
    .dev = {
        .platform_data = &rt3261_pdata,
    },
};
...
static int get_call_key_value(void)
{
    /* GPIOA_25 */
    int val =  gpio_in_get(PAD_GPIOA_25);
    //printk("get call key value %d\n", val);
    return val;
}

static int get_phone_ring_value(void)
{
	/* PAD_GPIOAO_6 */
    int val =  gpio_in_get(PAD_GPIOAO_6);
    //printk("get phone ring value %d\n", val);
    return val;
}
...
struct call_key_platform_data callkey_pdata = {
	.hp_in_value = 1,
	.hp_out_value = 0,
	.get_hp_status = rt3261_hp_detect,
	.get_call_key_value = get_call_key_value,
	.get_phone_ring_value = get_phone_ring_value,
};

static struct platform_device callkey_device = {
    .name = "callkey",
    .id = 0,
    .num_resources = 0,
    .resource = NULL,
    .dev = {
    	.platform_data = &callkey_pdata,
    }
};
...
static int modem_power_off(void)
{
	...
	axp_gpio_set_io(0,0);	//set AXP 202 GPIO0 low ,3G power off
	gpio_out(PAD_GPIOA_3,0);//GPIOA_3 = power_key
	mdelay(8);
	gpio_out(PAD_GPIOA_2,0); //GPIOA_2 = off_sleep
	mdelay(5);
...
static struct platform_device  *platform_devs[] = {
    ...
    &aml_wm8960_audio,
#endif
#if defined(CONFIG_AML_CALL_KEY)
    &callkey_device,
#endif
...
static __init void meson_init_machine(void)
{
    ...
    m6g38_97203_lcd_init();
#endif
	pm_power_off = power_off;
	
	//for 3G
	gpio_out(PAD_GPIOA_3,1);
}
...
MACHINE_START(MESON6_G38_9203, "Amlogic Meson6 g38_97203 customer platform")
