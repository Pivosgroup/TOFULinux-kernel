...
#define CAM_FRONT_PWDN		PAD_GPIOE_11
#define CAM_BACK_PWDN		PAD_GPIOE_10
#define    JTAG_ENBLE_BASE       0xfb000000	//4K_SIZE
//add by emdoor jim.kuang 2012-06-06
#define    rJTAG_ENABLE         (*(volatile unsigned long *)(JTAG_ENBLE_BASE+0x4))	
#define    rGPIOAO_OEN          (*(volatile unsigned long *)(IO_AOBUS_BASE+0x24))	
#define    rGPIOAO_OUT          (*(volatile unsigned long *)(IO_AOBUS_BASE+0x24))
...
static int init_irq(void)
{
    ...
    gpio_irq_set(170, GPIO_IRQ(GT82X_IRQ-INT_GPIO_0, GPIO_IRQ_RISING));
       return 0;
}
...
static u8 gt82x_config[] = {
		...
		0x08,0x12,0xFF,0xFF,0xFF,0x0E,0x0F,0x10,0x11,0x12,
		0x0F,0x03,0x48,0x10,0x10,0x27,0x00,0x00,0x08,0x00,
		0x00,0x02,0x50,0x30,0x14,0x03,0x00,0x05,0x00,0x02,
		0x58,0x04,0x00,0x49,0x49,0x44,0x44,0x27,0x00,0x25,
...
static pinmux_item_t uart_pins[] = {
    {
	   ...
	   //USE GPIOX_23 GPIOX_24 for UARTB_TX UARTB_RX
    {
        .reg = PINMUX_REG(4),
        .setmask = 0xf << 10
    },
...
static void gps_power_on(void)
{
    printk("power on gps\n");
    //GPS Power On
    gpio_out(PAD_GPIOA_14, 1);//GPS_PWER#1
    //GPS Reset
    gpio_out(PAD_GPIOC_9, 0);//GPS_RESET#0
    mdelay(500);
    gpio_out(PAD_GPIOC_9, 1);//GPS_RESET#1
}
static void gps_power_off(void)
{
    printk("power off gps\n");
    //GPS Power down
    gpio_out(PAD_GPIOA_14, 0);//GPS_PWER#0
    //GPS Reset down
    gpio_out(PAD_GPIOC_9, 0);//GPS_RESET#0
}
...
int hp_last_state = 0;
static int is_hp_connect_using_adc_channel(const int channel,const int standard_value,const int tolerance)
{
    int adc_value,retry_loops,ret,retry_loops2;

    retry_loops = 0;
retry:
    adc_value = get_adc_sample(channel);
    //printk("channel  = %d,adc = %d\n",channel,adc_value);
    if(adc_value < 0) { //perhaps,be busy now
        //printk("hp detect,adc_value < 0\n");
        ret = adc_value;
    }

    else if((adc_value >= (standard_value - tolerance)) && (adc_value <= (standard_value + tolerance)))
        ret = 1;
    else
        ret = 0;

    //once more check
    if(hp_last_state != ret) {
        if(retry_loops++ < 2)
            goto retry;
    }
