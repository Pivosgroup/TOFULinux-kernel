...
#define OSD_PANEL_PIX       (CONFIG_FB_OSD1_DEFAULT_WIDTH*CONFIG_FB_OSD1_DEFAULT_HEIGHT) 
#define B16BpP  (2)
#define B32BpP  (4)
...
#if defined(CONFIG_TVIN_VDIN_MINI_CONFIG)
#define VDIN_ADDR_START     CODEC_ADDR_START //U_ALIGN(CODEC_ADDR_END)
#define VDIN_ADDR_END       (CODEC_ADDR_START+(SZ_1M*16)-1) //(VDIN_ADDR_START + CODEC_MEM_SIZE - 1)
#else
...
#else
#define DI_MEM_SIZE         (SZ_1M*0) //we didn't enable DEINTERLACE, so we can set to 0
#endif
#define DI_ADDR_START       U_ALIGN(CODEC_ADDR_END)//for the VM use the memory alloced for VDEC
#define DI_ADDR_END         (DI_ADDR_START+DI_MEM_SIZE-1)

//32 bytes align
#ifdef CONFIG_POST_PROCESS_MANAGER
#ifdef CONFIG_POST_PROCESS_MANAGER_PPSCALER
#define PPMGR_MEM_SIZE               (CONFIG_FB_OSD1_DEFAULT_WIDTH * (((CONFIG_FB_OSD1_DEFAULT_HEIGHT+15)>>5)<<5) * 21)
#else
#define PPMGR_MEM_SIZE               (CONFIG_FB_OSD1_DEFAULT_WIDTH * (((CONFIG_FB_OSD1_DEFAULT_HEIGHT+15)>>5)<<5) * 18)
#endif
#else
