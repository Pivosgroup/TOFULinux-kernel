/*
 * arch/arm/mach-meson3/board-m6g11-t80-panel.c
 *
...
extern Lcd_Config_t m6g11_t80_lcd_config;
...
static void minilvds_ports_ctrl(Bool_t status)
{
	...
	{		
		...
		WRITE_MPEG_REG(0x202c, READ_MPEG_REG(0x202c) | (0x7e<<12));  //set tcon pinmux
	...
	{	
		...
		WRITE_MPEG_REG(0x202c, READ_MPEG_REG(0x202c) & ~(0x7e<<12));  //clear tcon pinmux
		WRITE_MPEG_REG(0x2012, READ_MPEG_REG(0x2012) | (0xfe<<2));  //set tcon as GPIO input
...
Lcd_Config_t m6g11_t80_lcd_config = {
    // Refer to LCD Spec
    .lcd_basic = {
        .h_active = H_ACTIVE,
        .v_active = V_ACTIVE,
...
static struct aml_bl_platform_data m6g11_t80_backlight_data =
{
...
static struct platform_device m6g11_t80_backlight_device = {
    .name = "aml-bl",
    .id = -1,
    .num_resources = 0,
    .resource = NULL,
    .dev = {
        .platform_data = &m6g11_t80_backlight_data,
    },
};

static struct aml_lcd_platform __initdata m6g11_t80_lcd_data = {
    .lcd_conf = &m6g11_t80_lcd_config,
};

static struct platform_device __initdata * m6g11_t80_lcd_devices[] = {
    &meson_device_lcd,
//    &meson_device_vout,
    &m6g11_t80_backlight_device,
};

int  m6g11_t80_lcd_init(void)
{
    int err;	
	lcd_sync_duration(&m6g11_t80_lcd_config);
	lcd_setup_gamma_table(&m6g11_t80_lcd_config);
	lcd_video_adjust(&m6g11_t80_lcd_config);	
    meson_lcd_set_platdata(&m6g11_t80_lcd_data, sizeof(struct aml_lcd_platform));
    err = platform_add_devices(m6g11_t80_lcd_devices, ARRAY_SIZE(m6g11_t80_lcd_devices));
    return err;
}
