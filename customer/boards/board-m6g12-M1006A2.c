...
//#define CONFIG_BATTERY_SGR241			//动力聚能5900
#define CONFIG_BATTERY_SGR240			//环宇达5900
//#define CONFIG_BATTERY_SGR269			//动力聚能6500		
...
//CONFIG_CAM_DEV(gc0307, gc0307_i2c, CAM_GC0307);
...
static struct aml_camera_i2c_fig1_s gc2035_front_custom_init_script[] = {
    {0x17,0x17}, //0x14   normal , 0x15 h--flip, =0x16  v--flip, =0x17  180度
    {0xff,0xff},
};
...
static int gc2035_v4l2_probe(struct i2c_adapter *adapter)
{
	...
	unsigned char reg[2];
	gc2035_v4l2_init();	
	reg[0] = aml_i2c_get_byte_add8(adapter, 0x78>>1, 0xf0);
	reg[1] = aml_i2c_get_byte_add8(adapter, 0x78>>1, 0xf1);
	printk("reg[0]=%x,reg[1]=%x\n",reg[0],reg[1]);
	if (reg[0] == 0x20 && reg[1] == 0x35)
		ret = 1;
	gc2035_v4l2_uninit();
	printk("gc2035 retttttt=%d\n",ret);
	return ret;
}
...
//    },
//#endif
#ifdef CONFIG_FOCALTECH_CAPACITIVE_TOUCHSCREEN_G12_ANDA_M1005A
    {
